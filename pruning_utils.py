...
def cal_mask_distance_in_self_heads_of_two_step_prune_heads_global(model, self_slimming_coef_records,
    ...
    normalized_dis1 = mindspore.tensor(distance/np.size(layers_masks1))
    normalized_dis2 = 1.0-mindspore.cosine_similarity(mindspore.tensor(layers_masks1.astype(float)).reshape(1,-1),mindspore.tensor(layers_masks2.astype(float)).reshape(1,-1))
