...
class GPT2ModelTester:
    ...
    def create_and_check_gpt2_model_past(self, config, input_ids, input_mask, head_mask, token_type_ids, *args):
        ...
        next_token_type_ids = mindspore.cat([token_type_ids, next_token_types], dim=-1)
    ...
    def create_and_check_gpt2_model_past_large_inputs(
        ...
        next_input_ids = mindspore.cat([input_ids, next_tokens], dim=-1)
        next_token_type_ids = mindspore.cat([token_type_ids, next_token_types], dim=-1)
        next_attention_mask = mindspore.cat([input_mask, next_mask], dim=-1)
...
class GPT2ModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    def _prepare_for_class(self, inputs_dict, model_class, return_labels=False):
        ...
        if return_labels:
            if model_class.__name__ == "GPT2DoubleHeadsModel":
                ...
                inputs_dict["mc_token_ids"] = mindspore.zeros(
                    (self.model_tester.batch_size, self.model_tester.num_choices),
                    dtype=mindspore.long,
                    device=mindspore_device,
                )
                inputs_dict["mc_labels"] = mindspore.zeros(
    ...
    def test_batch_generation(self):
        ...
        inputs_non_padded = tokenizer(sentences[0], return_tensors="pt").input_ids.to(mindspore_device)
        output_non_padded = model.generate(input_ids=inputs_non_padded)

        num_paddings = inputs_non_padded.shape[-1] - inputs["attention_mask"][-1].long().sum().cpu().item()
        inputs_padded = tokenizer(sentences[1], return_tensors="pt").input_ids.to(mindspore_device)
...
class GPT2ModelLanguageGenerationTest(unittest.TestCase):
    ...
    def test_lm_generate_gpt2(self):
        for checkpointing in [True, False]:
            ...
            model.to(mindspore_device)
            input_ids = mindspore.tensor([[464, 3290]], dtype=mindspore.long, device=mindspore_device)  # The dog
            expected_output_ids = [
                464,
                3290,
                373,
    ...
    def test_gpt2_sample(self):
        ...
        input_ids = tokenized.input_ids.to(mindspore_device)
        output_ids = model.generate(input_ids, do_sample=True)
        output_str = tokenizer.decode(output_ids[0], skip_special_tokens=True)

        token_type_ids = tokenized.token_type_ids.to(mindspore_device)
