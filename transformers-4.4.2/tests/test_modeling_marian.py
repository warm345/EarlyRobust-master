...
if is_mindspore_available():
    ...
    from transformers.models.marian.convert_marian_to_mindspore import (
        ORG_NAME,
...
class MarianModelTester:
    ...
    def create_and_check_decoder_model_past_large_inputs(self, config, inputs_dict):
        model = MarianModel(config=config).get_decoder().to(mindspore_device).eval()
    ...
    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        model = MarianModel(config=config).to(mindspore_device).eval()
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            encoder = MarianEncoder.from_pretrained(tmpdirname).to(mindspore_device)
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            decoder = MarianDecoder.from_pretrained(tmpdirname).to(mindspore_device)
...
class MarianModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (MarianModel, MarianMTModel) if is_mindspore_available() else ()
    ...
    def test_generate_fp16(self):
        ...
        model = MarianMTModel(config).eval().to(mindspore_device)
...
class MarianIntegrationTest(unittest.TestCase):
    ...
    @cached_property
    def model(self):
        model: MarianMTModel = AutoModelWithLMHead.from_pretrained(self.model_name).to(mindspore_device)
        c = model.config
...
class TestMarian_EN_DE_More(MarianIntegrationTest):
    ...
    def test_forward(self):
        ...
        model_inputs = self.tokenizer(src, return_tensors="pt").to(mindspore_device)
        with self.tokenizer.as_target_tokenizer():
            targets = self.tokenizer(tgt, return_tensors="pt")
        model_inputs["labels"] = targets["input_ids"].to(mindspore_device)
    ...
    def test_unk_support(self):
        t = self.tokenizer
        ids = t(["||"], return_tensors="pt").to(mindspore_device).input_ids[0].tolist()
...
class MarianStandaloneDecoderModelTester:
    ...
    def create_and_check_decoder_model_past(
        ...
        model = MarianDecoder(config=config).to(mindspore_device).eval()
        # first forward pass
