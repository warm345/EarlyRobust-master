...
from transformers.testing_utils import is_pipeline_test, require_mindspore, slow, mindspore_device
...
DEFAULT_DEVICE_NUM = -1 if mindspore_device == "cpu" else 0
...
class SimpleConversationPipelineTests(unittest.TestCase):
    def get_pipeline(self):
        ...
        bias = mindspore.zeros(V)
        bias[76] = 1
        weight = mindspore.zeros((V, D), requires_grad=True)

        model.lm_head.bias = mindspore.nn.Parameter(bias)
        model.lm_head.weight = mindspore.nn.Parameter(weight)
    ...
    @require_mindspore
    def test_integration_mindspore_conversation(self):
...
class ConversationalPipelineTests(MonoInputPipelineCommonMixin, unittest.TestCase):
    ...
    @require_mindspore
    @slow
    def test_integration_mindspore_conversation_truncated_history(self):
        # When
    ...
    @require_mindspore
    @slow
    def test_integration_mindspore_conversation_dialogpt_input_ids(self):
    ...
    @require_mindspore
    @slow
    def test_integration_mindspore_conversation_blenderbot_400M_input_ids(self):
    ...
    @require_mindspore
    @slow
    def test_integration_mindspore_conversation_blenderbot_400M(self):
        tokenizer = AutoTokenizer.from_pretrained("facebook/blenderbot-400M-distill")
        model = AutoModelForSeq2SeqLM.from_pretrained("facebook/blenderbot-400M-distill")
        nlp = ConversationalPipeline(model=model, tokenizer=tokenizer)
    ...
    @require_mindspore
    @slow
    def test_integration_mindspore_conversation_encoder_decoder(self):
        # When
