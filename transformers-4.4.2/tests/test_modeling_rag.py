...
from transformers.file_utils import cached_property, is_datasets_available, is_faiss_available, is_mindspore_available
...
from transformers.testing_utils import (
    ...
    require_tokenizers,
    require_mindspore,
    require_mindspore_non_multi_gpu,
    slow,
    mindspore_device,
)
...
def require_retrieval(test_case):
    ...
    if not (is_mindspore_available() and is_datasets_available() and is_faiss_available()):
        test_case = unittest.skip("test requires mindspore, datasets and faiss")(test_case)
    return test_case
...
class RagTestMixin:
    ...
    def check_model_with_retriever(
        ...
        for model_class in self.all_model_classes:
            model = model_class(config, retriever=self.get_retriever(config)).to(mindspore_device)
            model.eval()
...
class RagModelIntegrationTests(unittest.TestCase):
    ...
    def test_rag_sequence_inference(self):
        ...
        decoder_input_ids = decoder_input_ids.to(mindspore_device)
        ...
        expected_shape = mindspore.Size([5, 5, 50264])
        self.assertEqual(output.logits.shape, expected_shape)

        expected_doc_scores = mindspore.tensor([[75.0286, 74.4998, 74.0804, 74.0306, 73.9504]]).to(mindspore_device)
        _assert_tensors_equal(expected_doc_scores, output.doc_scores, atol=TOLERANCE)

        expected_loss = mindspore.tensor([36.7368]).to(mindspore_device)
    ...
    def test_rag_token_inference(self):
        ...
        expected_loss = mindspore.tensor([36.3557]).to(mindspore_device)
        _assert_tensors_equal(expected_loss, output.loss, atol=TOLERANCE)
    ...
    def test_rag_sequence_generate_batch(self):
        ...
        input_ids = input_dict.input_ids.to(mindspore_device)
        attention_mask = input_dict.attention_mask.to(mindspore_device)
    ...
    def test_rag_sequence_generate_batch_from_context_input_ids(self):
        ...
        doc_scores = mindspore.bmm(
            ...
            docs_dict["retrieved_doc_embeds"].to(mindspore_device).float().transpose(1, 2),
        ).squeeze(1)

        output_ids = rag_sequence.generate(
            context_input_ids=docs_dict["context_input_ids"].to(mindspore_device),
            context_attention_mask=docs_dict["context_attention_mask"].to(mindspore_device),
            doc_scores=doc_scores.to(mindspore_device),
            do_deduplication=True,
        )
...
class RagModelSaveLoadTests(unittest.TestCase):
    ...
    def test_rag_sequence_from_pretrained(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dirname:
            ...
            rag_sequence.to(mindspore_device)
