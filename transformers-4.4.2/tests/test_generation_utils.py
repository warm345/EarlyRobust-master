...
class GenerationTesterMixin:
    ...
    def _get_input_ids_and_config(self):
        ...
        attention_mask = mindspore.ones_like(input_ids, dtype=mindspore.long)
    ...
    def _get_encoder_outputs(
        ...
        input_ids = mindspore.zeros_like(input_ids[:, :1]) + model._get_decoder_start_token_id()
        attention_mask = None
    ...
    def test_greedy_generate(self):
        ...
        for model_class in self.all_generative_model_classes:
            ...
            model = model_class(config).to(mindspore_device).eval()
    ...
    def test_beam_search_generate_dict_output(self):
        for model_class in self.all_generative_model_classes:
            ...
            self.assertTrue(
                mindspore.allclose(output_generate["sequences_scores"], output_beam_search["sequences_scores"], atol=1e-3)
            )
    ...
    def test_beam_sample_generate_dict_output(self):
        for model_class in self.all_generative_model_classes:
            ...
            self.assertTrue(
                mindspore.allclose(output_generate["sequences_scores"], output_beam_sample["sequences_scores"], atol=1e-3)
            )
            self.assertTrue(output_generate["sequences_scores"].shape == (output_generate["sequences"].shape[0],))
            self.assertTrue((output_generate["sequences_scores"] < 0).all().item())
...
class UtilsFunctionsTest(unittest.TestCase):
    ...
    def test_top_k_top_p_filtering(self):
        ...
        non_inf_expected_idx = mindspore.tensor(
            [[0, 0], [0, 10], [0, 25], [0, 26], [1, 13], [1, 17], [1, 20], [1, 27]],
            dtype=mindspore.long,
            device=mindspore_device,
        )  # expected non filtered idx as noted above

        non_inf_expected_output = mindspore.tensor(
            [
                8.2221,
                8.4321,
                7.4402,
        ...
        non_inf_output = output[output != -float("inf")].to(device=mindspore_device)
        non_inf_idx = (output != -float("inf")).nonzero().to(device=mindspore_device)

        self.assertTrue(mindspore.allclose(non_inf_expected_output, non_inf_output, atol=1e-12))
        self.assertTrue(mindspore.all(mindspore.eq(non_inf_expected_idx, non_inf_idx)))
...
class GenerationIntegrationTests(unittest.TestCase):
    ...
    def test_diverse_beam_search(self):
        ...
        bart_model = BartForConditionalGeneration.from_pretrained("facebook/bart-large-cnn").to(mindspore_device)
        input_ids = bart_tokenizer(article, return_tensors="pt").input_ids.to(mindspore_device)
    ...
    def test_max_length_backward_compat_greedy(self):
        ...
        bart_model = BartForConditionalGeneration.from_pretrained("sshleifer/bart-tiny-random").to(mindspore_device)
        input_ids = bart_tokenizer(article, return_tensors="pt").input_ids.to(mindspore_device)
