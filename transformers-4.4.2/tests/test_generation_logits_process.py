...
class LogitsProcessorTest(unittest.TestCase):
    ...
    def test_min_lenght_dist_processor(self):
        ...
        scores = self._get_uniform_logits(batch_size, vocab_size)
        scores_before_min_length = min_dist_processor(input_ids, scores)
        self.assertFalse(mindspore.isinf(scores_before_min_length).any())
    ...
    def test_temperature_dist_warper(self):
        ...
        self.assertTrue(mindspore.allclose(probs[0, :], warped_prob_sharp[0, :], atol=1e-3))
        self.assertTrue(mindspore.allclose(probs[0, :], warped_prob_smooth[0, :], atol=1e-3))
    ...
    def test_repetition_penalty_dist_process(self):
        input_ids = mindspore.tensor([[0, 1], [5, 0]], device=mindspore_device, dtype=mindspore.long)
        vocab_size = 10
    ...
    def test_top_k_dist_warper(self):
        ...
        ramp_logits = (
            mindspore.arange(vocab_size, device=mindspore_device, dtype=mindspore.float).unsqueeze(0).repeat(batch_size, 1)
        )
        ...
        self.assertListEqual(mindspore.isinf(scores[0]).tolist(), 7 * [True] + 3 * [False])
        self.assertListEqual(mindspore.isinf(scores[1]).tolist(), 2 * [True] + 3 * [False] + 5 * [True])
        ...
        self.assertListEqual((scores == 0.0).to(mindspore.long).sum(dim=-1).tolist(), [0, 0])

        ramp_logits = mindspore.arange(length, device=mindspore_device, dtype=mindspore.float).unsqueeze(0).repeat(batch_size, 1)
        scores = top_k_warp_safety_check(input_ids, ramp_logits)

        # min_tokens overwrites k: 3 tokens are kept => 2 tokens are nullified
        self.assertListEqual((scores == 0.0).to(mindspore.long).sum(dim=-1).tolist(), [2, 2])
    ...
    def test_top_p_dist_warper(self):
        ...
        dist = mindspore.log(
            mindspore.tensor([[0.3, 0.1, 0.1, 0.5], [0.15, 0.3, 0.3, 0.25]], device=mindspore_device, dtype=mindspore.float)
        )

        top_p_warp = TopPLogitsWarper(0.7)
        filtered_dist = mindspore.exp(top_p_warp(input_ids, dist))

        # dist should be filtered to keep min num values so that sum is >= 0.7
        # exp (-inf) => 0
        EXPECTED_FILTERED_DIST = mindspore.tensor(
            [[0.3, 0.0, 0.0, 0.5], [0.0, 0.3, 0.3, 0.25]], device=mindspore_device, dtype=mindspore.float
        )
        self.assertTrue(mindspore.allclose(filtered_dist, EXPECTED_FILTERED_DIST, atol=1e-3))

        # check edge cases with negative and extreme logits
        ramp_logits = mindspore.arange(vocab_size, device=mindspore_device, dtype=mindspore.float).unsqueeze(0).repeat(
            batch_size, 1
        ) - (vocab_size // 2)
        ...
        self.assertListEqual((filtered_dist != 0.0).to(mindspore.long).sum(dim=-1).tolist(), [3, 2])

    def test_no_repeat_ngram_dist_processor(self):
        vocab_size = 3
        batch_size = 2

        input_ids = mindspore.tensor([[1, 1, 2, 1], [0, 1, 0, 1]], device=mindspore_device, dtype=mindspore.long)
        scores = self._get_uniform_logits(batch_size, vocab_size)
        ...
        self.assertListEqual(mindspore.isinf(filtered_scores_2_gram).tolist(), [[False, True, True], [True, False, False]])

        # 3-gram would forbid no token at 1st batch and 1st token (0) at 2nd batch
        self.assertListEqual(
            mindspore.isinf(filtered_scores_3_gram).tolist(), [[False, False, False], [True, False, False]]
        )
    ...
    def test_encoder_no_repeat_ngram_dist_processor(self):
        ...
        encoder_input_ids = mindspore.tensor([1, 2, 1, 1], device=mindspore_device, dtype=mindspore.long)

        input_ids = mindspore.tensor([[1, 2, 1], [8, 0, 2]], device=mindspore_device, dtype=mindspore.long)
        ...
        self.assertListEqual(mindspore.isinf(filtered_scores_2_gram).tolist(), [[False, True, True], [False, True, False]])

        # 3-gram would forbid 1st token at 1st beam and no token at 2nd beam
        self.assertListEqual(
            mindspore.isinf(filtered_scores_3_gram).tolist(), [[False, True, False], [False, False, False]]
        )
        ...
        num_beams = 2
        batch_size = 2
        encoder_input_ids = mindspore.tensor([[1, 2, 1, 1], [0, 0, 2, 1]], device=mindspore_device, dtype=mindspore.long)

        input_ids = mindspore.tensor([[1, 2, 1], [1, 0, 2], [0, 0, 0], [0, 2, 2]], device=mindspore_device, dtype=mindspore.long)
        scores = self._get_uniform_logits(batch_size * num_beams, vocab_size)
        ...
        #   - Beam 2: tokens (1) forbidden
        self.assertListEqual(
            mindspore.isinf(filtered_scores_2_gram).tolist(),
        ...
        #   - Beam 2: tokens () forbidden
        self.assertListEqual(
            mindspore.isinf(filtered_scores_3_gram).tolist(),
    ...
    def test_no_bad_words_dist_processor(self):
        ...
        input_ids = mindspore.tensor([[0, 1, 3, 1], [0, 1, 0, 1]], device=mindspore_device, dtype=mindspore.long)
        ...
        self.assertListEqual(
            mindspore.isinf(filtered_scores).tolist(), [[True, True, False, True, False], [True, True, True, False, False]]
        )

        # check edge case
        no_bad_words_dist_proc = NoBadWordsLogitsProcessor(bad_words_ids=[[4]], eos_token_id=eos_token_id)
        filtered_scores = no_bad_words_dist_proc(input_ids, scores.clone())
        self.assertTrue(mindspore.allclose(scores, filtered_scores, atol=1e-3))
    ...
    def test_processor_list(self):
        ...
        self.assertTrue(mindspore.allclose(scores, scores_comp, atol=1e-3))
    ...
    def test_prefix_constrained_logits_processor(self):
        ...
        self.assertListEqual(
            mindspore.isinf(filtered_scores).tolist(), [[False, False, True, True, True], [True, True, False, False, True]]
        )
    ...
    def test_hamming_diversity(self):
        ...
        current_tokens = mindspore.tensor([0, 3, 1, 2], device=mindspore_device, dtype=mindspore.long)
        ...
        self.assertTrue(
            mindspore.allclose(
                processed_scores[0], mindspore.tensor([-0.7500, 0.2500, 0.2500, 0.2500], device=mindspore_device), atol=1e-3
            )
        )
        self.assertTrue(
            mindspore.allclose(
                processed_scores[1], mindspore.tensor([0.2500, -0.7500, 0.2500, 0.2500], device=mindspore_device), atol=1e-3
            )
        )
    ...
    def test_forced_bos_token_logits_processor(self):
        ...
        self.assertTrue(mindspore.isneginf(scores[:, bos_token_id + 1 :]).all())
        ...
        scores = self._get_uniform_logits(batch_size, vocab_size)
        scores = logits_processor(input_ids, scores)
        self.assertFalse(mindspore.isinf(scores).any())
    ...
    def test_forced_eos_token_logits_processor(self):
        ...
        scores = self._get_uniform_logits(batch_size, vocab_size)
        scores = logits_processor(input_ids, scores)
        self.assertTrue(mindspore.isneginf(scores[:, eos_token_id + 1 :]).all())
