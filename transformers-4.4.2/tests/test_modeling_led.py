...
class LEDModelTester:
    ...
    def prepare_config_and_inputs_for_common(self):
        ...
        global_attention_mask = mindspore.zeros_like(inputs_dict["input_ids"])
    ...
    def create_and_check_decoder_model_past_large_inputs(self, config, inputs_dict):
        model = LEDModel(config=config).get_decoder().to(mindspore_device).eval()
        ...
        self.parent.assertTrue(mindspore.allclose(output_from_past_slice, output_from_no_past_slice, atol=1e-2))

    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        model = LEDModel(config=config).to(mindspore_device).eval()
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            encoder = LEDEncoder.from_pretrained(tmpdirname).to(mindspore_device)
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            decoder = LEDDecoder.from_pretrained(tmpdirname).to(mindspore_device)
    ...
    def check_global_attention(self, config, inputs_dict):
        ...
        global_attention_mask = mindspore.zeros_like(attention_mask)
...
class LEDModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    def test_generate_fp16(self):
        ...
        model = LEDForConditionalGeneration(config).eval().to(mindspore_device)
...
class LEDModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        model = LEDModel.from_pretrained("allenai/led-base-16384").to(mindspore_device)
        ...
        expected_shape = mindspore.Size((1, 1024, 768))
        self.assertEqual(output.shape, expected_shape)
        # change to expected output here
        expected_slice = mindspore.tensor(
            [[2.3050, 2.8279, 0.6531], [-1.8457, -0.1455, -3.5661], [-1.0186, 0.4586, -2.2043]], device=mindspore_device
        )
        self.assertTrue(mindspore.allclose(output[:, :3, :3], expected_slice, atol=TOLERANCE))

    def test_inference_head(self):
        model = LEDForConditionalGeneration.from_pretrained("allenai/led-base-16384").to(mindspore_device)
        ...
        expected_shape = mindspore.Size((1, 1024, model.config.vocab_size))
        self.assertEqual(output.shape, expected_shape)
        # change to expected output here
        expected_slice = mindspore.tensor(
            [[33.6507, 6.4572, 16.8089], [5.8739, -2.4238, 11.2902], [-3.2139, -4.3149, 4.2783]], device=mindspore_device
        )
        self.assertTrue(mindspore.allclose(output[:, :3, :3], expected_slice, atol=TOLERANCE))

    def test_seq_to_seq_generation(self):
        # this test requires 16GB of RAM
        hf = LEDForConditionalGeneration.from_pretrained("allenai/led-large-16384-arxiv").to(mindspore_device)
