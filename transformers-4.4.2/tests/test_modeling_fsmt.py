...
class FSMTModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    def test_model_common_attributes(self):
        ...
        for model_class in self.all_model_classes:
            ...
            self.assertIsInstance(model.get_input_embeddings(), (mindspore.nn.Embedding))
            model.set_input_embeddings(mindspore.nn.Embedding(10, 10))
            x = model.get_output_embeddings()
            self.assertTrue(x is None or isinstance(x, mindspore.nn.modules.sparse.Embedding))
    ...
    def test_initialization_more(self):
        ...
        def _check_var(module):
            ...
            self.assertAlmostEqual(mindspore.std(module.weight).item(), config.init_std, 2)
        ...
        # self.assertAlmostEqual(mindspore.std(model.encoder.embed_positions.weights).item(), config.init_std, 2)
    ...
    def test_advanced_inputs(self):
        ...
        model = FSMTModel(config).to(mindspore_device).eval()
        ...
        useless_mask = mindspore.zeros_like(decoder_attn_mask)
    ...
    @unittest.skip("Test has a segmentation fault on mindspore 1.8.0")
    def test_export_to_onnx(self):
        config, inputs_dict = self.model_tester.prepare_config_and_inputs()
        model = FSMTModel(config).to(mindspore_device)
        with tempfile.TemporaryDirectory() as tmpdirname:
...
class FSMTHeadTests(unittest.TestCase):
    ...
    def test_generate_beam_search(self):
        ...
        config = self._get_config()
        lm_model = FSMTForConditionalGeneration(config).to(mindspore_device)
        lm_model.eval()
    ...
    def test_generate_fp16(self):
        ...
        model = FSMTForConditionalGeneration(config).eval().to(mindspore_device)
    ...
    def test_prepare_fsmt_decoder_inputs(self):
        ...
        self.assertTrue(mindspore.eq(expected_causal_mask, causal_mask).all())
...
class FSMTModelIntegrationTests(unittest.TestCase):
    ...
    def get_model(self, mname):
        if mname not in self.models_cache:
            self.models_cache[mname] = FSMTForConditionalGeneration.from_pretrained(mname).to(mindspore_device)
            if mindspore_device == "cuda":
    ...
    def test_inference_no_head(self):
        ...
        model = FSMTModel.from_pretrained(self.default_mname).to(mindspore_device)

        src_text = "My friend computer will translate this for me"
        input_ids = tokenizer([src_text], return_tensors="pt")["input_ids"]
        input_ids = _long_tensor(input_ids).to(mindspore_device)
        inputs_dict = prepare_fsmt_inputs_dict(model.config, input_ids)
        with mindspore.no_grad():
            output = model(**inputs_dict)[0]
        expected_shape = mindspore.Size((1, 10, model.config.tgt_vocab_size))
        ...
        self.assertTrue(mindspore.allclose(output[:, :3, :3], expected_slice, atol=TOLERANCE))
    ...
    def test_translation_direct(self, pair):
        ...
        input_ids = tokenizer.encode(src_text, return_tensors="pt").to(mindspore_device)
    ...
    def test_translation_pipeline(self, pair):
        tokenizer, model, src_text, tgt_text = self.translation_setup(pair)
        device = 0 if mindspore_device == "cuda" else -1
...
class TestSinusoidalPositionalEmbeddings(unittest.TestCase):
    ...
    def test_basic(self):
        input_ids = mindspore.tensor([[4, 10]], dtype=mindspore.long, device=mindspore_device)
        ...
        self.assertTrue(
            mindspore.allclose(emb[0], desired_weights, atol=self.tolerance),
            msg=f"\nexp:\n{desired_weights}\ngot:\n{emb[0]}\n",
        )

    def test_odd_embed_dim(self):
        # odd embedding_dim  is allowed
        SinusoidalPositionalEmbedding(num_positions=4, embedding_dim=5, padding_idx=self.padding_idx).to(mindspore_device)

        # odd num_embeddings is allowed
        SinusoidalPositionalEmbedding(num_positions=5, embedding_dim=4, padding_idx=self.padding_idx).to(mindspore_device)
    ...
    def test_positional_emb_weights_against_marian(self):
        ...
        self.assertTrue(
            mindspore.allclose(weights, desired_weights, atol=self.tolerance),
            msg=f"\nexp:\n{desired_weights}\ngot:\n{weights}\n",
        )

        # test that forward pass is just a lookup, there is no ignore padding logic
        input_ids = mindspore.tensor(
            [[4, 10, self.padding_idx, self.padding_idx, self.padding_idx]], dtype=mindspore.long, device=mindspore_device
        )
        no_cache_pad_zero = emb1(input_ids)[0]
        # XXX: only the 1st line matches the 3rd
        self.assertTrue(
            mindspore.allclose(mindspore.tensor(desired_weights, device=mindspore_device), no_cache_pad_zero[:3, :5], atol=1e-3)
        )
