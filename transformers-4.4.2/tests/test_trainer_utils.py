...
from transformers.file_utils import is_mindspore_available
...
class TrainerUtilsTest(unittest.TestCase):
    ...
    def test_label_smoothing(self):
        epsilon = 0.1
        num_labels = 12
        random_logits = mindspore.randn(4, 5, num_labels)
        random_labels = mindspore.randint(0, num_labels, (4, 5))
        loss = mindspore.nn.functional.cross_entropy(random_logits.view(-1, num_labels), random_labels.view(-1))
        ...
        log_probs = -mindspore.nn.functional.log_softmax(random_logits, dim=-1)
        expected_loss = (1 - epsilon) * loss + epsilon * log_probs.mean()
        self.assertTrue(mindspore.allclose(label_smoothed_loss, expected_loss))
    ...
    def test_group_by_length(self):
        ...
        lengths = mindspore.randint(0, 25, (100,)).tolist()
    ...
    def test_get_parameter_names(self):
        ...
        # fmt: off
        self.assertEqual(
            get_parameter_names(model, [mindspore.nn.LayerNorm]),
