...
""" Testing suite for the mindspore M2M100 model. """
...
class M2M100ModelTester:
    ...
    def create_and_check_decoder_model_past_large_inputs(self, config, inputs_dict):
        model = M2M100Model(config=config).get_decoder().to(mindspore_device).eval()
    ...
    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        model = M2M100Model(config=config).to(mindspore_device).eval()
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            encoder = M2M100Encoder.from_pretrained(tmpdirname).to(mindspore_device)
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            decoder = M2M100Decoder.from_pretrained(tmpdirname).to(mindspore_device)
...
class M2M100ModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    def test_generate_fp16(self):
        ...
        model = M2M100ForConditionalGeneration(config).eval().to(mindspore_device)
...
class M2M100ModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        model = M2M100Model.from_pretrained("facebook/m2m100_418M").to(mindspore_device)
        ...
        expected_slice = mindspore.tensor(
            [[-0.7780, -0.1676, 0.1038], [-6.7556, -1.3992, 0.0567], [-7.5383, -0.5920, -0.2779]], device=mindspore_device
        )
        self.assertTrue(mindspore.allclose(output[:, :3, :3], expected_slice, atol=TOLERANCE))

    def test_inference_head(self):
        model = M2M100ForConditionalGeneration.from_pretrained("facebook/m2m100_418M").to(mindspore_device)
        ...
        with mindspore.no_grad():
            output = model(**inputs_dict)[0]
        expected_shape = mindspore.Size((1, 11, model.config.vocab_size))
        self.assertEqual(output.shape, expected_shape)
        # change to expected output here
        expected_slice = mindspore.tensor(
            [[-1.0448, -1.0411, 3.7992], [-3.2191, -3.2386, -1.3451], [-3.6210, -3.5993, 0.4925]], device=mindspore_device
        )
        self.assertTrue(mindspore.allclose(output[:, :3, :3], expected_slice, atol=TOLERANCE))
