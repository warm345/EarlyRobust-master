...
def prepare_bart_inputs_dict(
    ...
    if head_mask is None:
        head_mask = mindspore.ones(config.encoder_layers, config.encoder_attention_heads, device=mindspore_device)
    if decoder_head_mask is None:
        decoder_head_mask = mindspore.ones(config.decoder_layers, config.decoder_attention_heads, device=mindspore_device)
    return {
        "input_ids": input_ids,
...
class BartModelTester:
    ...
    def create_and_check_decoder_model_past_large_inputs(self, config, inputs_dict):
        model = BartModel(config=config).get_decoder().to(mindspore_device).eval()
        ...
        next_attention_mask = mindspore.cat([attention_mask, next_attn_mask], dim=-1)
        ...
        self.parent.assertTrue(mindspore.allclose(output_from_past_slice, output_from_no_past_slice, atol=1e-3))

    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        model = BartModel(config=config).to(mindspore_device).eval()
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            encoder = BartEncoder.from_pretrained(tmpdirname).to(mindspore_device)
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            decoder = BartDecoder.from_pretrained(tmpdirname).to(mindspore_device)
...
class BartHeadTests(unittest.TestCase):
    ...
    def test_sequence_classification_forward(self):
        ...
        labels = _long_tensor([2] * batch_size).to(mindspore_device)
        model = BartForSequenceClassification(config)
        model.to(mindspore_device)
        outputs = model(input_ids=input_ids, decoder_input_ids=input_ids, labels=labels)
        expected_shape = mindspore.Size((batch_size, config.num_labels))
        self.assertEqual(outputs["logits"].shape, expected_shape)
        self.assertIsInstance(outputs["loss"].item(), float)

    def test_question_answering_forward(self):
        config, input_ids, batch_size = self._get_config_and_data()
        sequence_labels = ids_tensor([batch_size], 2).to(mindspore_device)
    ...
    def test_lm_forward(self):
        config, input_ids, batch_size = self._get_config_and_data()
        lm_labels = ids_tensor([batch_size, input_ids.shape[1]], self.vocab_size).to(mindspore_device)
        lm_model = BartForConditionalGeneration(config)
        lm_model.to(mindspore_device)
    ...
    def test_lm_uneven_forward(self):
        ...
        lm_model = BartForConditionalGeneration(config).to(mindspore_device)
        context = mindspore.Tensor([[71, 82, 18, 33, 46, 91, 2], [68, 34, 26, 58, 30, 2, 1]]).long().to(mindspore_device)
        summary = mindspore.Tensor([[82, 71, 82, 18, 2], [58, 68, 2, 1, 1]]).long().to(mindspore_device)
        outputs = lm_model(input_ids=context, decoder_input_ids=summary, labels=summary)
        expected_shape = (*summary.shape, config.vocab_size)
        self.assertEqual(outputs["logits"].shape, expected_shape)

    def test_generate_beam_search(self):
        input_ids = mindspore.Tensor([[71, 82, 2], [68, 34, 2]]).long().to(mindspore_device)
        config = BartConfig(
            vocab_size=self.vocab_size,
            d_model=24,
            encoder_layers=2,
    ...
    def test_shift_tokens_right(self):
        input_ids = mindspore.Tensor([[71, 82, 18, 33, 2, 1, 1], [68, 34, 26, 58, 30, 82, 2]]).long()
        shifted = shift_tokens_right(input_ids, 1, 2)
        n_pad_before = input_ids.eq(1).float().sum()
        n_pad_after = shifted.eq(1).float().sum()
        self.assertEqual(shifted.shape, input_ids.shape)
        self.assertEqual(n_pad_after, n_pad_before - 1)
        self.assertTrue(mindspore.eq(shifted[:, 0], 2).all())
    ...
    def test_generate_fp16(self):
        config, input_ids, batch_size = self._get_config_and_data()
        attention_mask = input_ids.ne(1).to(mindspore_device)
        model = BartForConditionalGeneration(config).eval().to(mindspore_device)
        if mindspore_device == "cuda":
            model.half()
    ...
    def test_resize_tokens_embeddings_more(self):
        ...
        self.assertTrue(mindspore.eq(input, output).all())
        new_vocab_size = 45
        ...
        self.assertTrue(mindspore.eq(input_new, output_new).all())
...
def assert_tensors_close(a, b, atol=1e-12, prefix=""):
    ...
    if a is None and b is None:
        return True
    try:
        if mindspore.allclose(a, b, atol=atol):
            return True
        raise
    except Exception:
        pct_different = (mindspore.gt((a - b).abs(), atol)).float().mean().item()
        if a.numel() > 100:
...
def _long_tensor(tok_lst):
    return mindspore.tensor(tok_lst, dtype=mindspore.long, device=mindspore_device)
...
class BartModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        model = BartModel.from_pretrained("facebook/bart-large").to(mindspore_device)
        input_ids = _long_tensor([[0, 31414, 232, 328, 740, 1140, 12695, 69, 46078, 1588, 2]])
        attention_mask = input_ids.ne(model.config.pad_token_id)
        with mindspore.no_grad():
            output = model(input_ids=input_ids, attention_mask=attention_mask).last_hidden_state
        expected_shape = mindspore.Size((1, 11, 1024))
        self.assertEqual(output.shape, expected_shape)
        expected_slice = mindspore.tensor(
            [[0.7144, 0.8143, -1.2813], [0.7144, 0.8143, -1.2813], [-0.0467, 2.5911, -2.1845]], device=mindspore_device
        )
        self.assertTrue(mindspore.allclose(output[:, :3, :3], expected_slice, atol=1e-3))
    ...
    def test_mnli_inference(self):
        ...
        expected_shape = mindspore.Size((2, 3))
        self.assertEqual(batched_logits.shape, expected_shape)
        expected_slice = mindspore.Tensor([[0.1907, 1.4342, -1.0289]]).to(mindspore_device)
    ...
    def test_xsum_summarization_same_as_fairseq(self):
        model = BartForConditionalGeneration.from_pretrained("facebook/bart-large-xsum").to(mindspore_device)
    ...
    def test_cnn_summarization_same_as_fairseq(self):
        hf = BartForConditionalGeneration.from_pretrained("facebook/bart-large-cnn").to(mindspore_device)
        ...
        hypotheses_batch = hf.generate(
            input_ids=dct["input_ids"].to(mindspore_device),
            attention_mask=dct["attention_mask"].to(mindspore_device),
            num_beams=2,
        )
...
class BartStandaloneDecoderModelTester:
    ...
    def create_and_check_decoder_model_past(
        ...
        model = BartDecoder(config=config).to(mindspore_device).eval()
        # first forward pass
