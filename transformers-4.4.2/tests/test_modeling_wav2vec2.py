...
from transformers.testing_utils import require_datasets, require_soundfile, require_mindspore, slow, mindspore_device
...
class Wav2Vec2ModelTester:
    ...
    def create_and_check_batch_inference(self, config, input_values, *args):
        ...
        # check: https://github.com/mindspore/fairseq/issues/3227
        model = Wav2Vec2Model(config=config)
        model.to(mindspore_device)
        model.eval()

        input_values = input_values[:3]
        attention_mask = mindspore.ones(input_values.shape, device=mindspore_device, dtype=mindspore.bool)
        ...
        for i in range(input_values.shape[0]):
            ...
            self.parent.assertTrue(mindspore.allclose(output, batch_output, atol=1e-3))
...
class Wav2Vec2ModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def test_retain_grad_hidden_states_attentions(self):
        ...
        input_lengths = mindspore.tensor(
            [input_values.shape[1] for _ in range(input_values.shape[0])], dtype=mindspore.long, device=mindspore_device
        )
...
class Wav2Vec2RobustModelTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (Wav2Vec2ForCTC, Wav2Vec2Model, Wav2Vec2ForMaskedLM) if is_mindspore_available() else ()
    test_pruning = False
    test_headmasking = False
    test_mindsporescript = False
...
class Wav2Vec2UtilsTest(unittest.TestCase):
    def test_compute_mask_indices(self):
        ...
        attention_mask = mindspore.ones((batch_size, sequence_length), device=mindspore_device, dtype=mindspore.long)
...
class Wav2Vec2ModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_ctc_normal(self):
        ...
        input_values = processor(input_speech, return_tensors="pt").input_values.to(mindspore_device)
    ...
    def test_inference_ctc_normal_batched(self):
        ...
        input_values = inputs.input_values.to(mindspore_device)
    ...
    def test_inference_ctc_robust_batched(self):
        model = Wav2Vec2ForCTC.from_pretrained("facebook/wav2vec2-large-960h-lv60-self").to(mindspore_device)
