...
class PegasusModelTester:
    ...
    def create_and_check_decoder_model_past_large_inputs(self, config, inputs_dict):
        model = PegasusModel(config=config).get_decoder().to(mindspore_device).eval()
    ...
    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        model = PegasusModel(config=config).to(mindspore_device).eval()
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            encoder = PegasusEncoder.from_pretrained(tmpdirname).to(mindspore_device)
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            decoder = PegasusDecoder.from_pretrained(tmpdirname).to(mindspore_device)
...
class PegasusModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    def test_generate_fp16(self):
        ...
        model = PegasusForConditionalGeneration(config).eval().to(mindspore_device)
...
class PegasusXSUMIntegrationTest(AbstractSeq2SeqIntegrationTest):
    ...
    @cached_property
    def model(self):
        return AutoModelForSeq2SeqLM.from_pretrained(self.checkpoint_name).to(mindspore_device)
    ...
    def test_pegasus_xsum_summary(self):
        ...
        if "cuda" not in mindspore_device:
            return
...
class PegasusStandaloneDecoderModelTester:
    ...
    def create_and_check_decoder_model_past(
        ...
        model = PegasusDecoder(config=config).to(mindspore_device).eval()
        # first forward pass
