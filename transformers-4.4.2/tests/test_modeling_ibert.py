...
class IBertModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def test_create_position_ids_respects_padding_index(self):
        ...
        input_ids = mindspore.as_tensor([[12, 31, 13, model.padding_idx]])
        expected_positions = mindspore.as_tensor(
            [[0 + model.padding_idx + 1, 1 + model.padding_idx + 1, 2 + model.padding_idx + 1, model.padding_idx]]
        )

        position_ids = create_position_ids_from_input_ids(input_ids, model.padding_idx)
        self.assertEqual(position_ids.shape, expected_positions.shape)
        self.assertTrue(mindspore.all(mindspore.eq(position_ids, expected_positions)))
    ...
    def test_create_position_ids_from_inputs_embeds(self):
        ...
        inputs_embeds = mindspore.Tensor(2, 4, 30)
        ...
        expected_positions = mindspore.as_tensor([expected_single_positions, expected_single_positions])
    ...
    def test_model_common_attributes(self):
        ...
        for model_class in self.all_model_classes:
            ...
            model.set_input_embeddings(mindspore.nn.Embedding(10, 10))
            x = model.get_output_embeddings()
            self.assertTrue(x is None or isinstance(x, mindspore.nn.Linear))
...
class IBertModelIntegrationTest(unittest.TestCase):
    def test_quant_embedding(self):
        ...
        embedding.weight = mindspore.nn.Parameter(embedding_weight)
        ...
        self.assertTrue(mindspore.allclose(x_scaling_factor, expected_scaling_factor, atol=1e-4))
        self.assertTrue(mindspore.allclose(x_scaling_factor, expected_scaling_factor, atol=1e-4))
        self.assertTrue(mindspore.allclose(y_scaling_factor, expected_scaling_factor, atol=1e-4))

        # quantization error should not exceed the scaling factor
        self.assertTrue(mindspore.allclose(x, embedding_weight[0], atol=expected_scaling_factor))
        self.assertTrue(mindspore.allclose(y, embedding_weight[1], atol=expected_scaling_factor))
    ...
    def test_quant_act(self):
        def _test_range():
            ...
            self.assertTrue(mindspore.allclose(act.x_min, expected_x_min, atol=1e-4))
            self.assertTrue(mindspore.allclose(act.x_max, expected_x_max, atol=1e-4))

            # scaling factor should follow the symmetric quantization rule
            expected_range = mindspore.max(expected_x_min.abs(), expected_x_max.abs())
            expected_scaling_factor = expected_range / (2 ** (activation_bit - 1) - 1)
            self.assertTrue(mindspore.allclose(y_scaling_factor, expected_scaling_factor, atol=1e-4))

            # quantization error should not exceed the scaling factor
            self.assertTrue(mindspore.allclose(x, y, atol=expected_scaling_factor))

            # output should be integer
            self.assertTrue(mindspore.allclose(y_int, y_int.round(), atol=1e-4))
        ...
        def _test_identity():
            ...
            self.assertTrue(mindspore.allclose(x + y, z, atol=0.1))
            self.assertTrue(mindspore.allclose(z_int, z_int.round(), atol=1e-4))
    ...
    def test_quant_linear(self):
        def _test(per_channel):
            ...
            linear_q.weight = mindspore.nn.Parameter(linear_weight)
            linear_dq.weight = mindspore.nn.Parameter(linear_weight)
            ...
            # scaling factor should follow the symmetric quantization rule
            self.assertTrue(mindspore.allclose(linear_q.fc_scaling_factor, expected_scaling_factor, atol=1e-4))

            # output of the normal linear layer and the quantized linear layer should be similar
            self.assertTrue(mindspore.allclose(q, dq, atol=0.5))

            # output of the quantized linear layer should be integer
            self.assertTrue(mindspore.allclose(q_int, q_int.round(), atol=1e-4))
    ...
    def test_int_gelu(self):
        ...
        gelu_dq = mindspore.nn.GELU()

        x_int = mindspore.range(-10000, 10000, 1)
    ...
    def test_force_dequant_gelu(self):
        ...
        for label, gelu_fdqs in gelu_fdqs_dict.items():
            for gelu_fdq in gelu_fdqs:
                ...
                if label:
                    self.assertTrue(mindspore.allclose(q, dq, atol=1e-4))
                else:
                    self.assertFalse(mindspore.allclose(q, dq, atol=1e-4))

    def test_int_softmax(self):
        output_bit = 8
        softmax_q = IntSoftmax(output_bit, quant_mode=True)
        softmax_dq = mindspore.nn.Softmax()
    ...
    def test_int_layernorm(self):
        ...
        ln_dq = mindspore.nn.LayerNorm(x.shape[1:], 1e-5)

        ln_q.weight = mindspore.nn.Parameter(mindspore.ones(x.shape[1:]))
        ln_q.bias = mindspore.nn.Parameter(mindspore.ones(x.shape[1:]))
        ln_dq.weight = mindspore.nn.Parameter(mindspore.ones(x.shape[1:]))
        ln_dq.bias = mindspore.nn.Parameter(mindspore.ones(x.shape[1:]))
    ...
    def test_force_dequant_layernorm(self):
        ...
        for label, ln_fdqs in ln_fdqs_dict.items():
            for ln_fdq in ln_fdqs:
                ln_fdq.weight = mindspore.nn.Parameter(mindspore.ones(x.shape[1:]))
                ln_fdq.bias = mindspore.nn.Parameter(mindspore.ones(x.shape[1:]))
    ...
    def test_inference_masked_lm(self):
        ...
        expected_shape = mindspore.Size((1, 11, 50265))
        ...
        output = model(input_ids)[0]
        self.assertEqual(output.shape, expected_shape)
        self.assertTrue(mindspore.allclose(output[:, :3, :3], expected_slice, atol=0.1))
    ...
    def test_inference_classification_head(self):
        ...
        input_ids = mindspore.tensor([[0, 31414, 232, 328, 740, 1140, 12695, 69, 46078, 1588, 2]])
        output = model(input_ids)[0]
        expected_shape = mindspore.Size((1, 3))
        self.assertEqual(output.shape, expected_shape)
        expected_tensor = mindspore.tensor([[-0.9469, 0.3913, 0.5118]])
        self.assertTrue(mindspore.allclose(output, expected_tensor, atol=1e-4))

        # I-BERT should be "similar" to RoBERTa if quantized
        self.quantize(model)
        output = model(input_ids)[0]
        self.assertEqual(output.shape, expected_shape)
        self.assertTrue(mindspore.allclose(output, expected_tensor, atol=0.1))
