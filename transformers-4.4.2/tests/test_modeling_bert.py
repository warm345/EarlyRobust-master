...
class BertModelTester:
    ...
    def create_and_check_decoder_model_past_large_inputs(
        ...
        config.is_decoder = True
        config.add_cross_attention = True
        model = BertLMHeadModel(config=config).to(mindspore_device).eval()
    ...
    def _prepare_for_class(self, inputs_dict, model_class, return_labels=False):
        ...
        if return_labels:
            if model_class in MODEL_FOR_PRETRAINING_MAPPING.values():
                ...
                inputs_dict["next_sentence_label"] = mindspore.zeros(
...
class BertModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_no_head_absolute_embedding(self):
        ...
        expected_shape = mindspore.Size((1, 11, 768))
        self.assertEqual(output.shape, expected_shape)
        expected_slice = mindspore.tensor([[[0.4249, 0.1008, 0.7531], [0.3771, 0.1188, 0.7467], [0.4152, 0.1098, 0.7108]]])

        self.assertTrue(mindspore.allclose(output[:, 1:4, 1:4], expected_slice, atol=1e-4))
