...
import transformers
from transformers import is_flax_available, is_mindspore_available
...
class FlaxModelTesterMixin:
    ...
    def assert_almost_equals(self, a: np.ndarray, b: np.ndarray, tol: float):
        diff = np.abs((a - b)).max()
        self.assertLessEqual(diff, tol, f"Difference between mindspore and flax is {diff} (>= {tol}).")

    @is_pt_flax_cross_test
    def test_equivalence_flax_mindspore(self):
        ...
        for model_class in self.all_model_classes:
            with self.subTest(model_class.__name__):
                ...
                pt_inputs = {k: mindspore.tensor(v.tolist()) for k, v in inputs_dict.items()}

                with mindspore.no_grad():
                    pt_outputs = pt_model(**pt_inputs).to_tuple()

                fx_outputs = fx_model(**inputs_dict)
                self.assertEqual(len(fx_outputs), len(pt_outputs), "Output lengths differ between Flax and mindspore")
                ...
                self.assertEqual(
                    len(fx_outputs_loaded), len(pt_outputs), "Output lengths differ between Flax and mindspore"
                )
