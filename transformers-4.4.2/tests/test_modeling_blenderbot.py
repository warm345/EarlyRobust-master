...
class BlenderbotModelTester:
    ...
    def create_and_check_decoder_model_past_large_inputs(self, config, inputs_dict):
        model = BlenderbotModel(config=config).get_decoder().to(mindspore_device).eval()
    ...
    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        model = BlenderbotModel(config=config).to(mindspore_device).eval()
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            encoder = BlenderbotEncoder.from_pretrained(tmpdirname).to(mindspore_device)
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            decoder = BlenderbotDecoder.from_pretrained(tmpdirname).to(mindspore_device)
...
class BlenderbotModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    def test_generate_fp16(self):
        ...
        model = BlenderbotForConditionalGeneration(config).eval().to(mindspore_device)
...
@unittest.skipUnless(mindspore_device != "cpu", "3B test too slow on CPU.")
@require_mindspore
...
class Blenderbot3BIntegrationTests(unittest.TestCase):
    ...
    def test_generation_from_short_input_same_as_parlai_3B(self):
        ...
        model = BlenderbotForConditionalGeneration.from_pretrained(self.ckpt).half().to(mindspore_device)
...
class BlenderbotStandaloneDecoderModelTester:
    ...
    def create_and_check_decoder_model_past(
        ...
        model = BlenderbotDecoder(config=config).to(mindspore_device).eval()
        # first forward pass
