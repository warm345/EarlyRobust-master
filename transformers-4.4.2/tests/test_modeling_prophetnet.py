...
from transformers.testing_utils import require_mindspore, slow, mindspore_device
...
class ProphetNetModelTester:
    ...
    def check_prepare_lm_labels_via_shift_left(
        ...
        triangular_mask = mindspore.tril(lm_labels.new_ones(lm_labels.shape)).logical_not()
    ...
    def create_and_check_with_lm_head(
        ...
        model = ProphetNetForConditionalGeneration(config=config).to(mindspore_device).eval()
        outputs = model(
            input_ids=input_ids,
            decoder_input_ids=decoder_input_ids,
            decoder_attention_mask=decoder_attention_mask,
    ...
    def create_and_check_causal_lm_decoder(
        ...
        model = ProphetNetForCausalLM(config=config).to(mindspore_device).eval()
        outputs = model(
    ...
    def create_and_check_generate_with_past_key_value_states(
        ...
        self.parent.assertTrue(mindspore.all(output_with_past_cache == output_without_past_cache))
    ...
    def create_and_check_model_fp16_forward(
        ...
        model = ProphetNetModel(config=config).to(mindspore_device).half().eval()
        output = model(input_ids, decoder_input_ids=input_ids, attention_mask=attention_mask)["last_hidden_state"]
        self.parent.assertFalse(mindspore.isnan(output).any().item())
    ...
    def create_and_check_encoder_decoder_shared_weights(
        ...
        for model_class in [ProphetNetModel, ProphetNetForConditionalGeneration]:
            mindspore.manual_seed(0)
            model = model_class(config=config).to(mindspore_device).eval()
            ...
            tied_model = model_class(config=tied_config).to(mindspore_device).eval()
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                tied_model.to(mindspore_device)
                tied_model.eval()
    ...
    def check_fast_integration(
        ...
        *args,
        ...
        input_ids = mindspore.tensor([[7, 4, 78, 0, 24, 52, 43]], device=mindspore_device, dtype=mindspore.long)
        decoder_input_ids = mindspore.tensor([[12, 62, 25, 11, 47, 15, 14]], device=mindspore_device, dtype=mindspore.long)
        attention_mask = mindspore.tensor([[1, 1, 1, 0, 1, 0, 0]], device=mindspore_device, dtype=mindspore.long)
        decoder_attention_mask = mindspore.tensor([[1, 1, 1, 0, 0, 1, 0]], device=mindspore_device, dtype=mindspore.long)
        lm_labels = mindspore.tensor([[62, 25, 11, 47, 15, 14, 24]], device=mindspore_device, dtype=mindspore.long)
        mindspore.manual_seed(0)
        config.ngram = 4
        ...
        self.parent.assertTrue(mindspore.allclose(result.loss, mindspore.tensor(4.5819, device=mindspore_device), atol=1e-3))

        expected_logit_slice = mindspore.tensor(
            [-0.1565, 0.0418, 0.1207, 0.0030, 0.0665, 0.0467, 0.0412], device=mindspore_device
        )
        self.parent.assertTrue(mindspore.allclose(result.logits[0, :, 1], expected_logit_slice, atol=1e-3))
    ...
    def check_model_with_attn_mask(self, config, input_ids, decoder_input_ids, *args):
        ...
        attention_mask = mindspore.ones_like(input_ids)
        decoder_attention_mask = mindspore.ones_like(decoder_input_ids)
    ...
    def check_causal_lm_from_pretrained(
        ...
        model = ProphetNetForConditionalGeneration(config).to(mindspore_device).eval()

        with tempfile.TemporaryDirectory() as tmp_dirname:
            model.save_pretrained(tmp_dirname)
            decoder = ProphetNetForCausalLM.from_pretrained(tmp_dirname).to(mindspore_device)
...
class ProphetNetStandaloneDecoderModelTester:
    ...
    def create_and_check_decoder_model_past(
        ...
        model = ProphetNetDecoder(config=config).to(mindspore_device).eval()
        # first forward pass
        ...
        next_input_ids = mindspore.cat([input_ids, next_tokens], dim=-1)
        ...
        assert mindspore.allclose(output_from_past_slice, output_from_no_past_slice, atol=1e-3)
    ...
    def create_and_check_decoder_model_attention_mask_past(
        ...
        attn_mask = mindspore.ones(input_ids.shape, dtype=mindspore.long, device=mindspore_device)
        ...
        next_input_ids = mindspore.cat([input_ids, next_tokens], dim=-1)
        attn_mask = mindspore.cat(
            [attn_mask, mindspore.ones((attn_mask.shape[0], 1), dtype=mindspore.long, device=mindspore_device)],
            dim=1,
        )
        ...
        # test that outputs are equal for slice
        assert mindspore.allclose(output_from_past_slice, output_from_no_past_slice, atol=1e-2)
...
class ProphetNetModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    @unittest.skipIf(mindspore_device == "cpu", "Cant do half precision")
...
class ProphetNetModelIntegrationTest(unittest.TestCase):
    ...
    def test_pretrained_checkpoint_hidden_states(self):
        ...
        decoder_prev_ids = mindspore.tensor([[102, 2129, 2116, 2372, 2024, 2006, 2169, 1997, 2122, 2048, 2780, 1029]]).to(
            mindspore_device
        )
        output = model(
        ...
        expected_shape = mindspore.Size((1, 12, 30522))
        ...
        #        self.assertTrue(mindspore.allclose(output_predited_logits[:, :3, :3], expected_slice, atol=1e-4))
        assert mindspore.allclose(output_predited_logits[:, :3, :3], expected_slice, atol=1e-4)
        ...
        expected_shape_encoder = mindspore.Size((1, 28, 1024))
        self.assertEqual(encoder_outputs.shape, expected_shape_encoder)
        #        self.assertTrue(mindspore.allclose(encoder_outputs[:, :3, :3], expected_encoder_outputs_slice, atol=1e-4))
        assert mindspore.allclose(encoder_outputs[:, :3, :3], expected_encoder_outputs_slice, atol=1e-4)
        ...
        #        self.assertTrue(mindspore.allclose(next_first_stream_logits[:, :3, :3], expected_slice, atol=1e-4))
        assert mindspore.allclose(next_first_stream_logits[:, :3, :3], expected_slice, atol=1e-4)
