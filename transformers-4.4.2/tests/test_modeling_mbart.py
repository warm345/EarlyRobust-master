...
class MBartModelTester:
    ...
    def create_and_check_decoder_model_past_large_inputs(self, config, inputs_dict):
        model = MBartModel(config=config).get_decoder().to(mindspore_device).eval()
    ...
    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        model = MBartModel(config=config).to(mindspore_device).eval()
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            encoder = MBartEncoder.from_pretrained(tmpdirname).to(mindspore_device)
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            decoder = MBartDecoder.from_pretrained(tmpdirname).to(mindspore_device)
...
class MBartModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    def test_generate_fp16(self):
        ...
        model = MBartForConditionalGeneration(config).eval().to(mindspore_device)
...
class AbstractSeq2SeqIntegrationTest(unittest.TestCase):
    ...
    def model(self):
        ...
        model = MBartForConditionalGeneration.from_pretrained(self.checkpoint_name).to(mindspore_device)
        if "cuda" in mindspore_device:
            model = model.half()
        return model
...
class MBartEnroIntegrationTest(AbstractSeq2SeqIntegrationTest):
    ...
    def test_mbart_fast_forward(self):
        ...
        lm_model = MBartForConditionalGeneration(config).to(mindspore_device)
...
class MBartCC25IntegrationTest(AbstractSeq2SeqIntegrationTest):
    ...
    def test_cc25_generate(self):
        inputs = self.tokenizer([self.src_text[0]], return_tensors="pt").to(mindspore_device)
    ...
    def test_fill_mask(self):
        inputs = self.tokenizer(["One of the best <mask> I ever read!"], return_tensors="pt").to(mindspore_device)
...
class MBartStandaloneDecoderModelTester:
    ...
    def create_and_check_decoder_model_past(
        ...
        model = MBartDecoder(config=config).to(mindspore_device).eval()
        # first forward pass
