...
class LxmertModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        visual_feats = mindspore.rand(self.batch_size, self.num_visual_features, self.visual_feat_dim, device=mindspore_device)
        bounding_boxes = mindspore.rand(self.batch_size, self.num_visual_features, 4, device=mindspore_device)
    ...
    def resize_lxmert_num_qa_labels(
        ...
        model_pretrain = LxmertForPreTraining(config=config).to(mindspore_device)
        model_qa = LxmertForQuestionAnswering(config=config).to(mindspore_device)
...
class LxmertModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    all_model_classes = (LxmertModel, LxmertForPreTraining, LxmertForQuestionAnswering) if is_mindspore_available() else ()
