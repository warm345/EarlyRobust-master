...
from transformers import AutoTokenizer, IntervalStrategy, PretrainedConfig, TrainingArguments, is_mindspore_available
...
if is_mindspore_available():
    import mindspore
    from mindspore.utils.data import IterableDataset
...
if is_mindspore_available():
    ...
    class RegressionModel(mindspore.nn.Module):
        def __init__(self, a=0, b=0, double_output=False):
            super().__init__()
            self.a = mindspore.nn.Parameter(mindspore.tensor(a).float())
            self.b = mindspore.nn.Parameter(mindspore.tensor(b).float())
            self.double_output = double_output
            self.config = None
        ...
        def forward(self, input_x, labels=None, **kwargs):
            ...
            loss = mindspore.nn.functional.mse_loss(y, labels)
            return (loss, y, y) if self.double_output else (loss, y)

    class RegressionDictModel(mindspore.nn.Module):
        ...
        def forward(self, input_x, labels=None, **kwargs):
            ...
            if labels is not None:
                result["loss"] = mindspore.nn.functional.mse_loss(y, labels)
            return result
    ...
    class RegressionPreTrainedModel(PreTrainedModel):
        ...
        def __init__(self, config):
            ...
            self.a = mindspore.nn.Parameter(mindspore.tensor(config.a).float())
            self.b = mindspore.nn.Parameter(mindspore.tensor(config.b).float())
    ...
    class TstLayer(mindspore.nn.Module):
        def __init__(self, hidden_size):
            super().__init__()
            self.linear1 = mindspore.nn.Linear(hidden_size, hidden_size)
            self.ln1 = mindspore.nn.LayerNorm(hidden_size)
            self.linear2 = mindspore.nn.Linear(hidden_size, hidden_size)
            self.ln2 = mindspore.nn.LayerNorm(hidden_size)
            self.bias = mindspore.nn.Parameter(mindspore.zeros(hidden_size))

        def forward(self, x):
            h = self.ln1(mindspore.nn.functional.relu(self.linear1(x)))
            h = mindspore.nn.functional.relu(self.linear2(x))
...
class TrainerIntegrationTest(unittest.TestCase):
    ...
    def check_trained_model(self, model, alternate_seed=False):
        ...
        self.assertTrue(mindspore.allclose(model.a, a))
        self.assertTrue(mindspore.allclose(model.b, b))
    ...
    def check_best_model_has_been_loaded(
        ...
        if is_pretrained:
            ...
            state_dict = mindspore.load(os.path.join(checkpoint, WEIGHTS_NAME))
            best_model.load_state_dict(state_dict)
            best_model.to(trainer.args.device)
        self.assertTrue(mindspore.allclose(best_model.a, trainer.model.a))
        self.assertTrue(mindspore.allclose(best_model.b, trainer.model.b))
    ...
    def test_train_and_eval_dataloaders(self):
        n_gpu = max(1, mindspore.cuda.device_count())
    ...
    def test_trainer_with_datasets(self):
        ...
        # Can return tensors.
        train_dataset.set_format(type="mindspore", dtype=mindspore.float32)
        model = RegressionModel()
        trainer = Trainer(model, args, train_dataset=train_dataset)
        trainer.train()
        self.check_trained_model(trainer.model)
    ...
    def test_custom_optimizer(self):
        ...
        args = TrainingArguments("./regression")
        model = RegressionModel()
        optimizer = mindspore.optim.SGD(model.parameters(), lr=1.0)
        lr_scheduler = mindspore.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda x: 1.0)
        trainer = Trainer(model, args, train_dataset=train_dataset, optimizers=(optimizer, lr_scheduler))
        trainer.train()

        (a, b) = self.default_trained_model
        self.assertFalse(mindspore.allclose(trainer.model.a, a))
        self.assertFalse(mindspore.allclose(trainer.model.b, b))
    ...
    def test_can_resume_training(self):
        if mindspore.cuda.device_count() > 2:
    ...
    def test_trainer_iterable_dataset(self):
        ...
        self.assertIsInstance(loader, mindspore.utils.data.DataLoader)
        self.assertIsInstance(loader.sampler, mindspore.utils.data.dataloader._InfiniteConstantSampler)
    ...
    def test_flos_extraction(self):
        ...
        assert_flos_extraction(trainer, mindspore.nn.DataParallel(trainer.model))
    ...
    def check_mem_metrics(self, trainer, check_func):
        ...
        if mindspore.cuda.device_count() > 0:
    ...
    def test_fp16_full_eval(self):
        ...
        a = mindspore.ones(1000, bs) + 0.001
        b = mindspore.ones(1000, bs) - 0.001
    ...
    def test_no_wd_param_group(self):
        model = mindspore.nn.Sequential(TstLayer(128), mindspore.nn.ModuleList([TstLayer(128), TstLayer(128)]))
