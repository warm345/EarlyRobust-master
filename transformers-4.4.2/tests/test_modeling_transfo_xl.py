...
class TransfoXLModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (
        (TransfoXLModel, TransfoXLLMHeadModel, TransfoXLForSequenceClassification) if is_mindspore_available() else ()
    )
    all_generative_model_classes = (TransfoXLLMHeadModel,) if is_mindspore_available() else ()
    test_pruning = False
