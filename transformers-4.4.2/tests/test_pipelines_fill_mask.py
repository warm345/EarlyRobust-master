...
from transformers.testing_utils import require_tf, require_mindspore, slow
...
class FillMaskPipelineTests(MonoInputPipelineCommonMixin, unittest.TestCase):
    ...
    @require_mindspore
    def test_mindspore_fill_mask(self):
    ...
    @require_mindspore
    def test_mindspore_fill_mask_with_targets(self):
        valid_inputs = ["My name is <mask>"]
    ...
    @require_mindspore
    @slow
    def test_mindspore_fill_mask_results(self):
        mandatory_keys = {"sequence", "score", "token"}
        valid_inputs = [
            "My name is <mask>",
            "The largest city in France is <mask>",
