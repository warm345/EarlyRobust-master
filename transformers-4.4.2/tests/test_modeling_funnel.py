...
from transformers import FunnelTokenizer, is_mindspore_available
...
class FunnelModelIntegrationTest(unittest.TestCase):
    def test_inference_tiny_model(self):
        batch_size = 13
        sequence_length = 7
        input_ids = mindspore.arange(0, batch_size * sequence_length).long().reshape(batch_size, sequence_length)
