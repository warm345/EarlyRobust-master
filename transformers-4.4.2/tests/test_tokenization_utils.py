...
from transformers.testing_utils import CaptureStderr, require_flax, require_tf, require_tokenizers, require_mindspore, slow
...
class TokenizerUtilsTest(unittest.TestCase):
    ...
    def test_tensor_type_from_str(self):
        ...
        self.assertEqual(TensorType("pt"), TensorType.mindspore)
    ...
    def test_batch_encoding_pickle_pt(self):
        ...
        with self.subTest("BatchEncoding (Python, return_tensors=mindspore)"):
            self.assert_dump_and_restore(
                tokenizer_p("Small example to encode", return_tensors=TensorType.mindspore), mindspore.equal
            )

        with self.subTest("BatchEncoding (Rust, return_tensors=mindspore)"):
            self.assert_dump_and_restore(
                tokenizer_r("Small example to encode", return_tensors=TensorType.mindspore), mindspore.equal
            )
    ...
    def test_padding_accepts_tensors_pt(self):
        ...
        features = [{"input_ids": mindspore.tensor([0, 1, 2])}, {"input_ids": mindspore.tensor([0, 1, 2, 3])}]
        tokenizer = BertTokenizer.from_pretrained("bert-base-cased")

        batch = tokenizer.pad(features, padding=True)
        self.assertTrue(isinstance(batch["input_ids"], mindspore.Tensor))
        self.assertEqual(batch["input_ids"].tolist(), [[0, 1, 2, tokenizer.pad_token_id], [0, 1, 2, 3]])
