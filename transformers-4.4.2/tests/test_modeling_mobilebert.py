...
class MobileBertModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        model = MobileBertModel.from_pretrained("google/mobilebert-uncased").to(mindspore_device)
        input_ids = _long_tensor([[101, 7110, 1005, 1056, 2023, 11333, 17413, 1029, 102]])
        with mindspore.no_grad():
            output = model(input_ids)[0]
        expected_shape = mindspore.Size((1, 9, 512))
        ...
        lower_bound = mindspore.all((expected_slice / output[..., :3, :3]) >= 1 - TOLERANCE)
        upper_bound = mindspore.all((expected_slice / output[..., :3, :3]) <= 1 + TOLERANCE)
