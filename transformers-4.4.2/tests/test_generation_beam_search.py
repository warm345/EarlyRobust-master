...
class BeamSearchTester:
    ...
    def prepare_inputs(self):
        ...
        next_tokens = ids_tensor((self.batch_size, 2 * self.num_beams), self.vocab_size).to(mindspore_device)
        next_indices = ids_tensor((self.batch_size, 2 * self.num_beams), self.num_beams).to(mindspore_device)
        next_scores, _ = (-floats_tensor((self.batch_size, 2 * self.num_beams)).to(mindspore_device)).sort(descending=True)
    ...
    def check_beam_scorer_update(self, input_ids, next_tokens, next_indices, next_scores):
        ...
        def cut_expected_tensor(tensor):
            return mindspore.cat([tensor[:, :1], tensor[:, 2 : self.num_beams + 1]], dim=1).flatten()
        ...
        expected_output_indices = (
            ...
            + (mindspore.arange(self.num_beams * self.batch_size, device=mindspore_device) // self.num_beams) * self.num_beams
        )
        ...
        self.parent.assertTrue(mindspore.allclose(expected_output_scores, output_scores, atol=1e-3))
    ...
    def check_beam_scores_finalize(self, input_ids, next_tokens, next_indices, next_scores):
        ...
        input_ids = mindspore.cat([input_ids[output_indices, :], output_tokens.unsqueeze(-1)], dim=-1)
