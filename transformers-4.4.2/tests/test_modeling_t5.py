...
class T5ModelTester:
    ...
    def create_and_check_with_lm_head(
        ...
        model = T5ForConditionalGeneration(config=config).to(mindspore_device).eval()
        outputs = model(
            input_ids=input_ids,
            decoder_input_ids=decoder_input_ids,
            decoder_attention_mask=decoder_attention_mask,
    ...
    def create_and_check_decoder_model_past(
        ...
        model = T5Model(config=config).get_decoder().to(mindspore_device).eval()
        # first forward pass
    ...
    def create_and_check_decoder_model_past_large_inputs(
        ...
        # append to next input_ids and
        next_input_ids = mindspore.cat([input_ids, next_tokens], dim=-1)
        next_attention_mask = mindspore.cat([attention_mask, next_mask], dim=-1)
    ...
    def create_and_check_model_fp16_forward(
        ...
        model = T5Model(config=config).to(mindspore_device).half().eval()
...
class T5ModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    all_model_classes = (T5Model, T5ForConditionalGeneration) if is_mindspore_available() else ()
    ...
    def test_export_to_onnx(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        model = T5Model(config_and_inputs[0]).to(mindspore_device)
        with tempfile.TemporaryDirectory() as tmpdirname:
...
class T5EncoderOnlyModelTester:
    ...
    def create_and_check_model_fp16_forward(
        ...
        model = T5EncoderModel(config=config).to(mindspore_device).half().eval()
...
class T5ModelIntegrationTests(unittest.TestCase):
    @cached_property
    def model(self):
        return T5ForConditionalGeneration.from_pretrained("t5-base").to(mindspore_device)
    ...
    def test_small_integration_test(self):
        ...
        model = T5ForConditionalGeneration.from_pretrained("t5-small").to(mindspore_device)
        tokenizer = T5Tokenizer.from_pretrained("t5-small")

        input_ids = tokenizer("Hello there", return_tensors="pt").input_ids
        labels = tokenizer("Hi I am", return_tensors="pt").input_ids

        loss = model(input_ids.to(mindspore_device), labels=labels.to(mindspore_device)).loss
    ...
    def test_small_v1_1_integration_test(self):
        ...
        model = T5ForConditionalGeneration.from_pretrained("google/t5-v1_1-small").to(mindspore_device)
    ...
    def test_translation_en_to_ro(self):
        ...
        inputs = tok(model.config.prefix + en_text, return_tensors="pt").to(mindspore_device)
