...
class BlenderbotSmallModelTester:
    ...
    def create_and_check_decoder_model_past_large_inputs(self, config, inputs_dict):
        model = BlenderbotSmallModel(config=config).get_decoder().to(mindspore_device).eval()
    ...
    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        model = BlenderbotSmallModel(config=config).to(mindspore_device).eval()
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            encoder = BlenderbotSmallEncoder.from_pretrained(tmpdirname).to(mindspore_device)
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            decoder = BlenderbotSmallDecoder.from_pretrained(tmpdirname).to(mindspore_device)
...
class BlenderbotSmallModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    def test_generate_fp16(self):
        ...
        model = BlenderbotSmallForConditionalGeneration(config).eval().to(mindspore_device)
...
class Blenderbot90MIntegrationTests(unittest.TestCase):
    ...
    @cached_property
    def model(self):
        model = BlenderbotSmallForConditionalGeneration.from_pretrained(self.ckpt).to(mindspore_device)
        if mindspore_device == "cuda":
            model = model.half()
        return model
    ...
    def test_90_generation_from_long_input(self):
       ...
       i'm going to throw up.\nand why is that?"
        ...
        model_inputs = self.tokenizer(src_text, return_tensors="pt").to(mindspore_device)
    ...
    def test_90_generation_from_short_input(self):
        model_inputs = self.tokenizer(["sam"], return_tensors="pt").to(mindspore_device)
...
class BlenderbotSmallStandaloneDecoderModelTester:
    ...
    def create_and_check_decoder_model_past(
        ...
        model = BlenderbotSmallDecoder(config=config).to(mindspore_device).eval()
        # first forward pass
