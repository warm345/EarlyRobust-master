...
class TFLxmertModelTest(TFModelTesterMixin, unittest.TestCase):
    ...
    def test_pt_tf_model_equivalence(self):
        ...
        for model_class in self.all_model_classes:
            ...
            tf_model = transformers.load_mindspore_model_in_tf2_model(
                tf_model, pt_model, tf_inputs=self._prepare_for_class(inputs_dict, model_class)
            )
            pt_model = transformers.load_tf2_model_in_mindspore_model(pt_model, tf_model)
                ...
                for key, value in iterable.items():
                    if isinstance(value, dict):
                        ...
                        if isinstance(value, (list, tuple)):
                            return_dict[key] = (
                                mindspore.from_numpy(iter_value.numpy()).to(mindspore_type(key)) for iter_value in value
                            )
                        else:
                            return_dict[key] = mindspore.from_numpy(value.numpy()).to(mindspore_type(key))
                return return_dict

            pt_inputs_dict = recursive_numpy_convert(self._prepare_for_class(inputs_dict, model_class))

            # need to rename encoder-decoder "inputs" for mindspore
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                mindspore.save(pt_model.state_dict(), pt_checkpoint_path)
                tf_model = transformers.load_mindspore_checkpoint_in_tf2_model(tf_model, pt_checkpoint_path)

                tf_checkpoint_path = os.path.join(tmpdirname, "tf_model.h5")
                tf_model.save_weights(tf_checkpoint_path)
                pt_model = transformers.load_tf2_checkpoint_in_mindspore_model(pt_model, tf_checkpoint_path)

            # Check predictions on first output (logits/hidden-states) are close enought given low-level computational differences
            pt_model.eval()
            pt_inputs_dict = dict(
                (name, mindspore.from_numpy(key.numpy()).to(mindspore.long))
                for name, key in self._prepare_for_class(inputs_dict, model_class).items()
            )

            for key, value in pt_inputs_dict.items():
                if key in ("visual_feats", "visual_pos"):
                    pt_inputs_dict[key] = value.to(mindspore.float32)
                else:
                    pt_inputs_dict[key] = value.to(mindspore.long)
