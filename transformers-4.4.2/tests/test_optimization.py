    ...
    for step in range(num_steps):
        ...
        if step == num_steps // 2:
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                mindspore.save(scheduler.state_dict(), file_name)

                state_dict = mindspore.load(file_name)
...
class OptimizationTest(unittest.TestCase):
    ...
    def test_adam_w(self):
        w = mindspore.tensor([0.1, -0.2, -0.1], requires_grad=True)
        target = mindspore.tensor([0.4, 0.2, -0.5])
...
class ScheduleInitTest(unittest.TestCase):
    m = mindspore.nn.Linear(50, 50) if is_mindspore_available() else None
    optimizer = AdamW(m.parameters(), lr=10.0) if is_mindspore_available() else None
    num_steps = 10
