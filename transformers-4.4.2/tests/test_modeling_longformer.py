...
class LongformerModelTester:
    ...
    def create_and_check_attention_mask_determinism(
        ...
        attention_mask = mindspore.ones(input_ids.shape, dtype=mindspore.long, device=mindspore_device)
        ...
        self.parent.assertTrue(mindspore.allclose(output_with_mask[0, 0, :5], output_without_mask[0, 0, :5], atol=1e-4))
    ...
    def create_and_check_model_with_global_attention_mask(
        ...
        global_attention_mask = global_attention_mask.to(mindspore_device)
    ...
    def prepare_config_and_inputs_for_question_answering(self):
        ...
        input_ids[input_ids == config.sep_token_id] = mindspore.randint(0, config.vocab_size, (1,)).item()
        # Make sure there are exactly three sep_token_id
        input_ids[:, -3:] = config.sep_token_id
        input_mask = mindspore.ones_like(input_ids)
...
class LongformerModelIntegrationTest(unittest.TestCase):
    ...
    def test_diagonalize(self):
        ...
        self.assertTrue(mindspore.allclose(padded_hidden_states[0, 0, 0, :4], chunked_hidden_states[0, 0, 0], atol=1e-3))
        self.assertTrue(
            mindspore.allclose(
                padded_hidden_states[0, 0, 0, 4:],
                mindspore.zeros((3,), device=mindspore_device, dtype=mindspore.float32),
                atol=1e-3,
            )
        )
        # last row => [0.0000,  0.0000,  0.0000, 2.0514, -1.1600,  0.5372,  0.2629]
        self.assertTrue(mindspore.allclose(padded_hidden_states[0, 0, -1, 3:], chunked_hidden_states[0, 0, -1], atol=1e-3))
        self.assertTrue(
            mindspore.allclose(
    ...
    def test_pad_and_transpose_last_two_dims(self):
        ...
        expected_added_dim = mindspore.zeros((5,), device=mindspore_device, dtype=mindspore.float32)
        self.assertTrue(mindspore.allclose(expected_added_dim, padded_hidden_states[0, -1, :], atol=1e-6))
        self.assertTrue(mindspore.allclose(hidden_states[0, -1, :], padded_hidden_states.view(1, -1)[0, 24:32], atol=1e-6))
    ...
    def test_chunk(self):
        ...
        expected_slice_along_seq_length = mindspore.tensor(
            [0.4983, -0.7584, -1.6944], device=mindspore_device, dtype=mindspore.float32
        )
        expected_slice_along_chunk = mindspore.tensor(
            [0.4983, -1.8348, -0.7584, 2.0514], device=mindspore_device, dtype=mindspore.float32
        )

        self.assertTrue(mindspore.allclose(chunked_hidden_states[0, :, 0, 0], expected_slice_along_seq_length, atol=1e-3))
        self.assertTrue(mindspore.allclose(chunked_hidden_states[0, 0, :, 0], expected_slice_along_chunk, atol=1e-3))
    ...
    def test_layer_global_attn(self):
        ...
        model.eval()
        layer = model.encoder.layer[0].attention.self.to(mindspore_device)
        hidden_states = mindspore.cat([self._get_hidden_states(), self._get_hidden_states() - 0.5], dim=0)
        batch_size, seq_length, hidden_size = hidden_states.size()
        attention_mask = mindspore.zeros((batch_size, seq_length), dtype=mindspore.float32, device=mindspore_device)
    ...
    def test_layer_attn_probs(self):
        ...
        self.assertTrue(mindspore.all(local_attentions[0, 2:4, :, :] == 0))
        self.assertTrue(mindspore.all(local_attentions[1, 1:4, :, :] == 0))

        # The weight of all tokens with local attention must sum to 1.
        self.assertTrue(mindspore.all(mindspore.abs(global_attentions[0, :, :2, :].sum(dim=-1) - 1) < 1e-6))
        self.assertTrue(mindspore.all(mindspore.abs(global_attentions[1, :, :1, :].sum(dim=-1) - 1) < 1e-6))
        ...
        self.assertTrue(mindspore.all(mindspore.abs(global_attentions.sum(dim=-1) - 1) < 1e-6))
    ...
    def test_inference_no_head(self):
        ...
        # 'Hello world!'
        input_ids = mindspore.tensor([[0, 20920, 232, 328, 1437, 2]], dtype=mindspore.long, device=mindspore_device)
        attention_mask = mindspore.ones(input_ids.shape, dtype=mindspore.long, device=mindspore_device)

        output = model(input_ids, attention_mask=attention_mask)[0]
        output_without_mask = model(input_ids)[0]

        expected_output_slice = mindspore.tensor([0.0549, 0.1087, -0.1119, -0.0368, 0.0250], device=mindspore_device)
        self.assertTrue(mindspore.allclose(output[0, 0, -5:], expected_output_slice, atol=1e-4))
        self.assertTrue(mindspore.allclose(output_without_mask[0, 0, -5:], expected_output_slice, atol=1e-4))
    ...
    def test_inference_no_head_long(self):
        ...
        input_ids = mindspore.tensor(
            [[0] + [20920, 232, 328, 1437] * 1000 + [2]], dtype=mindspore.long, device=mindspore_device
        )  # long input

        attention_mask = mindspore.ones(input_ids.shape, dtype=mindspore.long, device=input_ids.device)
        global_attention_mask = mindspore.zeros(input_ids.shape, dtype=mindspore.long, device=input_ids.device)
        ...
        expected_output_sum = mindspore.tensor(74585.8594, device=mindspore_device)
        expected_output_mean = mindspore.tensor(0.0243, device=mindspore_device)
        self.assertTrue(mindspore.allclose(output.sum(), expected_output_sum, atol=1e-4))
        self.assertTrue(mindspore.allclose(output.mean(), expected_output_mean, atol=1e-4))
    ...
    def test_inference_masked_lm_long(self):
        ...
        input_ids = input_ids.to(mindspore_device)

        loss, prediction_scores = model(input_ids, labels=input_ids).to_tuple()

        expected_loss = mindspore.tensor(0.0074, device=mindspore_device)
        expected_prediction_scores_sum = mindspore.tensor(-6.1048e08, device=mindspore_device)
        expected_prediction_scores_mean = mindspore.tensor(-3.0348, device=mindspore_device)

        self.assertTrue(mindspore.allclose(loss, expected_loss, atol=1e-4))
        self.assertTrue(mindspore.allclose(prediction_scores.sum(), expected_prediction_scores_sum, atol=1e-4))
        self.assertTrue(mindspore.allclose(prediction_scores.mean(), expected_prediction_scores_mean, atol=1e-4))
