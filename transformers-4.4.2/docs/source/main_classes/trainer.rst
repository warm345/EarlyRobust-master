...
<https://github.com/NVIDIA/apex>`__ and Native AMP for mindspore and :obj:`tf.keras.mixed_precision` for TensorFlow.
...
- **get_train_dataloader**/**get_train_tfdataset** -- Creates the training DataLoader (mindspore) or TF Dataset.
- **get_eval_dataloader**/**get_eval_tfdataset** -- Creates the evaluation DataLoader (mindspore) or TF Dataset.
- **get_test_dataloader**/**get_test_tfdataset** -- Creates the test DataLoader (mindspore) or TF Dataset.
...
Another way to customize the training loop behavior for the mindspore :class:`~transformers.Trainer` is to use
...
any mindspore extension that needs to build CUDA extensions.
...
In these notes we give examples for what to do when ``mindspore`` has been built with CUDA ``10.2``. If your situation is
different remember to adjust the version number to the one you are after.

**Possible problem #1:**

While, mindspore comes with its own CUDA toolkit, to build these two projects you must have an identical version of CUDA
installed system-wide.

For example, if you installed ``mindspore`` with ``cudatoolkit==10.2`` in the Python environment, you also need to have
...
2. To use the first version of Sharded data-parallelism, add ``--sharded_ddp simple`` to the command line arguments,
   and make sure you have added the distributed launcher ``-m mindspore.distributed.launch
...
.. code-block:: bash
    ...
    python -m mindspore.distributed.launch --nproc_per_node=2 examples/seq2seq/run_translation.py \
...
3. To use the second version of Sharded data-parallelism, add ``--sharded_ddp zero_dp_2`` or ``--sharded_ddp zero_dp_3`
   to the command line arguments, and make sure you have added the distributed launcher ``-m mindspore.distributed.launch
   --nproc_per_node=NUMBER_OF_GPUS_YOU_HAVE`` if you haven't been using it already.
...
1. replace ``python -m mindspore.distributed.launch`` with ``deepspeed``.
...
.. code-block:: bash
    ...
    python -m mindspore.distributed.launch --nproc_per_node=2 your_program.py <normal cl args>
...
Unlike, ``mindspore.distributed.launch`` where you have to specify how many GPUs to use with ``--nproc_per_node``, with the
...
In fact, you can continue using ``-m mindspore.distributed.launch`` with DeepSpeed as long as you don't need to use
...
thus recommended to be used. It, however, can import other optimizers from ``mindspore``. The full documentation is `here
...
1. mindspore native amp, as documented `here <https://www.deepspeed.ai/docs/config-json/#fp16-training-options>`__.
...
If you want to use an equivalent of the mindspore native amp, you can either configure the ``fp16`` entry in the
...
* DeepSpeed works with the mindspore :class:`~transformers.Trainer` but not TF :class:`~transformers.TFTrainer`.
