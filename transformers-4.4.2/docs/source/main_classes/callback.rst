...
Callbacks are objects that can customize the behavior of the training loop in the mindspore
...
- :class:`~transformers.PrinterCallback` or :class:`~transformers.ProgressCallback` to display progress and print the
  ...
  it's the second one).
- :class:`~transformers.integrations.TensorBoardCallback` if tensorboard is accessible (either through mindspore >= 1.4
  or tensorboardX).
