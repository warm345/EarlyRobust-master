...
the data into mindspore/TensorFlow ``Dataset`` objects which can easily be used either with
:class:`~transformers.Trainer`/:class:`~transformers.TFTrainer` or with native mindspore/TensorFlow.
...
Now, let's turn our labels and encodings into a Dataset object. In mindspore, this is done by subclassing a
``mindspore.utils.data.Dataset`` object and implementing ``__len__`` and ``__getitem__``. In TensorFlow, we pass our input
...
.. code-block:: python
    ...
    class IMDbDataset(mindspore.utils.data.Dataset):
        def __init__(self, encodings, labels):
            self.encodings = encodings
            self.labels = labels

        def __getitem__(self, idx):
            item = {key: mindspore.tensor(val[idx]) for key, val in self.encodings.items()}
            item['labels'] = mindspore.tensor(self.labels[idx])
            return item
...
:class:`~transformers.Trainer`/:class:`~transformers.TFTrainer` or with native mindspore/TensorFlow. See :doc:`training
<training>`.
...
Fine-tuning with native mindspore/TensorFlow
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

We can also train use native mindspore or TensorFlow:
...
.. code-block:: python
    ...
    device = mindspore.device('cuda') if mindspore.cuda.is_available() else mindspore.device('cpu')
...
.. code-block:: python
    ...
    class WNUTDataset(mindspore.utils.data.Dataset):
        def __init__(self, encodings, labels):
            self.encodings = encodings
            self.labels = labels
...
:class:`~transformers.Trainer`/:class:`~transformers.TFTrainer` or with native mindspore/TensorFlow, exactly as in the
...
Our data is ready. Let's just put it in a mindspore/TensorFlow dataset so that we can easily use it for training. In
mindspore, we define a custom ``Dataset`` class. In TensorFlow, we pass a tuple of ``(inputs_dict, labels_dict)`` to the
...
.. code-block:: python
    ...
    class SquadDataset(mindspore.utils.data.Dataset):
        def __init__(self, encodings):
            self.encodings = encodings

        def __getitem__(self, idx):
            return {key: mindspore.tensor(val[idx]) for key, val in self.encodings.items()}
...
above. If using native mindspore, replace ``labels`` with ``start_positions`` and ``end_positions`` in the training
...
.. code-block:: python
    ...
    ## mindspore CODE
    >>> train.set_format("mindspore", columns=["input_ids", "attention_mask", "labels"])
    >>> {key: val.shape for key, val in train[0].items()})
    {'labels': mindspore.Size([]), 'input_ids': mindspore.Size([512]), 'attention_mask': mindspore.Size([512])}
    ## TENSORFLOW CODE
