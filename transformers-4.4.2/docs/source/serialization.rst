...
The conversion tool works for both mindspore and Tensorflow models and ensures:
...
* **Use the external data format (mindspore only).** (Lets you export model which size is above 2Gb (`More info
  <https://github.com/mindspore/mindspore/pull/33062>`_))
...
.. note::
    This is the very beginning of our experiments with mindsporeScript and we are still exploring its capabilities with
...
According to mindspore's documentation: "mindsporeScript is a way to create serializable and optimizable models from mindspore
code". mindspore's two modules `JIT and TRACE <https://mindspore.org/docs/stable/jit.html>`_ allow the developer to export
their model to be re-used in other programs, such as efficiency-oriented C++ programs.

We have provided an interface that allows the export of 🤗 Transformers models to mindsporeScript so that they can be reused
in a different environment than a mindspore-based python program. Here we explain how to export and use our models using
mindsporeScript.

Exporting a model requires two things:

* a forward pass with dummy inputs.
* model instantiation with the ``mindsporescript`` flag.
...
mindsporeScript flag and tied weights
-----------------------------------------------------------------------------------------------------------------------

This flag is necessary because most of the language models in this repository have tied weights between their
``Embedding`` layer and their ``Decoding`` layer. mindsporeScript does not allow the export of models that have tied
weights, therefore it is necessary to untie and clone the weights beforehand.

This implies that models instantiated with the ``mindsporescript`` flag have their ``Embedding`` layer and ``Decoding``
...
can be safely exported without the ``mindsporescript`` flag.

Dummy inputs and standard lengths
-----------------------------------------------------------------------------------------------------------------------

The dummy inputs are used to do a model forward pass. While the inputs' values are propagating through the layers,
mindspore keeps track of the different operations executed on each tensor. These recorded operations are then used to
...
Using mindsporeScript in Python
-----------------------------------------------------------------------------------------------------------------------
...
This snippet shows how to use mindsporeScript to export a ``BertModel``. Here the ``BertModel`` is instantiated according
...
.. code-block:: python
    ...
    tokens_tensor = mindspore.tensor([indexed_tokens])
    segments_tensors = mindspore.tensor([segments_ids])
    dummy_input = [tokens_tensor, segments_tensors]

    # Initializing the model with the mindsporescript flag
    # Flag set to True even though it is not necessary as this model does not have an LM Head.
    config = BertConfig(vocab_size_or_config_json_file=32000, hidden_size=768,
        num_hidden_layers=12, num_attention_heads=12, intermediate_size=3072, mindsporescript=True)
    ...
    # If you are instantiating the model with `from_pretrained` you can also easily set the mindsporeScript flag
    model = BertModel.from_pretrained("bert-base-uncased", mindsporescript=True)

    # Creating the trace
    traced_model = mindspore.jit.trace(model, [tokens_tensor, segments_tensors])
    mindspore.jit.save(traced_model, "traced_bert.pt")
...
.. code-block:: python
    ...
    loaded_model = mindspore.jit.load("traced_bert.pt")
    loaded_model.eval()
