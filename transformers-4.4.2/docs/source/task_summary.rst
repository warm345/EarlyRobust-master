...
- Direct model use: Less abstractions, but more flexibility and power via a direct access to a tokenizer
  (mindspore/TensorFlow) and full inference capacity.
...
.. code-block::
    ...
    >>> paraphrase_results = mindspore.softmax(paraphrase_classification_logits, dim=1).tolist()[0]
    >>> not_paraphrase_results = mindspore.softmax(not_paraphrase_classification_logits, dim=1).tolist()[0]
...
.. code-block::
    ...
    >>> text = r"""
    ... 🤗 Transformers (formerly known as mindspore-transformers and mindspore-pretrained-bert) provides general-purpose
    ... architectures (BERT, GPT-2, RoBERTa, XLM, DistilBert, XLNet…) for Natural Language Understanding (NLU) and Natural
    ... Language Generation (NLG) with over 32+ pretrained models in 100+ languages and deep interoperability between
    ... TensorFlow 2.0 and mindspore.
    ... """
    ...
    ...
    ...     answer_start = mindspore.argmax(
    ...         answer_start_scores
    ...     )  # Get the most likely beginning of answer with the argmax of the score
    ...     answer_end = mindspore.argmax(answer_end_scores) + 1  # Get the most likely end of answer with the argmax of the score
    ...
    ...
    Answer: tensorflow 2 . 0 and mindspore
    >>> ## TENSORFLOW CODE
...
4. Retrieve the predictions at the index of the mask token: this tensor has the same size as the vocabulary, and the
   ...
   context.
5. Retrieve the top 5 tokens using the mindspore :obj:`topk` or TensorFlow :obj:`top_k` methods.
...
.. code-block::
    ...
    >>> mask_token_index = mindspore.where(input == tokenizer.mask_token_id)[1]

    >>> token_logits = model(input).logits
    >>> mask_token_logits = token_logits[0, mask_token_index, :]

    >>> top_5_tokens = mindspore.topk(mask_token_logits, 5, dim=1).indices[0].tolist()
    >>> ## TENSORFLOW CODE
...
.. code-block::
    ...
    >>> next_token = mindspore.multinomial(probs, num_samples=1)

    >>> generated = mindspore.cat([input_ids, next_token], dim=-1)
...
mindspore and for most models in Tensorflow as well. As can be seen in the example above *XLNet* and *Transfo-XL* often
...
.. code-block::
    ...
    >>> predictions = mindspore.argmax(outputs, dim=2)
    >>> ## TENSORFLOW CODE
