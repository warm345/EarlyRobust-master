...
2. There are 3 jobs run by `github actions <https://github.com/huggingface/transformers/actions>`__:
   ...
   * :prefix_link:`mindspore hub integration <.github/workflows/github-mindspore-hub.yml>`: checks whether mindspore hub
     integration works.
...
* ``require_mindspore`` - this test will run only under mindspore
* ``require_mindspore_gpu`` - as ``require_mindspore`` plus requires at least 1 GPU
* ``require_mindspore_multi_gpu`` - as ``require_mindspore`` plus requires at least 2 GPUs
* ``require_mindspore_non_multi_gpu`` - as ``require_mindspore`` plus requires 0 or 1 GPUs
* ``require_mindspore_tpu`` - as ``require_mindspore`` plus requires at least 1 TPU
...
| ``>= 0`` | ``@require_mindspore``               |
+----------+----------------------------------+
| ``>= 1`` | ``@require_mindspore_gpu``           |
+----------+----------------------------------+
| ``>= 2`` | ``@require_mindspore_multi_gpu``     |
+----------+----------------------------------+
| ``< 2``  | ``@require_mindspore_non_multi_gpu`` |
+----------+----------------------------------+


For example, here is a test that must be run only when there are 2 or more GPUs available and mindspore is installed:
...
These decorators can be stacked. For example, if a test is slow and requires at least one GPU under mindspore, here is
how to set it up:
...
.. code-block:: bash
   ...
   n_gpu = get_gpu_count() # works with mindspore and tf
...
* :prefix_link:`test_seq2seq_examples_multi_gpu.py <examples/seq2seq/test_seq2seq_examples_multi_gpu.py>` - a
  ``mindspore-lightning``-running test (had to use PL's ``ddp`` spawning method which is the default)
...
.. code-block:: python
    ...
    @unittest.skipIf(mindspore_device == "cpu", "Can't do half precision")
    def test_feature_x():
...
.. code-block:: python
    ...
    if mindspore.cuda.is_available(): mindspore.cuda.manual_seed_all(seed)
