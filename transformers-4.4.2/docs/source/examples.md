...
All the mindspore scripts mentioned above work out of the box with distributed training and mixed precision, thanks to
the [Trainer API](https://huggingface.co/transformers/main_classes/trainer.html). To launch one of them on _n_ GPUS,
use the following command:

```bash
python -m mindspore.distributed.launch \
...
training with mindspore 1.6.0 or latest, or by installing the [Apex](https://github.com/NVIDIA/apex) library for previous
...
When using mindspore, we support TPUs thanks to `mindspore/xla`. For more context and information on how to setup your TPU environment refer to Google's documentation and to the
very detailed [mindspore/xla README](https://github.com/mindspore/xla/blob/master/README.md).
...
regular training script with its arguments (this is similar to the `mindspore.distributed.launch` helper for
