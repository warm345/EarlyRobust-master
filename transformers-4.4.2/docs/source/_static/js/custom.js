...
function addGithubButton() {
    const div = `
        <div class="github-repo">
            <a 
                class="github-button"
                href="https://github.com/huggingface/transformers" data-size="large" data-show-count="true" aria-label="Star huggingface/mindspore-transformers on GitHub">
                Star
            </a>
        </div>
    `;
    ...
    if (hasNotebook.includes(pageName)) {
        ...
        <div class="colab-dropdown">
            ...
            <div class="colab-dropdown-content">
                ...
                <button onclick=" window.open('${baseURL}mindspore/${pageName}.ipynb')">mindspore</button>
...
function platformToggle() {
    ...
    const mindsporeIdentifier = "## mindspore CODE";
    const tensorflowIdentifier = "## TENSORFLOW CODE";

    const promptSpanIdentifier = `<span class="gp">&gt;&gt;&gt; </span>`
    const mindsporeSpanIdentifier = `<span class="c1">${mindsporeIdentifier}</span>`;
    ...
    const getFrameworkSpans = filteredCodeBlock => {
        ...
        const mindsporeSpanPosition = spans.indexOf(mindsporeSpanIdentifier);
        const tensorflowSpanPosition = spans.indexOf(tensorflowSpanIdentifier);

        let mindsporeSpans;
        let tensorflowSpans;

        if(mindsporeSpanPosition < tensorflowSpanPosition){
            ...
            mindsporeSpans = spans.slice(mindsporeSpanPosition + mindsporeSpanIdentifier.length + 1, finalTensorflowSpanPosition);
            tensorflowSpans = spans.slice(tensorflowSpanPosition + tensorflowSpanIdentifier.length + 1, spans.length);
        }else{
            const isPrompt = spans.slice(
                spans.indexOf(mindsporeSpanIdentifier) - promptSpanIdentifier.length,
                spans.indexOf(mindsporeSpanIdentifier)
            ) == promptSpanIdentifier;
            const finalmindsporeSpanPosition = isPrompt ? mindsporeSpanPosition - promptSpanIdentifier.length : mindsporeSpanPosition;

            tensorflowSpans = spans.slice(tensorflowSpanPosition + tensorflowSpanIdentifier.length + 1, finalmindsporeSpanPosition);
            mindsporeSpans = spans.slice(mindsporeSpanPosition + mindsporeSpanIdentifier.length + 1, spans.length);
        }

        return {
            ...filteredCodeBlock,
            mindsporeSample: mindsporeSpans ,
            tensorflowSample: tensorflowSpans
        }
    };

    const createFrameworkButtons = sample => {
            const mindsporeButton = document.createElement("button");
            mindsporeButton.classList.add('mindspore-button')
            mindsporeButton.innerText = "mindspore";
            ...
            selectorDiv.appendChild(mindsporeButton);
            selectorDiv.appendChild(tensorflowButton);
            sample.element.parentElement.prepend(selectorDiv);

            // Init on mindspore
            sample.element.innerHTML = sample.mindsporeSample;
            mindsporeButton.classList.add("selected");
            tensorflowButton.classList.remove("selected");

            mindsporeButton.addEventListener("click", () => {
                for(const codeBlock of updatedCodeBlocks){
                    codeBlock.element.innerHTML = codeBlock.mindsporeSample;
                }
                Array.from(document.getElementsByClassName('mindspore-button')).forEach(button => {
    ...
    const updatedCodeBlocks = codeBlocks
        ...
        .filter(codeBlock => codeBlock.innerText.includes(mindsporeIdentifier) && codeBlock.innerText.includes(tensorflowIdentifier))
