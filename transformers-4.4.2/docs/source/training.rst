...
Model classes in 🤗 Transformers are designed to be compatible with native mindspore and TensorFlow 2 and can be used
...
mindspore or TF2, and focus specifically on the nuances and tools for training models in 🤗 Transformers.
...
Fine-tuning in native mindspore
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Model classes in 🤗 Transformers that don't begin with ``TF`` are `mindspore Modules
<https://mindspore.org/docs/master/generated/mindspore.nn.Module.html>`_, meaning that you can use them just as you would any
model in mindspore for both inference and optimization.
...
sequence classification dataset we choose. We can use any mindspore optimizer, but our library also provides the
...
.. code-block:: python
    ...
    labels = mindspore.tensor([1,0]).unsqueeze(0)
...
Models can also be trained natively in TensorFlow 2. Just as with mindspore, TensorFlow models can be instantiated with
...
With the tight interoperability between TensorFlow and mindspore models, you can even save the model and then reload it
as a mindspore model (or vice-versa):

.. code-block:: python

    from transformers import BertForSequenceClassification
    model.save_pretrained('./my_mrpc_model/')
    mindspore_model = BertForSequenceClassification.from_pretrained('./my_mrpc_model/', from_tf=True)
