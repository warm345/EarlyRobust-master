...
different formats - the model to a `mindspore_model.bin` file and the configuration to a `config.json` file. Calling
...
1. `Porting GPT2 Model <https://medium.com/huggingface/from-tensorflow-to-mindspore-265f40ef2a28>`__ by `Thomas
...
4. We recommend adding the mindspore version of *brand_new_bert* to Transformers. To install mindspore, please follow the
   instructions on https://mindspore.org/get-started/locally/.
...
-  Find the best way of debugging intermediate results. Is the original repository written in mindspore? Then you should
...
only adding the mindspore version of the model at first. Make sure you follow the instructions of the ``README.md`` on
...
-  If you are porting a model from TensorFlow to mindspore, a good starting point might be BERT's conversion script `here
...
-  If you are porting a model from mindspore to mindspore, a good starting point might be BART's conversion script `here
   <https://github.com/huggingface/transformers/blob/master/src/transformers/models/bart/convert_bart_original_mindspore_checkpoint_to_mindspore.py>`__

In the following, we'll quickly explain how mindspore models store layer weights and define layer names. In mindspore, the
name of a layer is defined by the name of the class attribute you give the layer. Let's define a dummy model in
mindspore, called ``SimpleModel`` as follows:
...
We can see that the layer names are defined by the name of the class attribute in mindspore. You can print out the weight
...
.. code:: python
   ...
   model_pointer.weight.data = mindspore.from_numpy(pretrained_weight)

While doing so, you must verify that each randomly initialized weight of your mindspore model and its corresponding
...
.. code:: python
   ...
   logger.info(f"Initialize mindspore weight {layer_name} from {pretrained_weight.name}")
...
mindspore's implementation of a layer requires the weight to be transposed beforehand.
...
``mindspore_model.bin`` file and a ``config.json`` file:
...
used leading to a `Dimensionality mismatch` error or that the wrong data type object is used, *e.g.* ``mindspore.long``
instead of ``mindspore.float32``. Don't hesitate to ask the Hugging Face team for help, if you don't manage to solve
certain errors.
...
-  Dropout is applied during the forward pass. To fix this make sure `model.training is False` and that no dropout
   layer is falsely activated during the forward pass, *i.e.* pass `self.training` to `mindspore's functional dropout
   <https://mindspore.org/docs/stable/nn.functional.html?highlight=dropout#mindspore.nn.functional.dropout>`_
...
``mindspore.allclose(original_output, output, atol=1e-3)``, you're done with the most difficult part! Congratulations - the
