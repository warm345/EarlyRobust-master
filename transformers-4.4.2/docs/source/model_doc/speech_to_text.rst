...
The original code can be found `here <https://github.com/mindspore/fairseq/tree/master/examples/speech_to_text>`__.
...
The feature extractor depends on :obj:`mindsporeaudio` and the tokenizer depends on :obj:`sentencepiece` so be sure to
install those packages before running the examples. You could either install those as extra speech dependancies with
``pip install transformers"[speech, sentencepiece]"`` or install the packages seperatly with ``pip install mindsporeaudio
sentencepiece``. Also ``mindsporeaudio`` requires the development version of the `libsndfile
