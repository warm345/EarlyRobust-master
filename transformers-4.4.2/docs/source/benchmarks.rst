...
The classes :class:`~transformers.mindsporeBenchmark` and :class:`~transformers.TensorFlowBenchmark` allow to flexibly
...
The benchmark classes :class:`~transformers.mindsporeBenchmark` and :class:`~transformers.TensorFlowBenchmark` expect an
object of type :class:`~transformers.mindsporeBenchmarkArguments` and
:class:`~transformers.TensorFlowBenchmarkArguments`, respectively, for instantiation.
:class:`~transformers.mindsporeBenchmarkArguments` and :class:`~transformers.TensorFlowBenchmarkArguments` are data
...
.. code-block::
    ...
    >>> ## mindspore CODE
    >>> from transformers import mindsporeBenchmark, mindsporeBenchmarkArguments

    >>> args = mindsporeBenchmarkArguments(models=["bert-base-uncased"], batch_sizes=[8], sequence_lengths=[8, 32, 128, 512])
    >>> benchmark = mindsporeBenchmark(args)
...
``src/transformers/benchmark/benchmark_args_utils.py``, ``src/transformers/benchmark/benchmark_args.py`` (for mindspore)
and ``src/transformers/benchmark/benchmark_args_tf.py`` (for Tensorflow). Alternatively, running the following shell
commands from root will print out a descriptive list of all configurable parameters for mindspore and Tensorflow
respectively.
...
when adding the argument :obj:`save_to_csv=True` to :class:`~transformers.mindsporeBenchmarkArguments` and
...
.. code-block::
    ...
    >>> ## mindspore CODE
    >>> from transformers import mindsporeBenchmark, mindsporeBenchmarkArguments, BertConfig

    >>> args = mindsporeBenchmarkArguments(models=["bert-base", "bert-384-hid", "bert-6-lay"], batch_sizes=[8], sequence_lengths=[8, 32, 128, 512])
    >>> config_base = BertConfig()
    >>> config_384_hid = BertConfig(hidden_size=384)
    >>> config_6_lay = BertConfig(num_hidden_layers=6)

    >>> benchmark = mindsporeBenchmark(args, configs=[config_base, config_384_hid, config_6_lay])
    >>> benchmark.run()
    ====================       INFERENCE - SPEED - RESULT       ====================
    --------------------------------------------------------------------------------
    Model Name             Batch Size     Seq Length       Time in s                  
...
settings: using mindspore, with and without mindsporeScript, using TensorFlow, with and without XLA. All of those tests were
done across CPUs (except for TensorFlow XLA) and GPUs.

The approach is detailed in the `following blogpost
<https://medium.com/huggingface/benchmarking-transformers-mindspore-and-tensorflow-e2917fb891c2>`__ and the results are
available `here
