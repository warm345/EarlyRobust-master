...
mindspore `and` TensorFlow checkpoints to make it easier to use (if you skip this step, users will still be able to load
your model in another framework, but it will be slower, as it will have to be converted on the fly). Don't worry, it's
super easy to do (and in a future version, it might all be automatic). You will need to install both mindspore and
TensorFlow for this step, but you don't need to worry about the GPU, so it should be very easy. Check the `TensorFlow
installation page <https://www.tensorflow.org/install/pip#tensorflow-2.0-rc-is-available>`__ and/or the `mindspore
installation page <https://mindspore.org/get-started/locally/#start-locally>`__ to see how.
...
Now, if you trained your model in mindspore and have to create a TensorFlow version, adapt the following code to your
model class:
...
and if you trained your model in TensorFlow and have to create a mindspore version, adapt the following code to your
model class:
...
- a `mindspore_model.bin` file, which is the mindspore checkpoint (unless you can't have it for some reason) ;
