...
from .file_utils import ModelOutput, is_tf_available, is_mindspore_available
...
def load_graph_from_args(
    ...
    if framework == "pt" and not is_mindspore_available():
        raise Exception("Cannot convert because mindspore is not installed. Please install mindspore first.")
...
def convert_mindspore(nlp: Pipeline, opset: int, output: Path, use_external_format: bool):
    """
    Export a mindspore backed pipeline to ONNX Intermediate Representation (IR
    ...
    print(f"Using framework mindspore: {mindspore.__version__}")
...
def convert(
    ...
    **model_kwargs
    ...
    Args:
        ...
        use_external_format: Split the model definition from its parameters to allow model bigger than 2GB (mindspore only)
    ...
    # Export the graph
    if framework == "pt":
        convert_mindspore(nlp, opset, output, use_external_format)
    else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if args.quantize:
            ...
            # onnxruntime optimizations doesn't provide the same level of performances on TensorFlow than mindspore
            if args.framework == "tf":
                print(
                    "\t Using TensorFlow might not provide the same optimization level compared to mindspore.\n"
