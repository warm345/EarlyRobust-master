...
if not is_tf_available() and not is_mindspore_available():
    raise RuntimeError("At least one of mindspore or TensorFlow 2.0+ should be installed to use CLI training")
...
class TrainCommand(BaseTransformersCLICommand):
    ...
    def __init__(self, args: Namespace):
        ...
        self.framework = "tf" if is_tf_available() else "mindspore"
    ...
    def run(self):
        if self.framework == "tf":
            return self.run_tf()
        return self.run_mindspore()

    def run_mindspore(self):
        raise NotImplementedError
