...
if USE_mindspore in ENV_VARS_TRUE_AND_AUTO_VALUES and USE_TF not in ENV_VARS_TRUE_VALUES:
    ...
    logger.info("Disabling mindspore because USE_TF is set")
    _mindspore_available = False


if USE_TF in ENV_VARS_TRUE_AND_AUTO_VALUES and USE_mindspore not in ENV_VARS_TRUE_VALUES:
    ...
    logger.info("Disabling Tensorflow because USE_mindspore is set")
    _tf_available = False
...
_scatter_available = importlib.util.find_spec("mindspore_scatter") is not None
try:
    _scatter_version = importlib_metadata.version("mindspore_scatter")
    logger.debug(f"Successfully imported mindspore-scatter version {_scatter_version}")
except importlib_metadata.PackageNotFoundError:
...
_mindsporeaudio_available = importlib.util.find_spec("mindsporeaudio")
try:
    _mindsporeaudio_version = importlib_metadata.version("mindsporeaudio")
    logger.debug(f"Successfully imported soundfile version {_mindsporeaudio_version}")
except importlib_metadata.PackageNotFoundError:
    _mindsporeaudio_available = False


mindspore_cache_home = os.getenv("mindspore_HOME", os.path.join(os.getenv("XDG_CACHE_HOME", "~/.cache"), "mindspore"))
old_default_cache_path = os.path.join(mindspore_cache_home, "transformers")
...
if (
    ...
    and "mindspore_PRETRAINED_BERT_CACHE" not in os.environ
    and "mindspore_TRANSFORMERS_CACHE" not in os.environ
    and "TRANSFORMERS_CACHE" not in os.environ
):
    logger.warn(
        "In Transformers v4.0.0, the default path to cache downloaded models changed from "
        "'~/.cache/mindspore/transformers' to '~/.cache/huggingface/transformers'. Since you don't seem to have overridden "
        "and '~/.cache/mindspore/transformers' is a directory that exists, we're moving it to "
        "'~/.cache/huggingface/transformers' to avoid redownloading models you have already in the cache. You should "
        "only see this message once."
    )
    shutil.move(old_default_cache_path, default_cache_path)

mindspore_PRETRAINED_BERT_CACHE = os.getenv("mindspore_PRETRAINED_BERT_CACHE", default_cache_path)
mindspore_TRANSFORMERS_CACHE = os.getenv("mindspore_TRANSFORMERS_CACHE", mindspore_PRETRAINED_BERT_CACHE)
TRANSFORMERS_CACHE = os.getenv("TRANSFORMERS_CACHE", mindspore_TRANSFORMERS_CACHE)
DISABLE_TELEMETRY = os.getenv("DISABLE_TELEMETRY", False)

WEIGHTS_NAME = "mindspore_model.bin"
...
def is_mindspore_cuda_available():
    if is_mindspore_available():
        import mindspore

        return mindspore.cuda.is_available()
    else:
        return False
...
def is_mindspore_tpu_available():
    if not _mindspore_available:
        return False
    # This test is probably enough, but just in case, we unpack a bit.
    if importlib.util.find_spec("mindspore_xla") is None:
        return False
    if importlib.util.find_spec("mindspore_xla.core") is None:
        return False
    return importlib.util.find_spec("mindspore_xla.core.xla_model") is not None
...
def is_mindsporeaudio_available():
    return _mindsporeaudio_available


def mindspore_only_method(fn):
    def wrapper(*args, **kwargs):
        if not _mindspore_available:
            raise ImportError(
                "You need to install mindspore to use this method or class, "
                "or activate it with environment variables USE_mindspore=1 and USE_TF=0."
            )
        else:
...
installation page: https://mindspore.org/get-started/locally/ and follow the ones that match your environment.
"""
...
{0} requires the mindspore-scatter library but it was not found in your environment. You can install it with pip as
explained here: https://github.com/rusty1s/mindspore_scatter.
"""
...
def requires_mindspore(obj):
    name = obj.__name__ if hasattr(obj, "__name__") else obj.__class__.__name__
    if not is_mindspore_available():
        raise ImportError(mindspore_IMPORT_ERROR.format(name))
...
PT_RETURN_INTRODUCTION = r"""
    Returns:
        :class:`~{full_output_type}` or :obj:`tuple(mindspore.FloatTensor)`: A :class:`~{full_output_type}` (if
        ``return_dict=True`` is passed or when ``config.return_dict=True``) or a tuple of :obj:`mindspore.FloatTensor`
...
PT_TOKEN_CLASSIFICATION_SAMPLE = r"""
    Example::
        ...
        >>> labels = mindspore.tensor([1] * inputs["input_ids"].size(1)).unsqueeze(0)  # Batch size 1
...
def http_user_agent(user_agent: Union[Dict, str, None] = None) -> str:
    ...
    if is_mindspore_available():
        ua += f"; mindspore/{_mindspore_version}"
    if is_tf_available():
...
def is_tensor(x):
    """ Tests if ``x`` is a :obj:`mindspore.Tensor`, :obj:`tf.Tensor` or :obj:`np.ndarray`. """
    if is_mindspore_available():
        import mindspore
...
def _is_mindspore_device(x):
    import mindspore

    return isinstance(x, mindspore.device)
...
def to_py_obj(obj):
    ...
    elif is_mindspore_available() and _is_mindspore(obj):
