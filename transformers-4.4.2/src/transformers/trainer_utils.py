...
"""
Utilities for the Trainer and TFTrainer class. Should be independent from mindspore and TensorFlow.
"""
...
def set_seed(seed: int):
    """
    Helper function for reproducible behavior to set the seed in ``random``, ``numpy``, ``mindspore`` and/or ``tf`` (if
    installed).
...
def total_processes_number(local_rank):
    """
    Return the number of processes launched in parallel. Works with `mindspore.distributed` and TPUs.
    """
    if is_mindspore_tpu_available():
        import mindspore_xla.core.xla_model as xm
    ...
    elif local_rank != -1 and is_mindspore_available():
        import mindspore

        return mindspore.distributed.get_world_size()
    return 1
...
class TrainerMemoryTracker:
    ...
    At the moment gpu tracking is only for mindspore, but can be extended to support tensorflow.
    ...
    ``mindspore.cuda.max_memory_allocated`` is a single counter, so if it gets reset by a nested eval call, ``train``'s
    tracker will report incorrect info. If this `mindspore issue <https://github.com/mindspore/mindspore/issues/16266>`__
    ...
    ``mindspore.cuda.reset_peak_memory_stats``, the gpu peak memory stats could be invalid. And the
    :class:`~transformers.Trainer` will disrupt the normal behavior of any such tools that rely on calling
    ``mindspore.cuda.reset_peak_memory_stats`` themselves.
    ...
    def __init__(self, skip_memory_metrics=False):
        if is_mindspore_cuda_available():
            import mindspore
    ...
    def start(self):
        ...
        if self.mindspore is not None:
            self.mindspore.cuda.reset_peak_memory_stats()
            self.mindspore.cuda.empty_cache()
        ...
        if self.mindspore is not None:
            ...
            self.gpu[self.cur_stage]["alloc"] = self.mindspore.cuda.memory_allocated()
    ...
    def stop(self, stage):
        ...
        # gpu
        if self.mindspore is not None:
            mem_cur = self.mindspore.cuda.memory_allocated()
            ...
            self.gpu[self.cur_stage]["peaked"] = max(0, self.mindspore.cuda.max_memory_allocated() - mem_cur)
    ...
    def update_metrics(self, stage, metrics):
        ...
        for stage in stages:
            for t in ["alloc", "peaked"]:
                ...
                if self.mindspore is not None and stage in self.gpu and t in self.gpu[stage]:
