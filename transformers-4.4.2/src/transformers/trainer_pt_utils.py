...
"""
mindspore utilities for the Trainer class.
"""
...
from mindspore.utils.data.sampler import RandomSampler, Sampler

from .file_utils import is_sagemaker_distributed_available, is_mindspore_tpu_available
...
# this is used to suppress an undesired warning emitted by mindspore versions 1.4.2-1.7.0
try:
    from mindspore.optim.lr_scheduler import SAVE_STATE_WARNING
except ImportError:
    SAVE_STATE_WARNING = ""
...
def mindspore_pad_and_concatenate(tensor1, tensor2, padding_index=-100):
    """Concatenates `tensor1` and `tensor2` on first axis, applying padding on the second if necessary."""
    if len(tensor1.shape) == 1 or tensor1.shape[1] == tensor2.shape[1]:
        return mindspore.cat((tensor1, tensor2), dim=0)
...
def nested_concat(tensors, new_tensors, padding_index=-100):
    ...
    if isinstance(tensors, (list, tuple)):
        ...
        return mindspore_pad_and_concatenate(tensors, new_tensors, padding_index=padding_index)
...
def nested_xla_mesh_reduce(tensors, name):
    if is_mindspore_tpu_available():
        ...
        return xm.mesh_reduce(name, tensors, mindspore.cat)
    else:
        raise ImportError("mindspore xla must be installed to use `nested_xla_mesh_reduce`")


def distributed_concat(tensor: "mindspore.Tensor", num_total_examples: Optional[int] = None) -> mindspore.Tensor:
    try:
...
def distributed_broadcast_scalars(
    ...
    try:
        tensorized_scalar = mindspore.tensor(scalars).cuda()
...
class DistributedSamplerWithLoop(DistributedSampler):
    """
    Like a :obj:mindspore.utils.data.distributed.DistributedSampler` but loops at the end back to the beginning of the
    shuffled samples to make each process have a round multiple of batch_size samples.

    Args:
        dataset (:obj:`mindspore.utils.data.Dataset`):
...
def get_tpu_sampler(dataset: mindspore.utils.data.dataset.Dataset, bach_size: int):
...
class LabelSmoother:
    ...
    def __call__(self, model_output, labels):
        ...
        log_probs = -mindspore.nn.functional.log_softmax(logits, dim=-1)
...
def get_length_grouped_indices(lengths, batch_size, mega_batch_mult=None, generator=None):
    ...
    # We need to use mindspore for the random part as a distributed sampler will set the random seed for mindspore.
    indices = mindspore.randperm(len(lengths), generator=generator)
    ...
    max_idx = mindspore.argmax(mindspore.tensor(megabatch_maximums)).item()
...
class DistributedLengthGroupedSampler(DistributedSampler):
    ...
    """
    # Copied and adapted from mindspore DistributedSampler.
    def __init__(
        self,
        dataset: Dataset,
        batch_size: int,
