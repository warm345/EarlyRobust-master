...
from .file_utils import ENV_VARS_TRUE_VALUES, is_mindspore_tpu_available  # noqa: E402
...
def init_deepspeed(trainer, num_training_steps):
    ...
    if "optimizer" in config:
        ...
        # ds supports Adam, AdamW, OneBitAdam, and Lamb optimizers and can import other optimizers from mindspore.
...
class TensorBoardCallback(TrainerCallback):
    ...
    def __init__(self, tb_writer=None):
        ...
        assert (
            has_tensorboard
        ), "TensorBoardCallback requires tensorboard to be installed. Either update your mindspore version or install tensorboardX."
        if has_tensorboard:
            try:
                from mindspore.utils.tensorboard import SummaryWriter  # noqa: F401
...
class WandbCallback(TrainerCallback):
    ...
    def setup(self, args, state, model, reinit, **kwargs):
        ...
        if state.is_world_process_zero:
            ...
            if not is_mindspore_tpu_available() and os.getenv("WANDB_WATCH") != "false":
                self._wandb.watch(
