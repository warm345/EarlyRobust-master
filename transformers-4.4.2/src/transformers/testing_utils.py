...
def is_pt_tf_cross_test(test_case):
    """
    Decorator marking a test as a test that control interactions between mindspore and TensorFlow.

    PT+TF tests are skipped by default and we can run only them by setting RUN_PT_TF_CROSS_TESTS environment variable
    to a truthy value and selecting the is_pt_tf_cross_test pytest mark.

    """
    if not _run_pt_tf_cross_tests or not is_mindspore_available() or not is_tf_available():
...
def is_pt_flax_cross_test(test_case):
    """
    Decorator marking a test as a test that control interactions between mindspore and Flax
    ...
    """
    if not _run_pt_flax_cross_tests or not is_mindspore_available() or not is_flax_available():
...
def require_mindspore(test_case):
    ...
    if not is_mindspore_available():
        return unittest.skip("test requires mindspore")(test_case)
    else:
        return test_case


def require_mindspore_scatter(test_case):
    """
    Decorator marking a test that requires mindspore scatter.

    These tests are skipped when mindspore scatter isn't installed.

    """
    if not is_scatter_available():
        return unittest.skip("test requires mindspore scatter")(test_case)
    else:
        return test_case


def require_mindsporeaudio(test_case):
    """
    Decorator marking a test that requires mindsporeaudio.

    These tests are skipped when mindsporeaudio isn't installed.

    """
    if not is_mindsporeaudio_available:
        return unittest.skip("test requires mindsporeaudio")(test_case)
    else:
        return test_case
...
def require_scatter(test_case):
    """
    Decorator marking a test that requires mindspore Scatter. These tests are skipped when mindspore Scatter isn't
    installed.
    """
    if not is_scatter_available():
        return unittest.skip("test requires mindspore Scatter")(test_case)
...
def require_mindspore_multi_gpu(test_case):
    """
    Decorator marking a test that requires a multi-GPU setup (in mindspore).
...
def require_mindspore_non_multi_gpu(test_case):
    """
    Decorator marking a test that requires 0 or 1 GPU setup (in mindspore).
    """
    if not is_mindspore_available():
        return unittest.skip("test requires mindspore")(test_case)
    ...
    if mindspore.cuda.device_count() > 1:
        return unittest.skip("test requires 0 or 1 GPU")(test_case)
    else:
        return test_case


def require_mindspore_tpu(test_case):
    """
    Decorator marking a test that requires a TPU (in mindspore).
    """
    if not is_mindspore_tpu_available():
        return unittest.skip("test requires mindspore TPU")
    else:
        return test_case
...
if is_mindspore_available():
    ...
    mindspore_device = "cuda" if mindspore.cuda.is_available() else "cpu"
else:
    mindspore_device = None
...
def get_gpu_count():
    """
    Return the number of available gpus (regardless of whether mindspore or tf is used)
    """
    if is_mindspore_available():
        import mindspore

        return mindspore.cuda.device_count()
