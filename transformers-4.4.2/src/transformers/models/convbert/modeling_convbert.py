...
def load_tf_weights_in_convbert(model, config, tf_checkpoint_path):
    ...
    for param in model.named_parameters():
        ...
        value = mindspore.from_numpy(tf_data[tf_name])
...
class SeparableConv1D(nn.Module):
    ...
    def __init__(self, config, input_filters, output_filters, kernel_size, **kwargs):
        ...
        self.bias = nn.Parameter(mindspore.zeros(output_filters, 1))
...
class ConvBertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        conv_attn_layer = mindspore.multiply(mixed_key_conv_attn_layer, mixed_query_layer)

        conv_kernel_layer = self.conv_kernel_layer(conv_attn_layer)
        conv_kernel_layer = mindspore.reshape(conv_kernel_layer, [-1, self.conv_kernel_size, 1])
        conv_kernel_layer = mindspore.softmax(conv_kernel_layer, dim=1)

        conv_out_layer = self.conv_out_layer(hidden_states)
        conv_out_layer = mindspore.reshape(conv_out_layer, [batch_size, -1, self.all_head_size])
        ...
        conv_out_layer = mindspore.reshape(conv_out_layer, [-1, self.attention_head_size, self.conv_kernel_size])
        conv_out_layer = mindspore.matmul(conv_out_layer, conv_kernel_layer)
        conv_out_layer = mindspore.reshape(conv_out_layer, [-1, self.all_head_size])
        ...
        attention_probs = mindspore.nn.functional.softmax(attention_scores, dim=-1)
        ...
        conv_out = mindspore.reshape(conv_out_layer, [batch_size, -1, self.num_attention_heads, self.attention_head_size])
        context_layer = mindspore.cat([context_layer, conv_out], 2)
...
class GroupedLinearLayer(nn.Module):
    def __init__(self, input_size, output_size, num_groups):
        ...
        self.weight = nn.Parameter(mindspore.Tensor(self.num_groups, self.group_in_dim, self.group_out_dim))
        self.bias = nn.Parameter(mindspore.Tensor(output_size))

    def forward(self, hidden_states):
        batch_size = list(hidden_states.size())[0]
        x = mindspore.reshape(hidden_states, [-1, self.num_groups, self.group_in_dim])
        x = x.permute(1, 0, 2)
        x = mindspore.matmul(x, self.weight)
        x = x.permute(1, 0, 2)
        x = mindspore.reshape(x, [batch_size, -1, self.output_size])
        x = x + self.bias
        return x
