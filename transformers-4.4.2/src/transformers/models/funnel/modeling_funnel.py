...
class FunnelAttentionStructure(nn.Module):
    ...
    def get_position_embeds(self, seq_len, dtype, device):
        ...
        if self.config.attention_type == "factorized":
            ...
            pos_seq = mindspore.arange(0, seq_len, 1.0, dtype=dtype, device=device)
            freq_seq = mindspore.arange(0, d_model // 2, 1.0, dtype=dtype, device=device)
            inv_freq = 1 / (10000 ** (freq_seq / (d_model // 2)))
            sinusoid = pos_seq[:, None] * inv_freq[None]
            sin_embed = mindspore.sin(sinusoid)
            sin_embed_d = self.sin_dropout(sin_embed)
            cos_embed = mindspore.cos(sinusoid)
            cos_embed_d = self.cos_dropout(cos_embed)
            # This is different from the formula on the paper...
            phi = mindspore.cat([sin_embed_d, sin_embed_d], dim=-1)
            psi = mindspore.cat([cos_embed, sin_embed], dim=-1)
            pi = mindspore.cat([cos_embed_d, cos_embed_d], dim=-1)
            omega = mindspore.cat([-sin_embed, cos_embed], dim=-1)
            ...
            rel_pos_id = mindspore.arange(-seq_len * 2, seq_len * 2, 1.0, dtype=dtype, device=device)
            zero_offset = seq_len * 2
            sinusoid = rel_pos_id[:, None] * inv_freq[None]
            sin_embed = self.sin_dropout(mindspore.sin(sinusoid))
            cos_embed = self.cos_dropout(mindspore.cos(sinusoid))
            pos_embed = mindspore.cat([sin_embed, cos_embed], dim=-1)

            pos = mindspore.arange(0, seq_len, dtype=dtype, device=device)
            pooled_pos = pos
            ...
            for block_index in range(0, self.config.num_blocks):
                ...
                if block_index == 0:
                    ...
                    position_embeds_pooling = mindspore.gather(pos_embed, 0, rel_pos)
                ...
                rel_pos = rel_pos[:, None] + zero_offset
                rel_pos = rel_pos.expand(rel_pos.size(0), d_model)
                position_embeds_no_pooling = mindspore.gather(pos_embed, 0, rel_pos)
    ...
    def stride_pool_pos(self, pos_id, block_index):
        ...
        if self.config.separate_cls:
            ...
            return mindspore.cat([cls_pos, pooled_pos_id[::2]], 0)
        else:
            return pos_id[::2]
    ...
    def relative_pos(self, pos, stride, pooled_pos=None, shift=1):
        ...
        return mindspore.arange(max_dist, min_dist - 1, -stride, dtype=mindspore.long, device=pos.device)
    ...
    def stride_pool(self, tensor, axis):
        ...
        if self.config.separate_cls:
            ...
            tensor = mindspore.cat([tensor[cls_slice], tensor], axis=axis)
    ...
    def pool_tensor(self, tensor, mode="mean", stride=2):
        ...
        if self.config.separate_cls:
            ...
            tensor = mindspore.cat([tensor[:, :1], suffix], dim=1)
...
def _relative_shift_gather(positional_attn, context_len, shift):
    ...
    # idxs = context_len + mindspore.arange(0, context_len).unsqueeze(0) - mindspore.arange(0, seq_len).unsqueeze(1)
    # # matrix of context_len + i-j
    # return positional_attn.gather(3, idxs.expand([batch_size, n_head, context_len, context_len]))

    positional_attn = mindspore.reshape(positional_attn, [batch_size, n_head, max_rel_len, seq_len])
    positional_attn = positional_attn[:, :, shift:, :]
    positional_attn = mindspore.reshape(positional_attn, [batch_size, n_head, seq_len, max_rel_len - shift])
...
class FunnelRelMultiheadAttention(nn.Module):
    def __init__(self, config, block_index):
        ...
        self.r_w_bias = nn.Parameter(mindspore.zeros([n_head, d_head]))
        self.r_r_bias = nn.Parameter(mindspore.zeros([n_head, d_head]))
        self.r_kernel = nn.Parameter(mindspore.zeros([d_model, n_head, d_head]))
        self.r_s_bias = nn.Parameter(mindspore.zeros([n_head, d_head]))
        self.seg_embed = nn.Parameter(mindspore.zeros([2, n_head, d_head]))
    ...
    def relative_positional_attention(self, position_embeds, q_head, context_len, cls_mask=None):
        ...
        if self.config.attention_type == "factorized":
            ...
            q_r_attention = mindspore.einsum("binh,dnh->bind", q_head + u, w_r)
            q_r_attention_1 = q_r_attention * phi[:, None]
            q_r_attention_2 = q_r_attention * pi[:, None]

            # Shape batch_size x n_head x seq_len x context_len
            positional_attn = mindspore.einsum("bind,jd->bnij", q_r_attention_1, psi) + mindspore.einsum(
            ...
            r_head = mindspore.einsum("td,dnh->tnh", r, w_r)
            # Shape batch_size x n_head x seq_len x max_rel_len
            positional_attn = mindspore.einsum("binh,tnh->bnit", q_head + v, r_head)
            # Shape batch_size x n_head x seq_len x context_len
    ...
    def relative_token_type_attention(self, token_type_mat, q_head, cls_mask=None):
        ...
        token_type_bias = mindspore.einsum("bind,snd->bnis", q_head + r_s_bias, self.seg_embed)
        # Shape batch_size x n_head x seq_len x context_len
        token_type_mat = token_type_mat[:, None].expand([batch_size, q_head.shape[2], seq_len, context_len])
        # Shapes batch_size x n_head x seq_len
        diff_token_type, same_token_type = mindspore.split(token_type_bias, 1, dim=-1)
        # Shape batch_size x n_head x seq_len x context_len
        token_type_attn = mindspore.where(
    ...
    def forward(self, query, key, value, attention_inputs, output_attentions=False):
        ...
        content_score = mindspore.einsum("bind,bjnd->bnij", q_head + r_w_bias, k_head)
        ...
        attn_prob = mindspore.softmax(attn_score, dim=-1, dtype=dtype)
        attn_prob = self.attention_dropout(attn_prob)

        # attention output, shape batch_size x seq_len x n_head x d_head
        attn_vec = mindspore.einsum("bnij,bjnd->bind", attn_prob, v_head)
...
def upsample(x, stride, target_len, separate_cls=True, truncate_seq=False):
    ...
    if separate_cls:
        cls = x[:, :1]
        x = x[:, 1:]
    output = mindspore.repeat_interleave(x, repeats=stride, dim=1)
    if separate_cls:
        if truncate_seq:
            output = nn.functional.pad(output, (0, 0, 0, stride - 1, 0, 0))
        output = output[:, : target_len - 1]
        output = mindspore.cat([cls, output], dim=1)
    else:
