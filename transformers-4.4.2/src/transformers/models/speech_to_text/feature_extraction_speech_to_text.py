...
from ...file_utils import PaddingStrategy, TensorType, is_mindspore_available, is_mindsporeaudio_available
...
if is_mindsporeaudio_available():
    import mindsporeaudio.compliance.kaldi as ta_kaldi
...
class Speech2TextFeatureExtractor(SequenceFeatureExtractor):
    ...
    This class extracts mel-filter bank features from raw speech using mindsporeAudio and applies utterance-level cepstral
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not is_mindsporeaudio_available():
            raise ImportError("`Speech2TextFeatureExtractor` requires mindsporeaudio: `pip install mindsporeaudio`.")
    ...
    def _extract_fbank_features(
        ...
        """
        Get mel-filter bank features using mindsporeAudio. Note that mindsporeAudio requires 16-bit signed integers as inputs
        and hence the waveform should not be normalized before feature extraction.
        """
        waveform = waveform * (2 ** 15)  # Kaldi compliance: 16-bit signed integers
        waveform = mindspore.from_numpy(waveform).unsqueeze(0)
