...
class Speech2TextSinusoidalPositionalEmbedding(nn.Module):
    ...
    def get_embedding(num_embeddings: int, embedding_dim: int, padding_idx: Optional[int] = None):
        ...
        emb = mindspore.exp(mindspore.arange(half_dim, dtype=mindspore.float) * -emb)
        emb = mindspore.arange(num_embeddings, dtype=mindspore.float).unsqueeze(1) * emb.unsqueeze(0)
        emb = mindspore.cat([mindspore.sin(emb), mindspore.cos(emb)], dim=1).view(num_embeddings, -1)
        if embedding_dim % 2 == 1:
            # zero pad
            emb = mindspore.cat([emb, mindspore.zeros(num_embeddings, 1)], dim=1)
...
class Speech2TextEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        """
        Args:
            hidden_states (:obj:`mindspore.FloatTensor`): input to the layer of shape :obj:`(seq_len, batch, embed_dim)`
...
class Speech2TextDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            attention_mask (:obj:`mindspore.FloatTensor`): attention mask of size
                :obj:`(batch, 1, tgt_len, src_len)` where padding elements are indicated by very large negative values.
            encoder_hidden_states (:obj:`mindspore.FloatTensor`): cross attention input to the layer of shape :obj:`(seq_len, batch, embed_dim)`
...
class Speech2TextPreTrainedModel(PreTrainedModel):
    ...
    def _get_subsampled_output_lengths(self, input_lengths: mindspore.LongTensor):
        """
    ...
    def _get_subsampled_encoder_attn_mask(self, attention_mask):
        ...
        attention_mask = mindspore.zeros((bsz, max_len), dtype=attention_mask.dtype, device=attention_mask.device)

        # these two operations makes sure that all values
        # before the output lengths indices are attended to
        attention_mask[(mindspore.arange(bsz, device=attention_mask.device), subsampled_lengths - 1)] = 1
...
SPEECH_TO_TEXT_INPUTS_DOCSTRING = r"""
    Args:
        input_features (:obj:`mindspore.LongTensor` of shape :obj:`(batch_size, sequence_length, feature_size)`):
            ...
            the fbank features, padding and conversion into a tensor of type :obj:`mindspore.FloatTensor`. See
    ...
    def forward(
        ...
        Args:
            input_features (:obj:`mindspore.LongTensor` of shape :obj:`(batch_size, sequence_length, feature_size)`):
                ...
                extracting the fbank features, padding and conversion into a tensor of type :obj:`mindspore.FloatTensor`.
        ...
        if attention_mask is None:
            padding_mask = mindspore.zeros_like(inputs_embeds, dtype=mindspore.long)
        else:
