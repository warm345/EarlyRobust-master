...
def _compute_mask_indices(
    ...
    <https://github.com/mindspore/fairseq/blob/e0788f7007a8473a76db573985031f3c94201e79/fairseq/data/data_utils.py#L376>`__.
    """
    bsz, all_sz = shape
...
class Wav2Vec2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            ...
            mindspore.nn.init.kaiming_normal_(module.weight.data)
        if isinstance(module, (nn.Linear, nn.Conv1d)) and module.bias is not None:
            module.bias.data.zero_()

    def _get_feat_extract_output_lengths(self, input_lengths: mindspore.LongTensor):
        """
        ...
        def _conv_out_length(input_length, kernel_size, stride):
            ...
            # from https://mindspore.org/docs/stable/generated/mindspore.nn.Conv1d.html
            return (input_length - kernel_size) // stride + 1

        for kernel_size, stride in zip(self.config.conv_kernel, self.config.conv_stride):
            input_lengths = _conv_out_length(input_lengths, kernel_size, stride)

        return input_lengths.to(mindspore.long)
...
WAV_2_VEC_2_INPUTS_DOCSTRING = r"""
    Args:
        input_values (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, sequence_length)`):
            ...
            be used for padding and conversion into a tensor of type `mindspore.FloatTensor`. See
...
class Wav2Vec2Model(Wav2Vec2PreTrainedModel):
    def __init__(self, config):
        ...
        self.masked_spec_embed = nn.Parameter(mindspore.FloatTensor(config.hidden_size).uniform_())
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            attention_mask = mindspore.zeros(
            ...
            attention_mask[
                (mindspore.arange(attention_mask.shape[0], device=hidden_states.device), output_lengths - 1)
            ] = 1
        ...
        if self.config.apply_spec_augment and self.training:
            ...
            if self.config.mask_time_prob > 0:
                ...
                hidden_states[mindspore.from_numpy(mask_time_indices)] = self.masked_spec_embed.to(hidden_states.dtype)
            ...
            if self.config.mask_feature_prob > 0:
                ...
                mask_feature_indices = mindspore.from_numpy(mask_feature_indices).to(hidden_states.device)
...
class Wav2Vec2ForMaskedLM(Wav2Vec2PreTrainedModel):
    ...
    def forward(
        ...
        r"""
        labels (:obj:`mindspore.LongTensor` of shape :obj:`(batch_size, sequence_length, hidden_size)`, `optional`):
        ...
        Example::
            ...
            >>> predicted_ids = mindspore.argmax(logits, dim=-1)
...
class Wav2Vec2ForCTC(Wav2Vec2PreTrainedModel):
    ...
    def forward(
        ...
        r"""
        labels (:obj:`mindspore.LongTensor` of shape :obj:`(batch_size, target_length)`, `optional`):
        ...
        if labels is not None:
            ...
            attention_mask = (
                attention_mask if attention_mask is not None else mindspore.ones_like(input_values, dtype=mindspore.long)
            )
