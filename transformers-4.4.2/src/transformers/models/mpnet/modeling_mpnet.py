...
class MPNetSelfAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        attention_scores = mindspore.matmul(q, k.transpose(-1, -2))
        ...
        c = mindspore.matmul(attention_probs, v)
...
class MPNetEncoder(nn.Module):
    ...
    def compute_position_bias(self, x, position_ids=None, num_buckets=32):
        ...
        if position_ids is not None:
            ...
            context_position = mindspore.arange(qlen, dtype=mindspore.long)[:, None]
            memory_position = mindspore.arange(klen, dtype=mindspore.long)[None, :]
    ...
    def relative_position_bucket(relative_position, num_buckets=32, max_distance=128):
        ...
        num_buckets //= 2
        ret += (n < 0).to(mindspore.long) * num_buckets
        n = mindspore.abs(n)
        ...
        val_if_large = max_exact + (
            mindspore.log(n.float() / max_exact) / math.log(max_distance / max_exact) * (num_buckets - max_exact)
        ).to(mindspore.long)

        val_if_large = mindspore.min(val_if_large, mindspore.full_like(val_if_large, num_buckets - 1))
        ret += mindspore.where(is_small, n, val_if_large)
        return ret
...
MPNET_INPUTS_DOCSTRING = r"""
    Args:
        ...
        attention_mask (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, sequence_length)`, `optional`, defaults to :obj:`None`):
