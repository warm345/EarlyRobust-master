...
#
# Original implementation: https://github.com/mindspore/fairseq/tree/master/examples/wmt19
# Authors:
...
#
"""mindspore Fairseq model, ported from https://github.com/mindspore/fairseq/tree/master/examples/wmt19"""
...
FSMT_INPUTS_DOCSTRING = r"""
    Args:
        ...
        encoder_outputs (:obj:`Tuple(mindspore.FloatTensor)`, `optional`):
            ...
            the decoder.
        past_key_values (:obj:`Tuple(mindspore.FloatTensor)` of length :obj:`config.n_layers` with each tuple having 4 tensors of shape :obj:`(batch_size, num_heads, sequence_length - 1, embed_size_per_head)`):
...
def triu_onnx(x, diagonal=0):
    l = x.shape[0]
    arange = mindspore.arange(l, device=x.device)
...
def _prepare_fsmt_decoder_inputs(
    ...
    causal_mask = triu_onnx(fill_with_neg_inf(mindspore.zeros(tgt_len, tgt_len)), 1).to(
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, encoder_padding_mask, layer_head_mask, output_attentions=False):
        """
        Args:
            x (:obj:`mindspore.Tensor`): input to the layer of shape `(seq_len, batch, embed_dim)`
            encoder_padding_mask (:obj:`mindspore.ByteTensor`): binary ByteTensor of shape
...
class FSMTEncoder(nn.Module):
    ...
    def forward(
        ...
        """
        Args:
            input_ids (:obj:`mindspore.LongTensor`): tokens in the source language of shape
                `(batch, src_len)`
            attention_mask (:obj:`mindspore.LongTensor`): indicating which indices are padding tokens
            head_mask (:obj:`mindspore.Tensor` of shape :obj:`(num_layers, num_heads)`, `optional`):
        ...
        Returns:
            BaseModelOutput or Tuple comprised of:
                ...
                - **x** (:obj:`mindspore.Tensor`): the last encoder layer's output of shape `(src_len, batch, embed_dim)`
                - **encoder_states** (:obj:`Tuple(mindspore.FloatTensor`)): all intermediate hidden states of shape
                  `(src_len, batch, embed_dim)`. Only populated if *output_hidden_states:* is True.
                - **all_attentions** (:obj:`Tuple(mindspore.FloatTensor`)): Attention weights for each layer.
...
class FSMTDecoder(nn.Module):
    ...
    def forward(
        ...
        Args:
            input_ids (:obj:`mindspore.LongTensor` of shape :obj:`(batch, tgt_len)`):
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        src_len = k.size(1)
        attn_weights = mindspore.bmm(q, k.transpose(1, 2))
        ...
        attn_output = mindspore.bmm(attn_probs, v)
    ...
    def _use_saved_state(self, k, v, saved_state, key_padding_mask, static_kv, bsz):
        ...
        if "prev_key" in saved_state:
            ...
            if static_kv:
                ...
                assert k is not None
                k = mindspore.cat([prev_key, k], dim=1)
        if "prev_value" in saved_state:
            ...
            if static_kv:
                ...
                assert v is not None
                v = mindspore.cat([prev_value, v], dim=1)
        ...
        if prev_key_padding_mask is not None:
            if static_kv:
                ...
                new_key_padding_mask = mindspore.cat([prev_key_padding_mask, key_padding_mask], dim=1)
        else:
...
class SinusoidalPositionalEmbedding(nn.Embedding):
    ...
    def make_positions(tensor, padding_idx: int):
        ...
        return (mindspore.cumsum(mask, dim=1).type_as(mask) * mask).long() + padding_idx
