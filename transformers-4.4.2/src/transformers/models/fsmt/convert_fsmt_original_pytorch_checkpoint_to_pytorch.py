...
def convert_fsmt_checkpoint_to_mindspore(fsmt_checkpoint_path, mindspore_dump_folder_path):

    # prep
    assert os.path.exists(fsmt_checkpoint_path)
    os.makedirs(mindspore_dump_folder_path, exist_ok=True)
    print(f"Writing results to {mindspore_dump_folder_path}")
    ...
    # weights, therefore we can't use mindspore.load() directly on the model file.
    ...
    data_root = dirname(mindspore_dump_folder_path)
    model_dir = basename(mindspore_dump_folder_path)
    ...
    src_vocab_file = os.path.join(mindspore_dump_folder_path, "vocab-src.json")
    ...
    tgt_vocab_file = os.path.join(mindspore_dump_folder_path, "vocab-tgt.json")
    print(f"Generating {tgt_vocab_file} of {tgt_vocab_size} of {tgt_lang} records")
    with open(tgt_vocab_file, "w", encoding="utf-8") as f:
        f.write(json.dumps(tgt_vocab, ensure_ascii=False, indent=json_indent))

    # merges_file (bpecodes)
    merges_file = os.path.join(mindspore_dump_folder_path, VOCAB_FILES_NAMES["merges_file"])
    ...
    # model config
    fsmt_model_config_file = os.path.join(mindspore_dump_folder_path, "config.json")
    ...
    # tokenizer config
    fsmt_tokenizer_config_file = os.path.join(mindspore_dump_folder_path, TOKENIZER_CONFIG_FILE)
    ...
    config = FSMTConfig.from_pretrained(mindspore_dump_folder_path)
    ...
    print(f"Generating {mindspore_weights_dump_path}")
    mindspore.save(model_state_dict, mindspore_weights_dump_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the official mindspore checkpoint file which is expected to reside in the dump dir with dicts, bpecodes, etc.",
    )
    parser.add_argument(
        "--mindspore_dump_folder_path", default=None, type=str, required=True, help="Path to the output mindspore model."
    )
    args = parser.parse_args()
    convert_fsmt_checkpoint_to_mindspore(args.fsmt_checkpoint_path, args.mindspore_dump_folder_path)
