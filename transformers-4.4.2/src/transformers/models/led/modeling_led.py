...
def _expand_mask(mask: mindspore.Tensor, dtype: mindspore.dtype, tgt_len: Optional[int] = None):
    ...
    expanded_attention_mask = inverted_mask.masked_fill(inverted_mask.bool(), mindspore.finfo(dtype).min)
...
class LEDEncoderSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if is_global_attn:
            ...
            attn_scores = mindspore.cat((global_key_attn_scores, attn_scores), dim=-1)

            # free memory
            del global_key_attn_scores

        attn_probs = F.softmax(attn_scores, dim=-1, dtype=mindspore.float32)  # use fp32 for numerical stability
        ...
        attn_probs = mindspore.masked_fill(attn_probs, is_index_masked[:, :, None, None], 0.0)
    ...
    def _sliding_chunks_query_key_matmul(self, query: mindspore.Tensor, key: mindspore.Tensor, window_overlap: int):
        ...
        diagonal_chunked_attention_scores = mindspore.einsum("bcxd,bcyd->bcxy", (query, key))  # multiply
    ...
    def _sliding_chunks_matmul_attn_probs_value(
        ...
        context = mindspore.einsum("bcwd,bcdh->bcwh", (chunked_attn_probs, chunked_value))
    ...
    def _get_global_attn_indices(is_index_global_attn):
        ...
        # helper variable
        is_local_index_global_attn = mindspore.arange(
    ...
    def _concat_with_global_key_attn_probs(
        ...
        attn_probs_from_global_key = mindspore.einsum("blhd,bshd->blhs", (query_vectors, key_vectors_only_global))
    ...
    def _compute_attn_output_with_global_indices(
        ...
        # attn = mindspore.einsum('blhs,bshd->blhd', (selected_attn_probs, selected_v))
        # compute attn output only global
        attn_output_only_global = mindspore.matmul(
    ...
    def _compute_global_attn_output_from_hidden(
        ...
        # compute attn scores
        global_attn_scores = mindspore.bmm(global_query_vectors_only_global, global_key_vectors.transpose(1, 2))
        ...
        # global attn output
        global_attn_output = mindspore.bmm(global_attn_probs, global_value_vectors)
...
class LEDEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        """
        Args:
            hidden_states (:obj:`mindspore.FloatTensor`): input to the layer of shape `(seq_len, batch, embed_dim)`
            attention_mask (:obj:`mindspore.FloatTensor`): attention mask of size
                `(batch, 1, tgt_len, src_len)` where padding elements are indicated by very large negative values.
            layer_head_mask (:obj:`mindspore.FloatTensor`): mask for attention heads in a given layer of size
...
class LEDDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            attention_mask (:obj:`mindspore.FloatTensor`): attention mask of size
                `(batch, 1, tgt_len, src_len)` where padding elements are indicated by very large negative values.
            encoder_hidden_states (:obj:`mindspore.FloatTensor`): cross attention input to the layer of shape `(seq_len, batch, embed_dim)`
            encoder_attention_mask (:obj:`mindspore.FloatTensor`): encoder attention mask of size
                `(batch, 1, tgt_len, src_len)` where padding elements are indicated by very large negative values.
            layer_head_mask (:obj:`mindspore.FloatTensor`): mask for attention heads in a given layer of size
                `(config.encoder_attention_heads,)`.
            encoder_layer_head_mask (:obj:`mindspore.FloatTensor`): mask for encoder attention heads in a given layer of
                size `(config.encoder_attention_heads,)`.
            past_key_value (:obj:`Tuple(mindspore.FloatTensor)`): cached past key and value projection states
...
class LEDEncoderBaseModelOutput(ModelOutput):
    ...
    Args:
        ...
        global_attentions (:obj:`tuple(mindspore.FloatTensor)`, `optional`, returned when ``output_attentions=True`` is passed or when ``config.output_attentions=True``):
            Tuple of :obj:`mindspore.FloatTensor` (one for each layer) of shape :obj:`(batch_size, num_heads,
    ...
    global_attentions: Optional[Tuple[mindspore.FloatTensor]] = None
...
class LEDSeq2SeqModelOutput(ModelOutput):
    ...
    Args:
        ...
        encoder_last_hidden_state (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, sequence_length, hidden_size)`, `optional`):
        ...
        encoder_attentions (:obj:`tuple(mindspore.FloatTensor)`, `optional`, returned when ``output_attentions=True`` is passed or when ``config.output_attentions=True``):
            ...
            self-attention heads.
        encoder_global_attentions (:obj:`tuple(mindspore.FloatTensor)`, `optional`, returned when ``output_attentions=True`` is passed or when ``config.output_attentions=True``):
            Tuple of :obj:`mindspore.FloatTensor` (one for each layer) of shape :obj:`(batch_size, num_heads,
            sequence_length, x)`, where ``x`` is the number of tokens with global attention mask.
        ...
        decoder_head_mask (:obj:`mindspore.Tensor` of shape :obj:`(num_layers, num_heads)`, `optional`):
    ...
    encoder_global_attentions: Optional[Tuple[mindspore.FloatTensor]] = None
    head_mask: Optional[mindspore.FloatTensor] = None
    decoder_head_mask: Optional[mindspore.FloatTensor] = None
...
class LEDSeq2SeqLMOutput(ModelOutput):
    ...
    Args:
        ...
        logits (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, sequence_length, config.vocab_size)`):
...
LED_INPUTS_DOCSTRING = r"""
    Args:
        ...
        decoder_input_ids (:obj:`mindspore.LongTensor` of shape :obj:`(batch_size, target_sequence_length)`, `optional`):
            ...
            :obj:`past_key_values`).
        decoder_attention_mask (:obj:`mindspore.LongTensor` of shape :obj:`(batch_size, target_sequence_length)`, `optional`):
        ...
        global_attention_mask (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, sequence_length)`, `optional`):
        ...
        past_key_values (:obj:`Tuple[Tuple[mindspore.Tensor]]` of length :obj:`config.n_layers` with each tuple having 2 tuples each of which has 2 tensors of shape :obj:`(batch_size, num_heads, sequence_length - 1, embed_size_per_head)`):
        ...
        decoder_inputs_embeds (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, target_sequence_length, hidden_size)`, `optional`):
...
class LEDEncoder(LEDPreTrainedModel):
    ...
    def _pad_to_window_size(
        ...
        if padding_len > 0:
            ...
            if inputs_embeds is not None:
                ...
                inputs_embeds = mindspore.cat([inputs_embeds, inputs_embeds_padding], dim=-2)
    ...
    def forward(
        ...
        if attention_mask is None:
            attention_mask = mindspore.ones(inputs_embeds.size()[:-1], device=inputs_embeds.device, dtype=mindspore.long)
    ...
    def forward(
        ...
        Args:
            ...
            global_attention_mask (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, sequence_length)`, `optional`):
                ...
                - 0 for local attention (a sliding window attention),
                - 1 for global attention (tokens that attend to all other tokens, and all other tokens attend to them).
            encoder_hidden_states (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, encoder_sequence_length, hidden_size)`, `optional`):
                Sequence of hidden-states at the output of the last layer of the encoder. Used in the cross-attention
                of the decoder.
            encoder_attention_mask (:obj:`mindspore.LongTensor` of shape :obj:`(batch_size, encoder_sequence_length)`, `optional`):
...
class LEDForConditionalGeneration(LEDPreTrainedModel):
    ...
    def __init__(self, config: LEDConfig):
        ...
        self.register_buffer("final_logits_bias", mindspore.zeros((1, self.led.shared.num_embeddings)))
...
class LEDForSequenceClassification(LEDPreTrainedModel):
    ...
    def forward(
        ...
        if len(mindspore.unique(eos_mask.sum(1))) > 1:
