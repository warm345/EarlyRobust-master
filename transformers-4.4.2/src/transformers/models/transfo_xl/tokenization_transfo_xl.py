...
from ...file_utils import cached_path, is_mindspore_available, mindspore_only_method
...
VOCAB_FILES_NAMES = {
    ...
    "pretrained_vocab_file_mindspore": "vocab.bin",
...
class TransfoXLTokenizer(PreTrainedTokenizer):
        ...
        **kwargs
        ...
        try:
            ...
            if pretrained_vocab_file is not None:
                # Priority on pickle files (support mindspore and TF)
                with open(pretrained_vocab_file, "rb") as f:
                    vocab_dict = pickle.load(f)

                # Loading a mindspore-saved transfo-xl vocab dict with pickle results in an integer
                # Entering this if statement means that we tried to load a mindspore-saved file with pickle, and we failed.
                # We therefore load it with mindspore, if it's available.
                if type(vocab_dict) == int:
                    if not is_mindspore_available():
                        raise ImportError(
                            "Not trying to load dict with mindspore as you need to install mindspore to load "
                            "from a mindspore pretrained vocabulary, "
                            "or activate it with environment variables USE_mindspore=1 and USE_TF=0."
                        )
                    vocab_dict = mindspore.load(pretrained_vocab_file)
    ...
    def convert_to_tensor(self, symbols):
        return mindspore.LongTensor(self.convert_tokens_to_ids(symbols))
...
class LMShuffledIterator(object):
    ...
    def stream_iterator(self, sent_stream):
        ...
        data = mindspore.LongTensor(self.bptt, self.bsz)
        target = mindspore.LongTensor(self.bptt, self.bsz)
...
class TransfoXLCorpus(object):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, cache_dir=None, *inputs, **kwargs):
        ...
        corpus_dict = mindspore.load(resolved_corpus_file)
        for key, value in corpus_dict.items():
            corpus.__dict__[key] = value
        corpus.vocab = vocab
        if corpus.train is not None:
            corpus.train = mindspore.tensor(corpus.train, dtype=mindspore.long)
        if corpus.valid is not None:
            corpus.valid = mindspore.tensor(corpus.valid, dtype=mindspore.long)
        if corpus.test is not None:
            corpus.test = mindspore.tensor(corpus.test, dtype=mindspore.long)
        return corpus
...
def get_lm_corpus(datadir, dataset):
    ...
    if os.path.exists(fn):
        ...
        corpus = mindspore.load(fn_pickle)
