...
from mindspore.autograd.function import Function
...
def _stable_argsort(vector, dim):
    # this function scales the vector so that mindspore.argsort is stable.
    # mindspore.argsort is not stable on its own
    scale_offset = mindspore.arange(vector.shape[dim], device=vector.device).view(1, 1, -1)
    scale_offset = scale_offset.expand(vector.shape)
    scaled_vector = vector.shape[dim] * vector + (scale_offset % vector.shape[dim])
    return mindspore.argsort(scaled_vector, dim=dim)
...
class AxialPositionEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        for axis, axial_pos_embd_dim in enumerate(self.axial_pos_embds_dim):
            ...
            self.weights.append(nn.Parameter(mindspore.ones(ax_shape, dtype=mindspore.float32)))
    ...
    def forward(self, position_ids):
        ...
        if self.training is True:
            ...
            if self.dropout > 0:
                weights = mindspore.cat(broadcasted_weights, dim=-1)
                ...
                position_encodings = mindspore.reshape(dropped_weights, (batch_size, sequence_length, -1))

            else:
                position_encodings = mindspore.cat(
                    [mindspore.reshape(weight, (batch_size, sequence_length, -1)) for weight in broadcasted_weights],
                    dim=-1,
                )
            ...
            position_encodings = mindspore.reshape(position_encodings, (batch_size, -1, position_encodings.shape[-1]))

            # select correct position encodings
            position_encodings = mindspore.cat(
                [
                    mindspore.index_select(position_encodings[i], 0, position_ids[i]).unsqueeze(0)
    ...
    def forward(self, input_ids=None, position_ids=None, inputs_embeds=None, start_idx_pos_encodings=0):
        ...
        if position_ids is None:
            position_ids = mindspore.arange(
                start_idx_pos_encodings, start_idx_pos_encodings + seq_length, dtype=mindspore.long, device=device
            )
...
class EfficientAttentionMixin:
    ...
    def _look_adjacent(self, vectors, num_chunks_before, num_chunks_after):
        ...
        for i in range(-num_chunks_before, num_chunks_after + 1):
            if i == 0:
                ...
                slices.append(mindspore.cat([vectors[:, :, i:, ...], vectors[:, :, :i, ...]], dim=2))
        return mindspore.cat(slices, dim=3)
    ...
    def _merge_hidden_size_dims(self, x, num_attn_heads, attn_head_size):
        ...
        """
        x = x.permute(0, 2, 1, 3)
        return mindspore.reshape(x, (x.size()[0], -1, num_attn_heads * attn_head_size))
    ...
    def _split_seq_length_dim_to(self, vectors, dim_factor_1, dim_factor_2, num_attn_heads, attn_head_size=None):
        ...
        if len(vectors.shape) == 4:
            return mindspore.reshape(vectors, split_dim_shape + (attn_head_size,))
        elif len(vectors.shape) == 3:
            return mindspore.reshape(vectors, split_dim_shape)
        else:
...
class LSHSelfAttention(nn.Module, EfficientAttentionMixin):
    def __init__(self, config):
        ...
        self.register_buffer("self_mask_value_float16", mindspore.tensor(-1e3))
        self.register_buffer("self_mask_value_float32", mindspore.tensor(-1e5))
        self.register_buffer("mask_value_float16", mindspore.tensor(-1e4))
        self.register_buffer("mask_value_float32", mindspore.tensor(-1e9))
        ...
        **kwargs
        ...
        if do_cached_attention:
            ...
            if past_buckets is not None:
                ...
                key_value_hidden_states = mindspore.cat([past_states, hidden_states], dim=1)
        ...
        if not do_standard_self_attention:
            ...
            sorted_bucket_idx_per_hash = mindspore.arange(sequence_length, device=query_key_vectors.device).repeat(
        ...
        if not do_standard_self_attention or (do_cached_attention and past_buckets is not None):
            ...
            if num_hashes > 1:
                ...
                probs_vectors = mindspore.exp(logits - mindspore.logsumexp(logits, dim=2, keepdim=True))
                out_vectors = mindspore.sum(out_vectors * probs_vectors, dim=2)
                # free memory
                del probs_vectors
    ...
    def _query_per_attn_head(self, hidden_states):
        ...
        query_key_vectors = mindspore.einsum("balh,ahr->balr", hidden_states, per_head_query_key)
    ...
    def _value_per_attn_head(self, hidden_states):
        ...
        # only relevant for inference and no bias => we can use einsum here
        value_vectors = mindspore.einsum("balh,ahr->balr", hidden_states, per_head_value)
    ...
    def _hash_vectors(self, vectors, num_hashes, attention_mask, increase_num_buckets=False):
        ...
        random_rotations = mindspore.randn(rotations_shape, device=vectors.device, dtype=vectors.dtype)
        # Output dim: Batch_Size x Num_Attn_Heads x Num_Hashes x Seq_Len x Num_Buckets/2
        rotated_vectors = mindspore.einsum("bmtd,mdhr->bmhtr", vectors, random_rotations)

        if isinstance(self.num_buckets, int) or len(self.num_buckets) == 1:
            rotated_vectors = mindspore.cat([rotated_vectors, -rotated_vectors], dim=-1)
            buckets = mindspore.argmax(rotated_vectors, dim=-1)
        else:
            ...
            for bucket_factor in self.num_buckets:
                ...
                rotated_vectors_factor = mindspore.cat([rotated_vectors_factor, -rotated_vectors_factor], dim=-1)
                if buckets is None:
                    buckets = mindspore.argmax(rotated_vectors_factor, dim=-1)
                else:
                    buckets = buckets + (cur_product * mindspore.argmax(rotated_vectors_factor, dim=-1))
        ...
        if attention_mask is not None and (attention_mask.sum().item() < batch_size * attention_mask.shape[-1]):
            ...
            buckets_mask = attention_mask.to(mindspore.uint8)[:, None, None, :].expand(buckets.shape)
            buckets = mindspore.where(
                buckets_mask, buckets, mindspore.tensor(num_buckets - 1, dtype=mindspore.long, device=buckets.device)
            )
        ...
        offsets = mindspore.arange(num_hashes, device=vectors.device)
    ...
    def _get_sorted_bucket_idx_and_undo_sorted_bucket_idx(self, sequence_length, buckets, num_hashes):
        ...
        with mindspore.no_grad():
            ...
            indices = (
                mindspore.arange(sorted_bucket_idx.shape[-1], device=buckets.device)
                .view(1, 1, -1)
    ...
    def _attend(
        ...
        query_key_dots = mindspore.matmul(query_vectors, key_vectors.transpose(-1, -2))
        ...
        if not do_standard_self_attention:
            ...
            query_bucket_idx = (query_key_dots.shape[-1] - 1) * mindspore.ones_like(query_key_dots)[:, :, :, -1]
            key_value_bucket_idx = mindspore.arange(
                query_key_dots.shape[-1], dtype=mindspore.long, device=query_key_dots.device
        ...
        if not do_cached_attention:
            ...
            if mask is not None:
                query_key_dots = mindspore.where(mask, query_key_dots, mask_value)
        ...
        self_mask = mindspore.ne(query_bucket_idx.unsqueeze(-1), key_value_bucket_idx.unsqueeze(-2)).to(
            query_bucket_idx.device
        )

        # apply self_mask
        query_key_dots = mindspore.where(self_mask, query_key_dots, self_mask_value)

        # free memory
        del self_mask

        logits = mindspore.logsumexp(query_key_dots, dim=-1, keepdim=True)
        ...
        attention_probs = mindspore.exp(query_key_dots - logits)
        ...
        # attend values
        out_vectors = mindspore.matmul(attention_probs, value_vectors)
    ...
    def _compute_attn_mask(
        ...
        if attention_mask is not None:
            ...
            attention_mask = attention_mask.to(mindspore.uint8)[:, None, :]
            if not do_standard_self_attention:
                # expand attn_mask to fit with key_value_bucket_idx shape
                attention_mask = attention_mask[:, None, :]
                attention_mask = attention_mask.expand(query_indices.shape[:-1] + (-1,))
                # extract attention mask from LSH sorted key_indices
                attention_mask = mindspore.gather(attention_mask, -1, key_indices)

            attention_mask = attention_mask.unsqueeze(-2).expand(query_key_dot_shape)

        # Causal mask
        if self.is_decoder is True:
            causal_mask = mindspore.ge(query_indices.unsqueeze(-1), key_indices.unsqueeze(-2)).to(query_indices.device)
    ...
    def _get_relevant_hid_states_and_buckets(
        ...
        hidden_states = mindspore.cat([past_states, hidden_states], dim=1)
        ...
        # concat buckets
        concat_buckets = mindspore.cat([past_buckets, query_buckets.unsqueeze(-1)], dim=-1)
        ...
        bucket_idx_batch_offset = sequence_length * (
            batch_size
            * mindspore.arange(relevant_bucket_idx_chunk.shape[-1], device=hidden_states.device, dtype=mindspore.long)
    ...
    def _expand_to_indices_in_relevant_chunk(self, indices, sequence_length):
        ...
        chunk_sequence_indices = expanded_start_indices + mindspore.arange(
            total_chunk_size, device=indices.device, dtype=mindspore.long
    ...
    def _len_and_dim_norm(self, vectors):
        ...
        vectors = vectors * mindspore.rsqrt(
            mindspore.tensor(self.attention_head_size, device=vectors.device, dtype=vectors.dtype)
        )
        return vectors
    ...
    def _len_norm(self, x, epsilon=1e-6):
        ...
        """
        variance = mindspore.mean(x ** 2, -1, keepdim=True)
        norm_x = x * mindspore.rsqrt(variance + epsilon)
        return norm_x
    ...
    def _gather_by_expansion(self, vectors, idxs, num_hashes):
        ...
        return mindspore.gather(vectors, 2, expanded_idxs)
...
class ReverseSort(Function):
    ...
    def forward(ctx, out_vectors, logits, sorted_bucket_idx, undo_sorted_bucket_idx):
        ...
        with mindspore.no_grad():
            ...
            out_vectors = mindspore.gather(out_vectors, 2, expanded_undo_sort_indices)
            logits = mindspore.gather(logits, 2, undo_sorted_bucket_idx)
    ...
    def backward(ctx, grad_out_vectors, grad_logits):
        ...
        grad_out_vectors = mindspore.gather(grad_out_vectors, 2, expanded_sort_indices)
        grad_logits = mindspore.gather(grad_logits, 2, sorted_bucket_idx)
    ...
    def forward(
        ...
        **kwargs
        ...
        if use_cache and past_buckets_states[1] is not None:
            ...
            key_value_hidden_states = mindspore.cat([key_value_hidden_states, hidden_states], dim=1)
        ...
        key_vectors = key_vectors / mindspore.sqrt(
            mindspore.tensor(self.attention_head_size, device=key_vectors.device, dtype=key_vectors.dtype)
        )

        # get sequence length indices
        indices = mindspore.arange(sequence_length, device=query_vectors.device).repeat(
            batch_size, self.num_attention_heads, 1
        )
    ...
    def forward(
        ...
        if use_cache:
            if past_buckets_states[self.layer_id][0] is None:
                ...
                past_buckets = mindspore.cat([past_buckets_states[self.layer_id][0], buckets], dim=-1)

            if past_buckets_states[self.layer_id][1] is None:
                # padded input should not be cached
                past_states = hidden_states[:, :orig_sequence_length]
            else:
                past_states = mindspore.cat([past_buckets_states[self.layer_id][1], hidden_states], dim=1)
...
class ReformerLayer(nn.Module):
    ...
    def _init_attention_seed(self):
        ...
        if hasattr(mindspore.cuda, "default_generators") and len(mindspore.cuda.default_generators) > 0:
            # GPU
            device_idx = mindspore.cuda.current_device()
            self.attention_seed = mindspore.cuda.default_generators[device_idx].seed()
        else:
            # CPU
    ...
    def _init_feed_forward_seed(self):
        ...
        if hasattr(mindspore.cuda, "default_generators") and len(mindspore.cuda.default_generators) > 0:
            # GPU
            device_idx = mindspore.cuda.current_device()
            self.feed_forward_seed = mindspore.cuda.default_generators[device_idx].seed()
        else:
            # CPU
    ...
    def backward_pass(
        ...
        # Implementation of RevNet (see Fig. 6 in https://towardsdatascience.com/illustrating-the-reformer-393575ac6ba0)
        # This code is heavily inspired by https://github.com/lucidrains/reformer-mindspore/blob/master/reformer_mindspore/reversible.py
...
class _ReversibleFunction(Function):
    ...
    heavily inspired by https://github.com/lucidrains/reformer-mindspore/blob/master/reformer_mindspore/reversible.py
    """
    ...
    def forward(
        ...
        hidden_states, attn_output = mindspore.chunk(hidden_states, 2, dim=-1)
        ...
        return mindspore.cat([attn_output, hidden_states], dim=-1)

    @staticmethod
    def backward(ctx, grad_hidden_states):
        grad_attn_output, grad_hidden_states = mindspore.chunk(grad_hidden_states, 2, dim=-1)
        ...
        grad_hidden_states = mindspore.cat([output.grad_attn_output, output.grad_hidden_states], dim=-1)
...
class ReformerEncoder(nn.Module):
    ...
    def forward(
        ...
        hidden_states = mindspore.cat([hidden_states, hidden_states], dim=-1)
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, AxialPositionEmbeddings):
            for weight in module.weights:
                mindspore.nn.init.normal_(weight, std=self.config.axial_norm_std)
...
class ReformerModelOutput(ModelOutput):
    ...
    Args:
        ...
        past_buckets_states (:obj:`List[Tuple(mindspore.LongTensor, mindspore.FloatTensor)]`, `optional`, returned when ``use_cache=True`` is passed or when ``config.use_cache=True``):
            List of :obj:`Tuple(mindspore.LongTensor, mindspore.FloatTensor` of length :obj:`config.n_layers`, with the first
        ...
        hidden_states (:obj:`tuple(mindspore.FloatTensor)`, `optional`, returned when ``output_hidden_states=True`` is passed or when ``config.output_hidden_states=True``):
            Tuple of :obj:`mindspore.FloatTensor` (one for the output of the embeddings and one for the output of each
    ...
    past_buckets_states: Optional[List[Tuple[mindspore.LongTensor, mindspore.FloatTensor]]] = None
...
class ReformerModelWithLMHeadOutput(ModelOutput):
    ...
    Args:
        ...
        past_buckets_states (:obj:`List[Tuple(mindspore.LongTensor, mindspore.FloatTensor)]`, `optional`, returned when ``use_cache=True`` is passed or when ``config.use_cache=True``):
            ...
            speed up sequential decoding.
        hidden_states (:obj:`tuple(mindspore.FloatTensor)`, `optional`, returned when ``output_hidden_states=True`` is passed or when ``config.output_hidden_states=True``):
            TTuple of :obj:`mindspore.FloatTensor` (one for the output of the embeddings and one for the output of each
            layer) of shape :obj:`(batch_size, sequence_length, hidden_size)`.
...
REFORMER_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`mindspore.LongTensor` of shape :obj:`(batch_size, sequence_length)`):
        ...
        attention_mask (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, sequence_length)`, `optional`):
        ...
        position_ids (:obj:`mindspore.LongTensor` of shape :obj:`(batch_size, sequence_length)`, `optional`):
        ...
        inputs_embeds (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, sequence_length, hidden_size)`, `optional`):
        ...
        past_buckets_states (:obj:`List[Tuple(mindspore.LongTensor, mindspore.FloatTensor)]`, `optional`):
            List of :obj:`Tuple(mindspore.LongTensor, mindspore.FloatTensor` of length :obj:`config.n_layers`, with the first
            element being the previous `buckets` of shape :obj:`(batch_size, num_heads, num_hashes, sequence_length)`)
            and the second being the previous `hidden_states` of shape :obj:`(batch_size, sequence_length,
            hidden_size)`).
...
class ReformerModel(ReformerPreTrainedModel):
    ...
    def _pad_to_mult_of_chunk_length(
        ...
        padded_input_ids = mindspore.full(
        ...
        if attention_mask is not None:
            pad_attention_mask = mindspore.zeros(input_shape[0], padding_length, device=device, dtype=attention_mask.dtype)

            attention_mask = mindspore.cat([attention_mask, pad_attention_mask], dim=-1)
        else:
            attention_mask = mindspore.cat(
                [
                    mindspore.ones(input_shape, device=device, dtype=mindspore.uint8),
                    mindspore.zeros((input_shape[0], padding_length), device=device, dtype=mindspore.uint8),
                ],
                dim=-1,
            )
        ...
        if input_ids is not None:
            input_ids = mindspore.cat([input_ids, padded_input_ids], dim=-1)
            input_shape = input_ids.size()

            # Pad position ids if given
            if position_ids is not None:
                padded_position_ids = mindspore.arange(input_shape[-1], padded_seq_length, dtype=mindspore.long, device=device)
                padded_position_ids = position_ids.unsqueeze(0).expand(input_shape[0], padding_length)
                position_ids = mindspore.cat([position_ids, padded_position_ids], dim=-1)

        # Extend `inputs_embeds` with padding to match least common multiple chunk_length
        if inputs_embeds is not None:
            padded_inputs_embeds = self.embeddings(padded_input_ids, position_ids)
            inputs_embeds = mindspore.cat([inputs_embeds, padded_inputs_embeds], dim=-2)
