...
def convert_tf_checkpoint_to_mindspore(
    task, reset_position_index_per_cell, tf_checkpoint_path, tapas_config_file, mindspore_dump_path
):
    ...
    # Save mindspore-model (weights and configuration)
    print("Save mindspore model to {}".format(mindspore_dump_path))
    model.save_pretrained(mindspore_dump_path[:-17])
    ...
    print("Save tokenizer files to {}".format(mindspore_dump_path))
    tokenizer.save_pretrained(mindspore_dump_path[:-17])
