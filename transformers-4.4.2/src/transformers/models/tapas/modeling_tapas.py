...
if is_scatter_available():
    from mindspore_scatter import scatter
...
class TableQuestionAnsweringOutput(ModelOutput):
    ...
    Args:
        loss (:obj:`mindspore.FloatTensor` of shape :obj:`(1,)`, `optional`, returned when :obj:`labels` (and possibly :obj:`answer`, :obj:`aggregation_labels`, :obj:`numeric_values` and :obj:`numeric_values_scale` are provided)):
            Total loss as the sum of the hierarchical cell selection log-likelihood loss and (optionally) the
            semi-supervised regression loss and (optionally) supervised loss for aggregations.
        logits (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, sequence_length)`):
            Prediction scores of the cell selection head, for every token.
        logits_aggregation (:obj:`mindspore.FloatTensor`, `optional`, of shape :obj:`(batch_size, num_aggregation_labels)`):
            Prediction scores of the aggregation head, for every aggregation operator.
        hidden_states (:obj:`tuple(mindspore.FloatTensor)`, `optional`, returned when ``output_hidden_states=True`` is passed or when ``config.output_hidden_states=True``):
            Tuple of :obj:`mindspore.FloatTensor` (one for the output of the embeddings + one for the output of each layer)
            of shape :obj:`(batch_size, sequence_length, hidden_size)`. Hidden-states of the model at the output of
            each layer plus the initial embedding outputs.
        attentions (:obj:`tuple(mindspore.FloatTensor)`, `optional`, returned when ``output_attentions=True`` is passed or when ``config.output_attentions=True``):
            Tuple of :obj:`mindspore.FloatTensor` (one for each layer) of shape :obj:`(batch_size, num_heads,
            sequence_length, sequence_length)`. Attentions weights after the attention softmax, used to compute the
            weighted average in the self-attention heads.
    """

    loss: Optional[mindspore.FloatTensor] = None
    logits: mindspore.FloatTensor = None
    logits_aggregation: mindspore.FloatTensor = None
...
def load_tf_weights_in_tapas(model, config, tf_checkpoint_path):
    """
    Load tf checkpoints in a mindspore model. This is an adaptation from load_tf_weights_in_bert
    ...
    for name, array in zip(names, arrays):
        ...
        pointer.data = mindspore.from_numpy(array)
    return model
...
class TapasEmbeddings(nn.Module):
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None):
        ...
        if position_ids is None:
            ...
            position_ids = mindspore.arange(seq_length, dtype=mindspore.long, device=device)
            ...
            if self.config.reset_position_index_per_cell:
                ...
                # shape (1, seq_len)
                position = mindspore.arange(seq_length, dtype=mindspore.long, device=device).unsqueeze(0)
                position_ids = mindspore.min(
                    mindspore.as_tensor(self.config.max_position_embeddings - 1, device=device), position - first_position
                )

        if token_type_ids is None:
            token_type_ids = mindspore.zeros(
                (input_shape + self.number_of_token_type_embeddings), dtype=mindspore.long, device=device
            )
...
class TapasSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            key_layer = mindspore.cat([past_key_value[0], key_layer], dim=2)
            value_layer = mindspore.cat([past_key_value[1], value_layer], dim=2)
        else:
            key_layer = self.transpose_for_scores(self.key(hidden_states))
            value_layer = self.transpose_for_scores(self.value(hidden_states))
        ...
        attention_scores = mindspore.matmul(query_layer, key_layer.transpose(-1, -2))
        ...
        context_layer = mindspore.matmul(attention_probs, value_layer)
...
TAPAS_START_DOCSTRING = r"""
    ...
    This model is also a mindspore `mindspore.nn.Module <https://mindspore.org/docs/stable/nn.html#mindspore.nn.Module>`__
    subclass. Use it as a regular mindspore Module and refer to the mindspore documentation for all matter related to
...
TAPAS_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`mindspore.LongTensor` of shape :obj:`({0})`):
        ...
        attention_mask (:obj:`mindspore.FloatTensor` of shape :obj:`({0})`, `optional`):
        ...
        token_type_ids (:obj:`mindspore.LongTensor` of shape :obj:`({0}, 7)`, `optional`):
        ...
        position_ids (:obj:`mindspore.LongTensor` of shape :obj:`({0})`, `optional`):
        ...
        head_mask (:obj:`mindspore.FloatTensor` of shape :obj:`(num_heads,)` or :obj:`(num_layers, num_heads)`, `optional`):
            Mask to nullify selected heads of the self-attention modules. Mask values selected in ``[0, 1]``: - 1
            indicates the head is **not masked**, - 0 indicates the head is **masked**.
        inputs_embeds (:obj:`mindspore.FloatTensor` of shape :obj:`({0}, hidden_size)`, `optional`):
...
class TapasModel(TapasPreTrainedModel):
    ...
    def forward(
        ...
        if attention_mask is None:
            attention_mask = mindspore.ones(input_shape, device=device)
        if token_type_ids is None:
            token_type_ids = mindspore.zeros(
                (*input_shape, len(self.config.type_vocab_sizes)), dtype=mindspore.long, device=device
            )

        # We can provide a self-attention mask of dimensions [batch_size, from_seq_length, to_seq_length]
        # ourselves in which case we just need to make it broadcastable to all heads.
        extended_attention_mask: mindspore.Tensor = self.get_extended_attention_mask(attention_mask, input_shape, device)
        ...
        if self.config.is_decoder and encoder_hidden_states is not None:
            ...
            if encoder_attention_mask is None:
                encoder_attention_mask = mindspore.ones(encoder_hidden_shape, device=device)
...
class TapasForMaskedLM(TapasPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs
        ...
        r"""
        labels (:obj:`mindspore.LongTensor` of shape :obj:`(batch_size, sequence_length)`, `optional`):
...
class TapasForQuestionAnswering(TapasPreTrainedModel):
    def __init__(self, config: TapasConfig):
        ...
        if config.init_cell_selection_weights_to_zero:
            ...
            self.output_weights = nn.Parameter(mindspore.zeros(config.hidden_size))
            self.column_output_weights = nn.Parameter(mindspore.zeros(config.hidden_size))
        else:
            self.output_weights = nn.Parameter(mindspore.empty(config.hidden_size))
            nn.init.normal_(
                self.output_weights, std=config.initializer_range
            )  # here, a truncated normal is used in the original implementation
            self.column_output_weights = nn.Parameter(mindspore.empty(config.hidden_size))
            nn.init.normal_(
                self.column_output_weights, std=config.initializer_range
            )  # here, a truncated normal is used in the original implementation
        self.output_bias = nn.Parameter(mindspore.zeros([]))
        self.column_output_bias = nn.Parameter(mindspore.zeros([]))
    ...
    def forward(
        ...
        r"""
        table_mask (:obj:`mindspore.LongTensor` of shape :obj:`(batch_size, seq_length)`, `optional`):
            Mask for the table. Indicates which tokens belong to the table (1). Question tokens, table headers and
            padding are 0.
        labels (:obj:`mindspore.LongTensor` of shape :obj:`(batch_size, seq_length)`, `optional`):
        ...
        aggregation_labels (:obj:`mindspore.LongTensor` of shape :obj:`(batch_size, )`, `optional`):
        ...
        float_answer (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, )`, `optional`):
        ...
        numeric_values (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, seq_length)`, `optional`):
        ...
        numeric_values_scale (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, seq_length)`, `optional`):
        ...
        row_index = IndexMap(
            indices=mindspore.min(row_ids, mindspore.as_tensor(self.config.max_num_rows - 1, device=row_ids.device)),
            num_segments=self.config.max_num_rows,
            batch_dims=1,
        )
        col_index = IndexMap(
            indices=mindspore.min(column_ids, mindspore.as_tensor(self.config.max_num_columns - 1, device=column_ids.device)),
        ...
        if table_mask is None:
            table_mask = mindspore.where(row_ids > 0, mindspore.ones_like(row_ids), mindspore.zeros_like(row_ids))
        # mindspore.FloatTensor[batch_size, seq_length]
        ...
        if labels is not None:
            ...
            dist_per_token = mindspore.distributions.Bernoulli(logits=logits)

            # Compute cell selection loss per example.
            selection_loss_per_example = None
            if not self.config.select_one_column:
                weight = mindspore.where(
                    labels == 0,
                    mindspore.ones_like(labels, dtype=mindspore.float32),
                    self.config.positive_label_weight * mindspore.ones_like(labels, dtype=mindspore.float32),
                )
                selection_loss_per_token = -dist_per_token.log_prob(labels) * weight
                selection_loss_per_example = mindspore.sum(selection_loss_per_token * input_mask_float, dim=1) / (
                    mindspore.sum(input_mask_float, dim=1) + EPSILON_ZERO_DIVISION
                )
            else:
            ...
            if self.config.disable_per_token_loss:
                ...
                total_loss += mindspore.mean(selection_loss_per_example)
            else:
                # For the not supervised case, do not assign loss for cell selection
                total_loss += mindspore.mean(selection_loss_per_example * (1.0 - aggregate_mask))
            ...
            if self.config.num_aggregation_labels > 0:
                if is_supervised:
                    ...
                    aggregation_labels = mindspore.zeros(labels.shape[0], dtype=mindspore.long, device=labels.device)
                    per_example_additional_loss = _calculate_aggregation_loss(
                        logits_aggregation,
                        aggregate_mask,
                        aggregation_labels,
                ...
                total_loss += mindspore.mean(per_example_additional_loss)

        else:
            # if no label ids are provided, set them to zeros in order to properly compute logits
            labels = mindspore.zeros_like(logits)
...
class TapasForSequenceClassification(TapasPreTrainedModel):
    ...
    def forward(
        ...
        r"""
        labels (:obj:`mindspore.LongTensor` of shape :obj:`(batch_size,)`, `optional`):
...
class IndexMap(object):
    ...
    def __init__(self, indices, num_segments, batch_dims=0):
        ...
        Args:
            indices (:obj:`mindspore.LongTensor`, same shape as a `values` Tensor to which the indices refer):
                Tensor containing the indices.
            num_segments (:obj:`mindspore.LongTensor`):
            ...
            batch_dims (:obj:`int`, `optional`, defaults to 0):
                ...
                index.
        """
        self.indices = mindspore.as_tensor(indices)
        self.num_segments = mindspore.as_tensor(num_segments, device=indices.device)
        self.batch_dims = batch_dims

    def batch_shape(self):
        return self.indices.size()[: self.batch_dims]  # returns a mindspore.Size object
...
class ProductIndexMap(IndexMap):
    ...
    def project_outer(self, index):
        ...
        return IndexMap(
            indices=(index.indices // self.inner_index.num_segments).type(mindspore.float).floor().type(mindspore.long),
    ...
    def project_inner(self, index):
        ...
        return IndexMap(
            indices=mindspore.fmod(index.indices, self.inner_index.num_segments)
            .type(mindspore.float)
            .floor()
            .type(mindspore.long),
...
def gather(values, index, name="segmented_gather"):
    ...
    Args:
        values (:obj:`mindspore.Tensor` of shape (B1, ..., Bn, num_segments, V1, ...)):
    ...
    Returns:
        :obj:`tuple(mindspore.Tensor)`: Tensor of shape (B1, ..., Bn, I1, ..., Ik, V1, ...) with the gathered values.
    """
    ...
    if len(values.shape[index.batch_dims :]) < 2:
        return mindspore.gather(
            values,
            index.batch_dims,
            indices.view(
                values.size()[0], -1
            ),  # mindspore.gather expects index to have the same number of dimensions as values
        ).view(indices.size())
    else:
        # this means we have a vectorized version
        # we have to adjust the index
        indices = indices.unsqueeze(-1).expand(values.shape)
        return mindspore.gather(values, index.batch_dims, indices)
...
def flatten(index, name="segmented_flatten"):
    ...
    batch_size = mindspore.prod(mindspore.tensor(list(index.batch_shape())))
    ...
    offset = mindspore.arange(start=0, end=batch_size, device=index.num_segments.device) * index.num_segments
...
def range_index_map(batch_shape, num_segments, name="range_index_map"):
    ...
    Args:
        batch_shape (:obj:`mindspore.Size`):
            Batch shape
    ...
    """
    batch_shape = mindspore.as_tensor(
        batch_shape, dtype=mindspore.long
    )  # create a rank 1 tensor vector containing batch_shape (e.g. [2])
    assert len(batch_shape.size()) == 1
    num_segments = mindspore.as_tensor(num_segments)  # create a rank 0 tensor (scalar) containing num_segments (e.g. 64)
    assert len(num_segments.size()) == 0

    indices = mindspore.arange(
        start=0, end=num_segments, device=num_segments.device
    )  # create a rank 1 vector with num_segments elements
    new_tensor = mindspore.cat(
        [mindspore.ones_like(batch_shape, dtype=mindspore.long, device=num_segments.device), num_segments.unsqueeze(dim=0)],
        dim=0,
    )
    ...
    multiples = mindspore.cat([batch_shape, mindspore.as_tensor([1])], dim=0)
    indices = indices.repeat(multiples.tolist())
    # equivalent (in Numpy:)
    # indices = mindspore.as_tensor(np.tile(indices.numpy(), multiples.tolist()))
...
def _segment_reduce(values, index, segment_reduce_fn, name):
    ...
    vector_shape = values.size()[len(index.indices.size()) :]  # mindspore.Size object
    flattened_shape = mindspore.cat(
        [mindspore.as_tensor([-1], dtype=mindspore.long), mindspore.as_tensor(vector_shape, dtype=mindspore.long)], dim=0
    )
    ...
    segment_means = scatter(
        src=flat_values,
        index=flat_index.indices.type(mindspore.long),
        dim=0,
    ...
    new_shape = mindspore.cat(
        [
            mindspore.as_tensor(index.batch_shape(), dtype=mindspore.long),
            mindspore.as_tensor([index.num_segments], dtype=mindspore.long),
            mindspore.as_tensor(vector_shape, dtype=mindspore.long),
        ],
        dim=0,
    )
...
def reduce_sum(values, index, name="segmented_reduce_sum"):
    ...
    Args:
        values (:obj:`mindspore.Tensor` of shape [B1, B2, ..., Bn, I1, .., Ik, V1, V2, ..]):
    ...
    Returns:
        output_values (:obj:`mindspore.Tensor`of shape [B1, B2, ..., Bn, num_segments, V1, V2, ..]): Tensor containing the
...
def compute_column_logits(
    ...
    Args:
        sequence_output (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, sequence_length, hidden_size)`):
            Also known as last_hidden_state. Sequence of hidden-states at the output of the last layer of the model.
        column_output_weights (:obj:`mindspore.FloatTensor` of shape :obj:`(hidden_size)`):
            Weights of the linear layer for column selection.
        column_output_bias (:obj:`mindspore.FloatTensor` of shape :obj:`()`):
            Bias of the linear layer for column selection.
        cell_index (:obj:`ProductIndexMap`):
            Index that groups tokens into cells.
        cell_mask (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, max_num_rows * max_num_cols)`):
            Mask for cells that exist in the table (i.e. that are not padding).
        allow_empty_column_selection (:obj:`bool`):
            Whether to allow not to select any column

    Returns:
        column_logits (:obj:`mindspore.FloatTensor`of shape :obj:`(batch_size, max_num_cols)`): Tensor containing the
        column logits for every example in the batch.
    """

    # First, compute the token logits (batch_size, seq_len) - without temperature
    token_logits = mindspore.einsum("bsj,j->bs", sequence_output, column_output_weights) + column_output_bias
    ...
    is_padding = mindspore.logical_and(cell_count < 0.5, ~mindspore.eq(out_index.indices, 0))
    column_logits += CLOSE_ENOUGH_TO_LOG_ZERO * mindspore.as_tensor(
        is_padding, dtype=mindspore.float32, device=is_padding.device
    )

    if not allow_empty_column_selection:
        column_logits += CLOSE_ENOUGH_TO_LOG_ZERO * mindspore.as_tensor(
            mindspore.eq(out_index.indices, 0), dtype=mindspore.float32, device=out_index.indices.device
        )
...
def _single_column_cell_selection_loss(token_logits, column_logits, labels, cell_index, col_index, cell_mask):
    ...
    Args:
        token_logits (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, sequence_length)`):
            Tensor containing the logits per token.
        column_logits (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, max_num_cols)`):
            Tensor containing the logits per column.
        labels (:obj:`mindspore.LongTensor` of shape :obj:`(batch_size, sequence_length)`):
            Labels per token.
        cell_index (:obj:`ProductIndexMap`):
            Index that groups tokens into cells.
    ...
    Returns:
        selection_loss_per_example (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size,)`): Loss for each example.
        logits (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, sequence_length)`): New logits which are only
    ...
    labels_per_column, _ = reduce_sum(mindspore.as_tensor(labels, dtype=mindspore.float32, device=labels.device), col_index)
    ...
    column_label = mindspore.argmax(labels_per_column, dim=-1)  # shape (batch_size,)
    ...
    no_cell_selected = mindspore.eq(
        mindspore.max(labels_per_column, dim=-1)[0], 0
    ...
    column_label = mindspore.where(
        no_cell_selected.view(column_label.size()), mindspore.zeros_like(column_label), column_label
    )

    column_dist = mindspore.distributions.Categorical(logits=column_logits)  # shape (batch_size, max_num_cols)
    ...
    labels_per_cell, labels_index = reduce_max(
        mindspore.as_tensor(labels, dtype=mindspore.long, device=labels.device), cell_index
    )
    ...
    column_mask = mindspore.as_tensor(
        mindspore.eq(column_id_for_cells, mindspore.unsqueeze(column_label, dim=-1)),
        dtype=mindspore.float32,
        device=cell_mask.device,
    )

    # Compute the log-likelihood for cells, but only for the selected column.
    cell_dist = mindspore.distributions.Bernoulli(logits=logits_per_cell)  # shape (batch_size, 64*32)
    cell_log_prob = cell_dist.log_prob(labels_per_cell.type(mindspore.float32))  # shape(batch_size, 64*32)

    cell_loss = -mindspore.sum(cell_log_prob * column_mask * cell_mask, dim=1)

    # We need to normalize the loss by the number of cells in the column.
    cell_loss /= mindspore.sum(column_mask * cell_mask, dim=1) + EPSILON_ZERO_DIVISION

    selection_loss_per_example = column_loss_per_example
    selection_loss_per_example += mindspore.where(
        no_cell_selected.view(selection_loss_per_example.size()),
        mindspore.zeros_like(selection_loss_per_example),
        cell_loss,
    )
    ...
    selected_column_id = mindspore.as_tensor(
        mindspore.argmax(column_logits, dim=-1), dtype=mindspore.long, device=column_logits.device
    )  # shape (batch_size,)

    # selected_column_mask: shape (batch_size, 64*32), equal to 1 if cell belongs to column selected by the model
    selected_column_mask = mindspore.as_tensor(
        mindspore.eq(column_id_for_cells, mindspore.unsqueeze(selected_column_id, dim=-1)),
        dtype=mindspore.float32,
        device=selected_column_id.device,
    )

    # Never select cells with the special column id 0.
    selected_column_mask = mindspore.where(
        mindspore.eq(column_id_for_cells, 0).view(selected_column_mask.size()),
        mindspore.zeros_like(selected_column_mask),
...
def compute_token_logits(sequence_output, temperature, output_weights, output_bias):
    ...
    Args:
        ...
        output_weights (:obj:`mindspore.FloatTensor` of shape :obj:`(hidden_size,)`):
            Weights of the linear layer for cell selection.
        output_bias (:obj:`mindspore.FloatTensor` of shape :obj:`()`):
            Bias of the linear layer for cell selection

    Returns:
        logits (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, sequence_length)`): Logits per token.
    """
    logits = (mindspore.einsum("bsj,j->bs", sequence_output, output_weights) + output_bias) / temperature
...
def _calculate_aggregate_mask(answer, pooled_output, cell_selection_preference, labels, aggregation_classifier):
    ...
    Args:
        answer (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, )`):
            Answer for every example in the batch. Nan if there is no scalar answer.
        pooled_output (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, hidden_size)`):
        ...
        labels (:obj:`mindspore.LongTensor` of shape :obj:`(batch_size, sequence_length)`):
            Labels per token. aggregation_classifier (:obj:`mindspore.nn.Linear`): Aggregation head

    Returns:
        aggregate_mask (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size,)`): A mask set to 1 for examples that
        should use aggregation functions.
    """
    # mindspore.FloatTensor(batch_size,)
    aggregate_mask_init = mindspore.logical_not(mindspore.isnan(answer)).type(mindspore.FloatTensor).to(answer.device)
    logits_aggregation = aggregation_classifier(pooled_output)
    dist_aggregation = mindspore.distributions.categorical.Categorical(logits=logits_aggregation)
    # Index 0 correponds to "no aggregation".
    aggregation_ops_total_mass = mindspore.sum(dist_aggregation.probs[:, 1:], dim=1)
    ...
    is_cell_supervision_available = mindspore.sum(labels, dim=1) > 0

    # mindspore.where is not equivalent to tf.where (in tensorflow 1)
    # hence the added .view on the condition to match the shape of the first tensor
    aggregate_mask = mindspore.where(
        mindspore.logical_and(is_pred_cell_selection, is_cell_supervision_available).view(aggregate_mask_init.size()),
        mindspore.zeros_like(aggregate_mask_init, dtype=mindspore.float32),
        aggregate_mask_init,
    )
...
def _calculate_aggregation_loss_known(
    ...
    Args:
        logits_aggregation (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, num_aggregation_labels)`):
            Logits per aggregation operation.
        aggregate_mask (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, )`):
            A mask set to 1 for examples that should use aggregation functions.
        aggregation_labels (:obj:`mindspore.LongTensor` of shape :obj:`(batch_size, )`):
    ...
    Returns:
        aggregation_loss_known (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size,)`): Aggregation loss (when its
        type is known during training) per example.
    """
    if use_answer_as_supervision:
        # Prepare "no aggregation" targets for cell selection examples.
        target_aggregation = mindspore.zeros_like(aggregate_mask, dtype=mindspore.long)
    else:
        # Use aggregation supervision as the target.
        target_aggregation = aggregation_labels

    one_hot_labels = mindspore.nn.functional.one_hot(target_aggregation, num_classes=num_aggregation_labels).type(
        mindspore.float32
    )
    log_probs = mindspore.nn.functional.log_softmax(logits_aggregation, dim=-1)

    # mindspore.FloatTensor[batch_size]
    per_example_aggregation_intermediate = -mindspore.sum(one_hot_labels * log_probs, dim=-1)
    if use_answer_as_supervision:
...
def _calculate_aggregation_loss_unknown(logits_aggregation, aggregate_mask):
    ...
    Returns:
        aggregation_loss_unknown (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size,)`): Aggregation loss (in case of
    ...
    # correct answer.
    return -mindspore.log(aggregation_ops_total_mass) * aggregate_mask
...
def _calculate_aggregation_loss(
    ...
    Returns:
        aggregation_loss (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size,)`): Aggregation loss per example.
    """
...
def _calculate_expected_result(
    ...
    Args:
        dist_per_cell (:obj:`mindspore.distributions.Bernoulli`):
            Cell selection distribution for each cell.
        numeric_values (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, seq_length)`):
            Numeric values of every token. Nan for tokens which are not numeric values.
        numeric_values_scale (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, seq_length)`):
            Scale of the numeric values of every token.
        input_mask_float (:obj: `mindspore.FloatTensor` of shape :obj:`(batch_size, seq_length)`):
    ...
    Returns:
        expected_result (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size,)`): The expected result per example.
    """
    if config.use_gumbel_for_cells:
        gumbel_dist = mindspore.distributions.RelaxedBernoulli(
    ...
    count_result = mindspore.sum(scaled_probability_per_cell, dim=1)
    numeric_values_masked = mindspore.where(
        mindspore.isnan(numeric_values), mindspore.zeros_like(numeric_values), numeric_values
    )  # Mask non-numeric table values to zero.
    sum_result = mindspore.sum(scaled_probability_per_cell * numeric_values_masked, dim=1)
    ...
    if avg_approximation == AverageApproximationFunction.RATIO:
        ...
        ex = mindspore.sum(scaled_probability_per_cell, dim=1, keepdim=True) - scaled_probability_per_cell + 1
        average_result = mindspore.sum(numeric_values_masked * scaled_probability_per_cell / ex, dim=1)
    elif avg_approximation == AverageApproximationFunction.SECOND_ORDER:
        # The sum of all probabilities except that correspond to other cells
        ex = mindspore.sum(scaled_probability_per_cell, dim=1, keepdim=True) - scaled_probability_per_cell + 1
        pointwise_var = scaled_probability_per_cell * (1 - scaled_probability_per_cell)
        var = mindspore.sum(pointwise_var, dim=1, keepdim=True) - pointwise_var

        multiplier = (var / mindspore.square(ex) + 1) / ex
        average_result = mindspore.sum(numeric_values_masked * scaled_probability_per_cell * multiplier, dim=1)
    else:
    ...
    if config.use_gumbel_for_aggregation:
        gumbel_dist = mindspore.distributions.RelaxedOneHotCategorical(
        ...
        # <float32>[batch_size, num_aggregation_labels - 1]
        aggregation_op_only_probs = mindspore.nn.functional.softmax(
            logits_aggregation[:, 1:] / config.aggregation_temperature, dim=-1
        )

    all_results = mindspore.cat(
        [
            mindspore.unsqueeze(sum_result, dim=1),
            mindspore.unsqueeze(average_result, dim=1),
            mindspore.unsqueeze(count_result, dim=1),
        ],
        dim=1,
    )

    expected_result = mindspore.sum(all_results * aggregation_op_only_probs, dim=1)
...
def huber_loss(input, target, delta: float = 1.0):
    errors = mindspore.abs(input - target)  # shape (batch_size,)
    return mindspore.where(errors < delta, 0.5 * errors ** 2, errors * delta - (0.5 * delta ** 2))
...
def _calculate_regression_loss(
    ...
    Returns:
        per_example_answer_loss_scaled (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size,)`): Scales answer loss for
        each example in the batch. large_answer_loss_mask (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size,)`): A
    ...
    # float32 (batch_size,)
    answer_masked = mindspore.where(mindspore.isnan(answer), mindspore.zeros_like(answer), answer)

    if config.use_normalized_answer_loss:
        normalizer = (mindspore.max(mindspore.abs(expected_result), mindspore.abs(answer_masked)) + EPSILON_ZERO_DIVISION).detach()
    ...
    if config.answer_loss_cutoff is None:
        large_answer_loss_mask = mindspore.ones_like(per_example_answer_loss, dtype=mindspore.float32)

    else:
        large_answer_loss_mask = mindspore.where(
            per_example_answer_loss > config.answer_loss_cutoff,
            mindspore.zeros_like(per_example_answer_loss, dtype=mindspore.float32),
            mindspore.ones_like(per_example_answer_loss, dtype=mindspore.float32),
        )
