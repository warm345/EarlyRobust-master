...
def convert_encoder_layer(opus_dict, layer_prefix: str, converter: dict):
    ...
    for k in opus_dict:
        ...
        sd[converter[stripped]] = mindspore.tensor(v).squeeze()
    return sd


def load_layers_(layer_lst: mindspore.nn.ModuleList, opus_state: dict, converter, is_decoder=False):
...
def convert_whole_dir(path=Path("marian_ckpt/")):
    for subdir in tqdm(list(path.ls())):
        ...
        if (dest_dir / "mindspore_model.bin").exists():
            continue
...
class OpusState:
    ...
    def load_marian_model(self) -> MarianMTModel:
        ...
        wemb_tensor = mindspore.nn.Parameter(mindspore.FloatTensor(self.wemb))
        bias_tensor = mindspore.nn.Parameter(mindspore.FloatTensor(self.final_bias))
        ...
        if "Wpos" in state_dict:
            ...
            wpos_tensor = mindspore.tensor(state_dict["Wpos"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dest", type=str, default=None, help="Path to the output mindspore model.")
