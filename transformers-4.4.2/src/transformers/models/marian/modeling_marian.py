...
class MarianSinusoidalPositionalEmbedding(nn.Embedding):
    ...
    def _init_weight(out: nn.Parameter):
        ...
        out.requires_grad = False  # set early to avoid an error in mindspore-1.8+
        sentinel = dim // 2 if dim % 2 == 0 else (dim // 2) + 1
        out[:, 0:sentinel] = mindspore.FloatTensor(np.sin(position_enc[:, 0::2]))
        out[:, sentinel:] = mindspore.FloatTensor(np.cos(position_enc[:, 1::2]))
        out.detach_()
        return out
...
MARIAN_GENERATION_EXAMPLE = r"""
        mindspore version of marian-nmt's transformer.h (c++). Designed for the OPUS-NMT translation checkpoints.
