    ...
    with tf.Session() as sess:
        ...
        for key in list(all_variables.keys()):
            ...
            try:
                ...
                raise
            logger.info(f"Initialize mindspore weight {key}")

            model_pointer.data = mindspore.from_numpy(array.astype(np.float32))
            keep_track_variables.pop(key, None)

        logger.info("Weights not copied to mindspore model: {}".format(", ".join(keep_track_variables.keys())))
        return model
...
class BertGenerationEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.LayerNorm = mindspore.nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
