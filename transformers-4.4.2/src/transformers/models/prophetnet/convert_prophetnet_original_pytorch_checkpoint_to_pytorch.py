...
def convert_prophetnet_checkpoint_to_mindspore(prophetnet_checkpoint_path: str, mindspore_dump_folder_path: str):
    """
        ...
        for attribute in attributes:
            ...
            if attribute == "weight":
                ...
                if attribute == "query_proj":
                    model.query_proj.weight = mindspore.nn.Parameter(old_model.in_proj_weight[:embed_dim, :])
                    model.query_proj.bias = mindspore.nn.Parameter(old_model.in_proj_bias[:embed_dim])

                elif attribute == "key_proj":
                    model.key_proj.weight = mindspore.nn.Parameter(old_model.in_proj_weight[embed_dim : 2 * embed_dim, :])
                    model.key_proj.bias = mindspore.nn.Parameter(old_model.in_proj_bias[embed_dim : 2 * embed_dim])
                elif attribute == "value_proj":
                    model.value_proj.weight = mindspore.nn.Parameter(old_model.in_proj_weight[2 * embed_dim :, :])
                    model.value_proj.bias = mindspore.nn.Parameter(old_model.in_proj_bias[2 * embed_dim :])
                is_key_init = True
                break
                ...
                model.position_embeddings.weight = mindspore.nn.Parameter(old_model.embed_positions.weight[:512, :])
                is_key_init = True
                break
    ...
    print(f"Saving model to {mindspore_dump_folder_path}")
    prophet.save_pretrained(mindspore_dump_folder_path)
...
if __name__ == "__main__":
    ...
    # Required parameters
    parser.add_argument(
        "--prophetnet_checkpoint_path", default=None, type=str, required=True, help="Path the official mindspore dump."
    )
    parser.add_argument(
        "--mindspore_dump_folder_path", default=None, type=str, required=True, help="Path to the output mindspore model."
    )
    args = parser.parse_args()
    convert_prophetnet_checkpoint_to_mindspore(args.prophetnet_checkpoint_path, args.mindspore_dump_folder_path)
