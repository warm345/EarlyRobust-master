...
PROPHETNET_START_DOCSTRING = r"""
    ...
    file ``convert_prophetnet_original_mindspore_checkpoint_to_mindspore.py``.

    This model is a mindspore `mindspore.nn.Module <https://mindspore.org/docs/stable/nn.html#mindspore.nn.Module>`_ sub-class. Use
    it as a regular mindspore Module and refer to the mindspore documentation for all matters related to general usage and
    behavior.
...
PROPHETNET_INPUTS_DOCSTRING = r"""
    Args:
        ...
        decoder_input_ids (:obj:`mindspore.LongTensor` of shape :obj:`(batch_size, target_sequence_length)`, `optional`):
        ...
        decoder_attention_mask (:obj:`mindspore.BoolTensor` of shape :obj:`(batch_size, target_sequence_length)`, `optional`):
        ...
        encoder_outputs (:obj:`tuple(tuple(mindspore.FloatTensor)`, `optional`):
            Tuple consists of (:obj:`last_hidden_state`, `optional`: :obj:`hidden_states`, `optional`:
            :obj:`attentions`) :obj:`last_hidden_state` of shape :obj:`(batch_size, sequence_length, hidden_size)`,
            `optional`) is a sequence of hidden-states at the output of the last layer of the encoder. Used in the
            cross-attention of the decoder.
        past_key_values (:obj:`tuple(tuple(mindspore.FloatTensor))` of length :obj:`config.n_layers` with each tuple having 4 tensors of shape :obj:`(batch_size, num_heads, sequence_length - 1, embed_size_per_head)`):
...
def ngram_attention_bias(sequence_length, ngram, device, dtype):
    ...
    """
    left_block = mindspore.ones((ngram, sequence_length, sequence_length), device=device, dtype=dtype) * float("-inf")
    ...
    left_block[:, :, 0] = 0
    return mindspore.cat([left_block, right_block], dim=2)
...
def compute_relative_buckets(num_buckets, max_distance, relative_positions, is_bidirectional=False):
    ...
    if is_bidirectional:
        ...
        rel_positions_bucket = (
            rel_positions_bucket
            + mindspore.lt(inv_relative_positions, mindspore.zeros_like(inv_relative_positions)).int() * num_buckets
        )
        inv_relative_positions = mindspore.abs(inv_relative_positions)
    else:
        inv_relative_positions = mindspore.max(inv_relative_positions, mindspore.zeros_like(inv_relative_positions))

    max_exact = num_buckets // 2
    is_small = mindspore.lt(inv_relative_positions, max_exact)
    val_if_large = max_exact + mindspore.log(inv_relative_positions.float() / max_exact) / math.log(
        max_distance / max_exact
    ) * (num_buckets - max_exact)
    val_if_large = mindspore.min(val_if_large, mindspore.ones_like(val_if_large) * (num_buckets - 1)).int()
    rel_positions_bucket = rel_positions_bucket + mindspore.where(is_small, inv_relative_positions.int(), val_if_large)
...
def compute_all_stream_relative_buckets(num_buckets, max_distance, position_ids):
    ...
    # predicting stream
    predicting_stream_relative_positions = mindspore.cat((position_ids - 1, position_ids), dim=-1).unsqueeze(1)
...
class ProphetNetSeq2SeqLMOutput(ModelOutput):
    ...
    Args:
        ...
        logits (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, decoder_sequence_length, config.vocab_size)`):
            Prediction scores of the main stream language modeling head (scores for each vocabulary token before
            SoftMax).
        logits_ngram (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, ngram * decoder_sequence_length, config.vocab_size)`):
            Prediction scores of the predict stream language modeling head (scores for each vocabulary token before
            SoftMax).
        past_key_values (:obj:`List[mindspore.FloatTensor]`, `optional`, returned when ``use_cache=True`` is passed or when ``config.use_cache=True``):
            List of :obj:`mindspore.FloatTensor` of length :obj:`config.n_layers`, with each tensor of shape :obj:`(2,
        ...
        decoder_hidden_states (:obj:`tuple(mindspore.FloatTensor)`, `optional`, returned when ``output_hidden_states=True`` is passed or when ``config.output_hidden_states=True``):
        ...
        decoder_ngram_hidden_states (:obj:`tuple(mindspore.FloatTensor)`, `optional`, returned when ``output_hidden_states=True`` is passed or when ``config.output_hidden_states=True``):
            Tuple of :obj:`mindspore.FloatTensor` (one for the output of the embeddings + one for the output of each layer)
            of shape :obj:`(batch_size, ngram * decoder_sequence_length, hidden_size)`.

            Hidden-states of the predict stream of the decoder at the output of each layer plus the initial embedding
            outputs.
        decoder_attentions (:obj:`tuple(mindspore.FloatTensor)`, `optional`, returned when ``output_attentions=True`` is passed or when ``config.output_attentions=True``):
            Tuple of :obj:`mindspore.FloatTensor` (one for each layer) of shape :obj:`(batch_size, num_attn_heads,
            decoder_sequence_length, decoder_sequence_length)`.

            Attentions weights of the decoder, after the attention softmax, used to compute the weighted average in the
            self-attention heads.
        decoder_ngram_attentions (:obj:`tuple(mindspore.FloatTensor)`, `optional`, returned when ``output_attentions=True`` is passed or when ``config.output_attentions=True``):
            Tuple of :obj:`mindspore.FloatTensor` (one for each layer) of shape :obj:`(batch_size, num_attn_heads,
            decoder_sequence_length, decoder_sequence_length)`.

            Attentions weights of the predict stream of the decoder, after the attention softmax, used to compute the
            weighted average in the self-attention heads.
        cross_attentions (:obj:`tuple(mindspore.FloatTensor)`, `optional`, returned when ``output_attentions=True`` is passed or when ``config.output_attentions=True``):
            Tuple of :obj:`mindspore.FloatTensor` (one for each layer) of shape :obj:`(batch_size, num_attn_heads,
        ...
        encoder_last_hidden_state (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, encoder_sequence_length, hidden_size)`, `optional`):
        ...
        encoder_attentions (:obj:`tuple(mindspore.FloatTensor)`, `optional`, returned when ``output_attentions=True`` is passed or when ``config.output_attentions=True``):
            Tuple of :obj:`mindspore.FloatTensor` (one for each layer) of shape :obj:`(batch_size, num_attn_heads,
    ...
    logits_ngram: Optional[mindspore.FloatTensor] = None
    past_key_values: Optional[Tuple[mindspore.FloatTensor]] = None
    decoder_hidden_states: Optional[Tuple[mindspore.FloatTensor]] = None
    decoder_ngram_hidden_states: Optional[Tuple[mindspore.FloatTensor]] = None
    decoder_attentions: Optional[Tuple[mindspore.FloatTensor]] = None
    decoder_ngram_attentions: Optional[Tuple[mindspore.FloatTensor]] = None
...
class ProphetNetSeq2SeqModelOutput(ModelOutput):
    ...
    Args:
        last_hidden_state (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, decoder_sequence_length, hidden_size)`):
            Sequence of main stream hidden-states at the output of the last layer of the decoder of the model.

            If :obj:`past_key_values` is used only the last hidden-state of the sequences of shape :obj:`(batch_size,
            1, hidden_size)` is output.
        last_hidden_state_ngram (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size,ngram * decoder_sequence_length, config.vocab_size)`):
    ...
    last_hidden_state_ngram: Optional[mindspore.FloatTensor] = None
    past_key_values: Optional[Tuple[mindspore.FloatTensor]] = None
    decoder_hidden_states: Optional[Tuple[mindspore.FloatTensor]] = None
    decoder_ngram_hidden_states: Optional[Tuple[mindspore.FloatTensor]] = None
    decoder_attentions: Optional[Tuple[mindspore.FloatTensor]] = None
...
class ProphetNetDecoderModelOutput(ModelOutput):
    ...
    Args:
        ...
        hidden_states (:obj:`tuple(mindspore.FloatTensor)`, `optional`, returned when ``output_hidden_states=True`` is passed or when ``config.output_hidden_states=True``):
            ...
            Hidden-states of main stream of the decoder at the output of each layer plus the initial embedding outputs.
        ngram_hidden_states (:obj:`tuple(mindspore.FloatTensor)`, `optional`, returned when ``output_hidden_states=True`` is passed or when ``config.output_hidden_states=True``):
            Tuple of :obj:`mindspore.FloatTensor` (one for the output of the embeddings + one for the output of each layer)
            of shape :obj:`(batch_size, ngram * decoder_sequence_length, hidden_size)`.
        ...
        attentions (:obj:`tuple(mindspore.FloatTensor)`, `optional`, returned when ``output_attentions=True`` is passed or when ``config.output_attentions=True``):
            ...
            Attentions weights of the decoder, after the attention softmax, used to compute the weighted average in the
            self-attention heads.
        ngram_attentions (:obj:`tuple(mindspore.FloatTensor)`, `optional`, returned when ``output_attentions=True`` is passed or when ``config.output_attentions=True``):
            Tuple of :obj:`mindspore.FloatTensor` (one for each layer) of shape :obj:`(batch_size, num_attn_heads,
            decoder_sequence_length, decoder_sequence_length)`.
    ...
    hidden_states_ngram: Optional[Tuple[mindspore.FloatTensor]] = None
    attentions: Optional[Tuple[mindspore.FloatTensor]] = None
    ngram_attentions: Optional[Tuple[mindspore.FloatTensor]] = None
    cross_attentions: Optional[Tuple[mindspore.FloatTensor]] = None
...
class ProphetNetPreTrainedModel(PreTrainedModel):
    ...
    def _shift_right(self, input_ids):
        ...
        assert mindspore.all(shifted_input_ids >= 0).item(), "Verify that `shifted_input_ids` has only positive values"
    ...
    def forward(self, inputs_shape, device, attention_mask=None, past_key_values=None, position_ids=None):
        ...
        if position_ids is None:
            if past_key_values is not None:
                ...
                position_ids = mindspore.ones((1, 1), dtype=mindspore.long, device=device) * (
                    int(self.padding_idx + num_input_ids)
                )
            else:
                if attention_mask is None:
                    attention_mask = mindspore.ones(inputs_shape, dtype=mindspore.long, device=device)

                # retrieve position_ids from input_ids / attention_mask
                position_ids = (
                    mindspore.cumsum(attention_mask, dim=1).type_as(attention_mask) * attention_mask
...
class ProphetNetNgramSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if past_key_value is not None:
            ...
            main_key_states = mindspore.cat((prev_main_key_states, main_key_states), dim=1)
            prev_main_value_states = past_key_value[1].view(batch_size * self.num_attn_heads, -1, self.head_dim)
            main_value_states = mindspore.cat((prev_main_value_states, main_value_states), dim=1)
        ...
        # MAIN-STREAM
        # main attn weights
        main_attn_weights = mindspore.bmm(main_query_states, main_key_states.transpose(1, 2))
        ...
        main_attn_output = mindspore.bmm(main_attn_probs, main_value_states)
        ...
        # PREDICT-STREAM
        # [ngram, B*head, T, c]
        predict_query_states = mindspore.cat(predict_query_states_list, 0).view(
            self.ngram, -1, sequence_length, self.head_dim
        )
        # [ngram, B*head, 2*T, c]
        predict_key_states = mindspore.cat(
            [mindspore.cat([main_key_states, key], 1).unsqueeze(0) for key in predict_key_states_list], 0
        )

        # [ngram, T, B, C]
        predict_hidden_states = mindspore.cat(hidden_states_predict_list, 0).view(
            self.ngram, sequence_length, batch_size, hidden_size
        )

        # [ngram, B*head, 2*T, c]
        predict_value_states = mindspore.cat(
            [mindspore.cat([main_value_states, v_p], 1).unsqueeze(0) for v_p in predict_value_states_list], 0
        )
        # [ngram, B*head, T, 2*T]
        predict_attn_weights = mindspore.einsum("nbtc,nbsc->nbts", (predict_query_states, predict_key_states))
        ...
        # [ngram, B*head, T, c]
        predict_attn_output = mindspore.einsum("nbts,nbsc->nbtc", (predict_attn_probs, predict_value_states))
        ...
        # [B, 1+ngram*T, C]
        attn_output = mindspore.cat([main_attn_output, predict_attn_output], 1).view(batch_size, -1, hidden_size)
    ...
    def get_main_relative_pos_embeddings(
        ...
        if main_relative_position_buckets is None:
            ...
            relative_positions = (
                mindspore.arange(1, attn_weights.shape[-1] + 1)
                .unsqueeze(0)
                .unsqueeze(0)
        ...
        main_relative_pos_embeddings = mindspore.gather(
    ...
    def get_predict_relative_pos_embeddings(
        ...
        if predict_relative_position_buckets is None:
            ...
            relative_positions = (
                mindspore.arange(0, key_sequence_length)
                .unsqueeze(0)
                .unsqueeze(0)
                .repeat(batch_size, sequence_length, 1)
                .to(position_ids.device)
        ...
        predict_relative_pos_embeddings = mindspore.gather(
...
class ProphetNetEncoder(ProphetNetPreTrainedModel):
    r"""
    word_embeddings  (:obj:`mindspore.nn.Embeddings` of shape :obj:`(config.vocab_size, config.hidden_size)`, `optional`):
...
class ProphetNetDecoder(ProphetNetPreTrainedModel):
    ...
    def forward(
        ...
        r"""
        encoder_hidden_states  (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, sequence_length, hidden_size)`, `optional`):
            Sequence of hidden-states at the output of the last layer of the encoder. Used in the cross-attention if
            the model is configured as a decoder.
        encoder_attention_mask (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, sequence_length)`, `optional`):
        ...
        hidden_states = mindspore.cat([hidden_states] + ngram_hidden_states, 1)
    ...
    def compute_buffered_relative_buckets(self, position_ids):
        ...
        position_ids = mindspore.arange(1, self.max_target_positions).to(position_ids.device).repeat(1, 1)
        ...
        predict_relative_buckets = mindspore.cat(
            [
    ...
    def prepare_attention_mask(self, hidden_states, attention_mask):
        ...
        causal_mask = mindspore.triu(causal_mask, 1)
    ...
    def prepare_predict_attention_mask(self, hidden_states, attention_mask):
        ...
        predict_causal_mask = mindspore.cat(
            [
        ...
        if attention_mask is not None:
            ...
            extended_attention_mask = mindspore.cat(
                [extended_attention_mask, mindspore.zeros_like(extended_attention_mask)], dim=-1
            )
