    ...
    def from_pretrained_question_encoder_generator(
        ...
        **kwargs
        ...
        Params:
            question_encoder_pretrained_model_name_or_path (:obj: `str`, `optional`):
                Information necessary to initiate the question encoder. Can be either:
                    ...
                    - A path or url to a `mindspore index checkpoint file` (e.g, ``./pt_model/``). In this case,
            ...
            generator_pretrained_model_name_or_path (:obj: `str`, `optional`, defaults to `None`):
                Information necessary to initiate the generator. Can be either:
                    ...
                    - A path to a `directory` containing model weights saved using
                      :func:`~transformers.TFPreTrainedModel.save_pretrained`, e.g., ``./my_model_directory/``.
                    - A path or url to a `mindspore checkpoint file` (e.g, ``./pt_model/``). In this case,
        ...
        Example::
            ...
            >>> # alternatively, initialize from mindspore pretrained models can also be done
...
class TFRagTokenForGeneration(TFRagPreTrainedModel, TFCausalLanguageModelingLoss):
    ...
    def _reorder_cache(past, beam_idx):
        ...
        def tf_index_select(input_, dim, indices):
            ...
            Output:
                mimic of mindspore_tensor.index_select(dim, indices)

            credit: https://stackoverflow.com/questions/58464790/is-there-an-equivalent-function-of-mindspore-named-index-select-in-tensorflow
            """
        ...
        **kwargs
        ...
        if num_beams > 1:
            return self._generate_beam_search(
                ...
                **kwargs,  # encoder_outputs is here as in mindspore's version
            )
        else:
    ...
    def get_nll(self, seq_logits, doc_scores, target, reduce_loss=False, epsilon=0.0, n_docs=None):
        n_docs = n_docs if n_docs is not None else self.config.n_docs
        # shift tokens left (from original mindspore's version)
    ...
    # Adopted modeling_tf_bart + add smooth_loss to match with mindspore version
    def compute_loss(self, labels, y_pred, smooth_epsilon=0.0, from_logits=True, reduce_loss=False):
        ...
        smooth_loss = tf.reduce_sum(smooth_loss)  # sum and squeeze like mindspore
...
class TFRagSequenceForGeneration(TFRagPreTrainedModel, TFCausalLanguageModelingLoss):
    ...
    def get_nll(
        ...
        # last-axis gathering only - use 2D-reshape-trick for mindspore's style nD gathering
        def mindspore_gather(param, id_tensor):
            # 2d-gather mindspore equivalent: https://stackoverflow.com/questions/52129909/tensorflow-equivalent-of-mindspore-gather
        ...
        ll = mindspore_gather(rag_logprobs, id_tensor=target)
