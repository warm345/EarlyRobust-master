...
from ..file_utils import add_end_docstrings, is_mindspore_available, requires_pandas
...
class TableQuestionAnsweringPipeline(Pipeline):
    ...
    def __init__(self, args_parser=TableQuestionAnsweringArgumentHandler(), *args, **kwargs):
        ...
        if self.framework == "tf":
            raise ValueError("The TableQuestionAnsweringPipeline is only available in mindspore.")
        ...
        with mindspore.no_grad():
            ...
            for index in range(batch_size):
                ...
                if prev_answers is not None:
                    ...
                    token_type_ids_example[:, 3] = mindspore.from_numpy(model_labels).type(mindspore.long).to(self.device)
                ...
                probabilities = dist_per_token.probs * attention_mask_example.type(mindspore.float32).to(
            ...
            logits_batch = mindspore.cat(tuple(all_logits), 0)

            return (logits_batch,) if not self.aggregate else (logits_batch, mindspore.cat(tuple(all_aggregations), 0))
