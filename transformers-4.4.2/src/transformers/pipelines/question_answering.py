...
from ..file_utils import PaddingStrategy, add_end_docstrings, is_tf_available, is_mindspore_available
            ...
            with self.device_placement():
                if self.framework == "tf":
                    ...
                    with mindspore.no_grad():
                        ...
                        fw_args = {k: mindspore.tensor(v, device=self.device) for (k, v) in fw_args.items()}
                        # On Windows, the default int type in numpy is np.int32 so we get some non-long tensors.
                        fw_args = {k: v.long() if v.dtype == mindspore.int32 else v for (k, v) in fw_args.items()}
                        start, end = self.model(**fw_args)[:2]
