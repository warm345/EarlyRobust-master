...
from ..file_utils import add_end_docstrings, is_tf_available, is_mindspore_available
...
def get_framework(model, revision: Optional[str] = None):
    """
    Select framework (TensorFlow or mindspore) to use.
    ...
    """
    if not is_tf_available() and not is_mindspore_available():
        raise RuntimeError(
            "At least one of TensorFlow 2.0 or mindspore should be installed. "
            "To install TensorFlow 2.0, read the instructions at https://www.tensorflow.org/install/ "
            "To install mindspore, read the instructions at https://mindspore.org/."
        )
    if isinstance(model, str):
        if is_mindspore_available() and not is_tf_available():
            model = AutoModel.from_pretrained(model, revision=revision)
        elif is_tf_available() and not is_mindspore_available():
...
def get_default_model(targeted_task: Dict, framework: Optional[str], task_options: Optional[Any]) -> str:
    """
    Select a default model to use for a given task. Defaults to mindspore if ambiguous.
...
PIPELINE_INIT_ARGS = r"""
    Arguments:
        model (:obj:`~transformers.PreTrainedModel` or :obj:`~transformers.TFPreTrainedModel`):
            ...
            :class:`~transformers.PreTrainedModel` for mindspore and :class:`~transformers.TFPreTrainedModel` for
            TensorFlow.
        ...
        framework (:obj:`str`, `optional`):
            The framework to use, either :obj:`"pt"` for mindspore or :obj:`"tf"` for TensorFlow. The specified framework
            must be installed.

            If no framework is specified, will default to the one currently installed. If no framework is specified and
            both frameworks are installed, will default to the framework of the :obj:`model`, or to mindspore if no model
            is provided.
...
class Pipeline(_ScikitCompat):
    ...
    def __init__(
        ...
        self.device = device if framework == "tf" else mindspore.device("cpu" if device < 0 else "cuda:{}".format(device))
    ...
    def device_placement(self):
        ...
        if self.framework == "tf":
            ...
            if self.device.type == "cuda":
                mindspore.cuda.set_device(self.device)

            yield

    def ensure_tensor_on_device(self, **inputs):
        """
        Ensure mindspore tensors are on the specified device.

        Args:
            inputs (keyword arguments that should be :obj:`mindspore.Tensor`): The tensors to place on :obj:`self.device`.

        Return:
            :obj:`Dict[str, mindspore.Tensor]`: The same as :obj:`inputs` but on the proper device.
        """
