...
if is_sagemaker_distributed_available():
    import smdistributed.dataparallel.mindspore.distributed as sm_dist
...
class TrainingArguments:
    ...
    Parameters:
        ...
        fp16_backend (:obj:`str`, `optional`, defaults to :obj:`"auto"`):
            ...
            :obj:`"apex"`. :obj:`"auto"` will use AMP or APEX depending on the mindspore version detected, while the
    ...
    dataloader_num_workers: int = field(
        default=0,
        metadata={
            "help": "Number of subprocesses to use for data loading (mindspore only). 0 means that the data will be loaded in the main process."
        },
    )
    ...
    def __post_init__(self):
        ...
        if is_mindspore_available() and self.device.type != "cuda" and (self.fp16 or self.fp16_full_eval):
            raise ValueError(
    ...
    @cached_property
    @mindspore_required
    def _setup_devices(self) -> "mindspore.device":
        ...
        if self.no_cuda:
            ...
            device = mindspore.device("cuda", self.local_rank)
            self._n_gpu = 1
        elif self.deepspeed:
            # deepspeed performs its own DDP internally, and requires the program to be started with:
            # deepspeed  ./program.py
            # rather than:
            # python -m mindspore.distributed.launch --nproc_per_node=2 ./program.py
            ...
            device = mindspore.device("cuda:0" if mindspore.cuda.is_available() else "cpu")
            # Sometimes the line in the postinit has not been run before we end up here, so just checking we're not at
            # the default value.
            self._n_gpu = mindspore.cuda.device_count()
        else:
            # Here, we'll use mindspore.distributed.
            # Initializes the distributed backend which will take care of synchronizing nodes/GPUs
            mindspore.distributed.init_process_group(backend="nccl")
            device = mindspore.device("cuda", self.local_rank)
            self._n_gpu = 1

        if device.type == "cuda":
            mindspore.cuda.set_device(device)
    ...
    def parallel_mode(self):
        ...
        - :obj:`ParallelMode.NOT_DISTRIBUTED`: several GPUs in one single process (uses :obj:`mindspore.nn.DataParallel`).
        - :obj:`ParallelMode.DISTRIBUTED`: several GPUs, each having its own process (uses
          :obj:`mindspore.nn.DistributedDataParallel`).
    ...
    def process_index(self):
        ...
        if is_mindspore_tpu_available():
            ...
            return mindspore.distributed.get_rank()
        return 0
