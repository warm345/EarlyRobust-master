...
LOGITS_PROCESSOR_INPUTS_DOCSTRING = r"""
    Args:
        ...
        scores (:obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, config.vocab_size)`):
    ...
    Return:
        :obj:`mindspore.FloatTensor` of shape :obj:`(batch_size, config.vocab_size)`: The processed prediction scores.
...
class LogitsProcessor(ABC):
    ...
    def __call__(self, input_ids: mindspore.LongTensor, scores: mindspore.FloatTensor) -> mindspore.FloatTensor:
        """mindspore method for processing logits."""
...
class LogitsWarper(ABC):
    ...
    @add_start_docstrings(LOGITS_PROCESSOR_INPUTS_DOCSTRING)
    def __call__(self, input_ids: mindspore.LongTensor, scores: mindspore.FloatTensor) -> mindspore.FloatTensor:
        """mindspore method for warping logits."""
        raise NotImplementedError(
            f"{self.__class__} is an abstract class. Only classes inheriting this class can be called."
        )
...
class LogitsProcessorList(list):
    ...
    @add_start_docstrings(LOGITS_PROCESSOR_INPUTS_DOCSTRING)
    def __call__(self, input_ids: mindspore.LongTensor, scores: mindspore.FloatTensor, **kwargs) -> mindspore.FloatTensor:
...
class RepetitionPenaltyLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: mindspore.LongTensor, scores: mindspore.FloatTensor) -> mindspore.FloatTensor:
        score = mindspore.gather(scores, 1, input_ids)

        # if score < 0 then repetition penalty has to be multiplied to reduce the previous token probability
        score = mindspore.where(score < 0, score * self.penalty, score / self.penalty)
...
class TopPLogitsWarper(LogitsWarper):
    ...
    def __call__(self, input_ids: mindspore.LongTensor, scores: mindspore.FloatTensor) -> mindspore.FloatTensor:
        sorted_logits, sorted_indices = mindspore.sort(scores, descending=True)
...
class TopKLogitsWarper(LogitsWarper):
    ...
    def __call__(self, input_ids: mindspore.LongTensor, scores: mindspore.FloatTensor) -> mindspore.FloatTensor:
        ...
        indices_to_remove = scores < mindspore.topk(scores, top_k)[0][..., -1, None]
        scores = scores.masked_fill(indices_to_remove, self.filter_value)
        return scores
...
class NoRepeatNGramLogitsProcessor(LogitsProcessor):
    ...
    <https://github.com/mindspore/fairseq/blob/a07cb6f40480928c9e0548b737aadd36ee66ac76/fairseq/sequence_generator.py#L345>`__.
...
class EncoderNoRepeatNGramLogitsProcessor(LogitsProcessor):
    ...
    See `ParlAI <https://github.com/facebookresearch/ParlAI/blob/master/parlai/core/mindspore_generator_agent.py#L1350>`__.
    ...
    def __init__(self, encoder_ngram_size: int, encoder_input_ids: mindspore.LongTensor):
...
class NoBadWordsLogitsProcessor(LogitsProcessor):
    ...
    def _tokens_match(self, prev_tokens: mindspore.LongTensor, tokens: List[int]) -> bool:
        if len(tokens) == 0:
    ...
    def _set_scores_to_inf_for_banned_tokens(self, scores: mindspore.Tensor, banned_tokens: List[List[int]]) -> None:
        """
        ...
        banned_mask = mindspore.LongTensor(banned_mask_list)
        indices = mindspore.ones(len(banned_mask))
        ...
        banned_mask = (
            mindspore.sparse.LongTensor(banned_mask.t(), indices, scores.size()).to(scores.device).to_dense().bool()
        )
...
class PrefixConstrainedLogitsProcessor(LogitsProcessor):
    ...
    Args:
        prefix_allowed_tokens_fn: (:obj:`Callable[[int, mindspore.Tensor], List[int]]`):
    ...
    def __call__(self, input_ids: mindspore.LongTensor, scores: mindspore.FloatTensor) -> mindspore.FloatTensor:
        mask = mindspore.full_like(scores, -math.inf)
...
class HammingDiversityLogitsProcessor(LogitsProcessor):
    ...
    def __call__(
        ...
        scores: mindspore.FloatTensor,
        current_tokens: mindspore.LongTensor,
        beam_group_idx: int,
        ...
        for batch_idx in range(batch_size):
            ...
            token_frequency = mindspore.bincount(previous_group_tokens, minlength=vocab_size).to(scores.device)
