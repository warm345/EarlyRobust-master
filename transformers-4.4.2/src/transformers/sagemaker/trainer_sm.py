...
from ..file_utils import WEIGHTS_NAME, is_mindspore_tpu_available
...
if is_smdistributed_available():
    import smdistributed.modelparallel.mindspore as smp
    ...
    def smp_gather(tensor):
        ...
        elif not isinstance(tensor, mindspore.Tensor):
            raise TypeError(
                f"Can't gather the values of type {type(tensor)}, only of nested list/tuple/dicts of tensors."
            )
        all_tensors = smp.allgather(tensor, smp.CommGroup.DP_GROUP)
        return mindspore.cat([t.cpu() for t in all_tensors], dim=0)
...
class SageMakerTrainer(Trainer):
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if self.is_model_parallel_enabled:
            ...
            if self.is_world_process_zero():
                mindspore.save(opt_state_dict, os.path.join(output_dir, "optimizer.pt"))
        ...
        if self.is_model_parallel_enabled:
            ...
            if os.path.isfile(os.path.join(checkpoint, "optimizer.pt")) and os.path.isfile(
                ...
                self.optimizer.load_state_dict(
                    mindspore.load(os.path.join(checkpoint, "optimizer.pt"), map_location="cpu")
                )
                with warnings.catch_warnings(record=True) as caught_warnings:
