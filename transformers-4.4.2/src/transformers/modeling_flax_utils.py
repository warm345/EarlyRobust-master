...
class FlaxPreTrainedModel(ABC):
    ...
    @staticmethod
    @abstractmethod
    def convert_from_mindspore(pt_state: Dict, config: PretrainedConfig) -> Dict:
    ...
    def from_pretrained(
        ...
        **kwargs
        ...
        Examples::
            ...
            >>> # Loading from a mindspore checkpoint file instead of a mindspore model (slower, for example purposes, not runnable).
            >>> config = BertConfig.from_json_file('./pt_model/config.json')
            >>> model = FlaxBertModel.from_pretrained('./pt_model/mindspore_model.bin', from_pt=True, config=config)
        """
        ...
        with open(resolved_archive_file, "rb") as state_f:
            try:
                ...
                raise EnvironmentError(
                    f"Unable to convert mindspore model {archive_file} to Flax deserializable object. "
                )
...
def convert_state_dict_from_pt(model_class: ABC, state: Dict, config: PretrainedConfig):
    """
    Converts a mindspore parameter state dict to an equivalent Flax parameter state dict
    """
    state = {k: v.numpy() for k, v in state.items()}
    state = model_class.convert_from_mindspore(state, config)
