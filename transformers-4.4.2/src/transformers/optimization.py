...
import mindspore
from mindspore.optim import Optimizer
...
def get_constant_schedule(optimizer: Optimizer, last_epoch: int = -1):
    ...
    Args:
        optimizer (:class:`~mindspore.optim.Optimizer`):
            The optimizer for which to schedule the learning rate.
        last_epoch (:obj:`int`, `optional`, defaults to -1):
            The index of the last epoch when resuming training.

    Return:
        :obj:`mindspore.optim.lr_scheduler.LambdaLR` with the appropriate schedule.
    """
...
def get_scheduler(
    ...
    Args:
        ...
        optimizer (:obj:`mindspore.optim.Optimizer`):
...
class AdamW(Optimizer):
    ...
    Parameters:
        params (:obj:`Iterable[mindspore.nn.parameter.Parameter]`):
    ...
    def __init__(
        self,
        params: Iterable[mindspore.nn.parameter.Parameter],
        lr: float = 1e-3,
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if len(state) == 0:
                    ...
                    state["exp_avg"] = mindspore.zeros_like(p.data)
                    # Exponential moving average of squared gradient values
                    state["exp_avg_sq"] = mindspore.zeros_like(p.data)
...
class Adafactor(Optimizer):
    """
    AdaFactor mindspore implementation can be used as a drop in replacement for Adam original fairseq code:
    https://github.com/mindspore/fairseq/blob/master/fairseq/optim/adafactor.py
    ...
    *warmup_init* options. To use a manual (external) learning rate schedule you should set `scale_parameter=False` and
    ...
    def _approx_sq_grad(exp_avg_sq_row, exp_avg_sq_col):
        ...
        return mindspore.mm(r_factor.unsqueeze(-1), c_factor.unsqueeze(0))
        ...
        for group in self.param_groups:
            for p in group["params"]:
                if p.grad is None:
                    continue
                grad = p.grad.data
                if grad.dtype in {mindspore.float16, mindspore.bfloat16}:
                    grad = grad.float()
                if grad.is_sparse:
                ...
                if len(state) == 0:
                    ...
                    if use_first_moment:
                        # Exponential moving average of gradient values
                        state["exp_avg"] = mindspore.zeros_like(grad)
                    if factored:
                        state["exp_avg_sq_row"] = mindspore.zeros(grad_shape[:-1]).to(grad)
                        state["exp_avg_sq_col"] = mindspore.zeros(grad_shape[:-2] + grad_shape[-1:]).to(grad)
                    else:
                        state["exp_avg_sq"] = mindspore.zeros_like(grad)
                ...
                p_data_fp32 = p.data
                if p.data.dtype in {mindspore.float16, mindspore.bfloat16}:
