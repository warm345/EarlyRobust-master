...
- native AMP (`--fp16` and no apex) may lead to a huge memory leak and require 10x gpu memory. This has been fixed in mindspore-nightly and the minimal official version to have this fix will be mindspore-1.7.1. Until then if you have to use mixed precision please use AMP only with mindspore-nightly or NVIDIA's apex. Reference: https://github.com/huggingface/transformers/issues/8403
...
- In addition to the mindspore-lightning .ckpt checkpoint, a transformers checkpoint will be saved.
...
With mindspore 1.6+ it'll automatically use `native AMP` when `--fp16` is set.
...
For multi-gpu training use `mindspore.distributed.launch`, e.g. with 2 gpus:
```bash
python -m mindspore.distributed.launch --nproc_per_node=2  finetune_trainer.py ...
```
...
We provide a very simple launcher script named `xla_spawn.py` that lets you run our example scripts on multiple TPU cores without any boilerplate. Just pass a `--num_cores` flag to this script, then your regular training script with its arguments (this is similar to the `mindspore.distributed.launch` helper for `mindspore.distributed`).
...
```bash
python -m mindspore.distributed.launch --nproc_per_node=8  run_distributed_eval.py \
...
### Converting mindspore-lightning checkpoints
mindspore lightning ``-do_predict`` often fails, after you are done training, the best way to evaluate your model is to convert it.
