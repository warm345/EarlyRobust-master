        ...
        while True:
            ...
            for model_inputs, labels in pbar:
                ...
                for astep in range(args.adv_steps):
                    ...
                    # grad_list = [mindspore.reshape(m.slimming_coef.grad,(1,-1)) if (isinstance(m, BertSelfAttention) and m.self_slimming) or (isinstance(m, BertLayer) and m.inter_slimming) else None for m in model.modules()]
                    # for grad in grad_list:
                    #     if grad!=None:
                    #         if all_coef_concat==None:
                    #             all_coef_concat = grad
                    #         else:
                    #             all_coef_concat = mindspore.cat((all_coef_concat,grad),dim=1)
                    # writer.add_scalar(tag="Grad_norm/coef",scalar_value=mindspore.norm(all_coef_concat),global_step=global_step)
